...
bool Master::_StartDB()
{
    ...
    async_threads = uint8(sConfigMgr->GetIntDefault("FusionCMSDatabase.WorkerThreads", 1));
    if (async_threads < 1 || async_threads > 32)
    {
    ...
    synch_threads = uint8(sConfigMgr->GetIntDefault("FusionCMSDatabase.SynchThreads", 2));

    ///- Initialize the FusionCMS database
    if (!FusionCMSDatabase.Open(dbstring, async_threads, synch_threads))
    {
