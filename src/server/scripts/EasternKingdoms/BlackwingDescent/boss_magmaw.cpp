...
Position const IgnitionPositions[2][21] = {
    {
        // Left Side
        {-355.258f, -66.156f, 215.363f, 3.32963f},  {-349.104f, -57.5792f, 214.837f, 3.35319f},
        {-355.983f, -79.9935f, 213.749f, 3.91082f}, {-348.35f, -71.9141f, 213.26f, 3.96973f},
        {-341.228f, -63.2021f, 212.833f, 3.96973f}, {-335.636f, -53.1296f, 212.332f, 5.80755f},
        {-344.959f, -47.9226f, 212.061f, 6.07851f}, {-354.565f, -47.2949f, 213.04f, 3.04373f},
        {-350.721f, -88.5883f, 213.92f, 3.97443f},  {-342.783f, -80.5202f, 213.868f, 3.93124f},
        {-335.879f, -72.941f, 212.87f, 4.06083f},   {-330.037f, -64.3112f, 212.393f, 4.14329f},
        {-324.021f, -54.4161f, 211.863f, 4.19434f}, {-317.603f, -44.6127f, 211.952f, 0.96793f},
        {-329.399f, -43.7365f, 211.748f, 6.02982f}, {-341.139f, -38.8838f, 211.426f, 6.12014f},
        {-350.813f, -26.2997f, 211.345f, 2.15388f}, {-352.253f, -37.0172f, 211.603f, 2.95734f},
        {-338.237f, -27.1878f, 211.154f, 3.06337f}, {-327.528f, -32.3648f, 211.394f, 3.06337f},
        {-316.344f, -33.1654f, 211.428f, 3.17725f},
    },
    {
        // Right Side
        {-313.627f, -72.6881f, 213.266f, 1.00637f}, {-319.881f, -82.0775f, 213.552f, 4.27756f},
        {-306.833f, -83.4444f, 213.633f, 4.36788f}, {-302.725f, -74.0836f, 213.345f, 4.69382f},
        {-304.055f, -63.3109f, 212.826f, 4.57601f}, {-300.848f, -54.0836f, 212.39f, 4.18332f},
        {-308.972f, -52.744f, 212.326f, 5.62846f},  {-315.607f, -58.0178f, 212.578f, 1.02995f},
        {-320.357f, -65.927f, 212.802f, 1.0378f},   {-326.56f, -74.2884f, 213.145f, 0.841454f},
        {-333.371f, -83.2f, 213.706f, 0.566564f},   {-343.042f, -91.8511f, 213.916f, 3.12304f},
        {-334.444f, -92.7097f, 213.903f, 3.06413f}, {-324.424f, -93.0766f, 213.909f, 2.98166f},
        {-315.574f, -93.3725f, 213.924f, 3.12696f}, {-304.845f, -93.5295f, 213.919f, 3.10733f},
        {-296.142f, -88.6009f, 214.03f, 4.72919f},  {-292.752f, -78.8147f, 213.567f, 4.71348f},
        {-295.368f, -68.4218f, 213.065f, 4.71741f}, {-293.046f, -56.9843f, 212.531f, 4.62709f},
  ...
  public:
    ...
    struct boss_magmawAI : public BossAI
    {
        ...
        void Reset()
        {
            ...
            if (Creature* drakonicDrudge = me->FindNearestCreature(NPC_DRAKONID_DRUDGE, 100.0f, true))
            {
                drakonicDrudge->CastSpell(me, SPELL_CHAIN_L, true);
            }
        ...
        void UpdateAI(uint32 const diff)
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_PILLAR_OF_FLAME: {
                    ...
                    if (target)
                        me->SummonCreature(NPC_PILLAR_OF_FLAME, target->GetPositionX(), target->GetPositionY(),
                                           target->GetPositionZ(), 0.0f, TEMPSUMMON_TIMED_DESPAWN, 20000);
      ...
      private:
        inline void SummonLightShow()
        {
            ...
            for (uint8 i = 0; i < 21; i++)
                me->SummonCreature(NPC_ROOM_STALKER, IgnitionPositions[side][i].GetPositionX(),
                                   IgnitionPositions[side][i].GetPositionY(), IgnitionPositions[side][i].GetPositionZ(),
                                   IgnitionPositions[side][i].GetOrientation(), TEMPSUMMON_TIMED_DESPAWN, 3500);
        }
        bool below30;
        uint64 nefarianGUID;
        Vehicle* vehicle;
  ...
  public:
    ...
    struct npc_lava_parasiteAI : public ScriptedAI
    {
        ...
        void IsSummonedBy(Unit* summoner)
        {
            if (instance)
            {
                ...
                if (Unit* victim = SelectTarget(SELECT_TARGET_RANDOM, 0,
                                                ParasiteSelectorPredicate(RAID_MODE(78941, 91913, 94678, 94679))))
                {
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spellInfo)
        {
            ...
            {
                me->SetVisible(false); // related to the spell system we cannot despawn the parasite because the damage
                                       // aura does not work without the owner
                me->SetReactState(REACT_PASSIVE);
                me->CombatStop(true);
...
class spell_magmaw_parasitic_infection_aura : public SpellScriptLoader
{
  public:
    ...
    class spell_magmaw_parasitic_infection_aura_AuraScript : public AuraScript
    {
        ...
        void Register()
        {
            OnEffectRemove += AuraEffectRemoveFn(spell_magmaw_parasitic_infection_aura_AuraScript::OnRemove, EFFECT_0,
...
class spell_magmaw_parasitic_infection : public SpellScriptLoader
{
  public:
    ...
    class spell_magmaw_parasitic_infection_SpellScript : public SpellScript
    {
        ...
        {
            OnEffectHitTarget +=
                SpellEffectFn(spell_magmaw_parasitic_infection_SpellScript::HandleDummy, EFFECT_2, SPELL_EFFECT_DUMMY);
        }
    };
    ...
    class spell_nefarius_magmaw_shadow_breath_SpellScript : public SpellScript
    {
        ...
        void Register()
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
                spell_nefarius_magmaw_shadow_breath_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_DEST_AREA_ENEMY);
        }
    };
