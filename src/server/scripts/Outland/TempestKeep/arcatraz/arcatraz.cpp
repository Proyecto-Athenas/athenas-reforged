                ...
                if (!Init)
                {
                    if (EventProgress_Timer <= diff)
                    {
                        if (Phase < 8)
                        {
                            ...
                            case 7:
                            {
                                if (Player *player = me->FindNearestPlayer(100))
            ...
            void SummonedCreatureDies(Creature* summoned, Unit* who)
            {
                switch (summoned->GetEntry())
                {
                    ...
                    case ENTRY_PH_HUNTER:
                        instance->SetData(DATA_WARDEN_1, DONE);
                        break;
                    case ENTRY_AKKIRIS:
                    case ENTRY_SULFURON:
                        instance->SetData(DATA_WARDEN_3, DONE);
                        break;
                    case ENTRY_TW_DRAK:
                    case ENTRY_BL_DRAK:
                        instance->SetData(DATA_WARDEN_4, DONE);
                        break;
                    case ENTRY_SKYRISS:
                        instance->SetData(DATA_WARDEN_5, DONE);
                        break;
                }
            }
