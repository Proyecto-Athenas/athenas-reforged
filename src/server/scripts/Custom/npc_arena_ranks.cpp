...
enum ArenaRankActionIds {
    ...
    TOP_PVP_KILLS = GOSSIP_ACTION_INFO_DEF + 6,
};

enum ArenaRankOptions {
    ARENA_MAX_RESULTS = 12,
};

enum ArenaGossipText {
    ARENA_GOSSIP_HELLO = 11,
    ARENA_GOSSIP_NOTEAMS = 12,
    ARENA_GOSSIP_TEAMS = 13,
    ARENA_GOSSIP_TEAM_LOOKUP = 14,    
};
...
class ArenaTeamRanks : public CreatureScript
{
    ...
    public:
        ...
        bool OnGossipHello(Player *player, Creature *creature) {
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_BATTLE, "Rank 2v2", GOSSIP_SENDER_MAIN, ARENA_2V2_LADDER);
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_BATTLE, "Rank 3v3", GOSSIP_SENDER_MAIN, ARENA_3V3_LADDER);
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_BATTLE, "Rank 5v5", GOSSIP_SENDER_MAIN, ARENA_5V5_LADDER);
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_BATTLE, "Top PVP Kills", GOSSIP_SENDER_MAIN, TOP_PVP_KILLS);
        ...
        bool OnGossipSelect(Player* player, Creature* creature, uint32 /*uiSender*/, uint32 uiAction) {
            ...
            switch(uiAction) {
                ...
                case ARENA_3V3_LADDER:
                {
                    ...
                    if(!result) {
                        ...
                        do {
                            ...
                            buffer << rank << ". Rating [" << rating << "] " << name;
                            buffer << " (winner:" << seasonWins << " - loser:" << seasonLosses << ")";
                ...
                case TOP_PVP_KILLS:
                {
                    QueryResult result = CharacterDatabase.PQuery("SELECT name, class , totalKills FROM characters WHERE totalKills != 0 ORDER BY totalKills DESC LIMIT 5");
                    if(!result)
                    {
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Goodbye", GOSSIP_SENDER_MAIN, ARENA_GOODBYE);
                        player->SEND_GOSSIP_MENU(ARENA_GOSSIP_NOTEAMS, creature->GetGUID());
                    ...
                    {
                        uint32 totalKills, rank = 1;
                        std::string name, clases;

                        do
                        {
                            Field * fields = result->Fetch();
                            name = fields[0].GetString();
                            clases = classToString(fields[1].GetUInt8());
                            totalKills = fields[2].GetUInt32();
                            
                            std::stringstream tops;
                            tops << rank << ". Name: " << name;
                            tops << ", Class: " << clases << " - Kills:" << totalKills;
                            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, tops.str(), GOSSIP_SENDER_MAIN, ARENA_GOODBYE);
                ...
                default:
                {
                    if (uiAction > ARENA_START_TEAM_LOOKUP) 
                    {
                        ...
                        // no team found
                        if(!result) {
                            player->GetSession()->SendNotification("The arena team was not found.");
                            player->PlayerTalkClass->SendCloseGossip();
                            return true;
                        }
                        ...
                        buf.str("");
                        buf << "This Weeks: " << weekWins << "-" << weekLosses << " (" << weekWinPercentage << " win), " << (weekWins + weekLosses) << " played"; 
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, buf.str(), GOSSIP_SENDER_MAIN, parentOption);
                        buf.str("");
                        buf << "This Season: " << seasonWins << "-" << seasonLosses << " (" << seasonWinPercentage << " win), " << (seasonWins + seasonLosses) << " played"; 
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, buf.str(), GOSSIP_SENDER_MAIN, parentOption);
                        ...
                        if(!members) {
                            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "The team has no members.", GOSSIP_SENDER_MAIN, parentOption);
                        } else {
                            uint32 memberPos = 1;
                            ...
                            do {
                                ...
                                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, buf.str(), GOSSIP_SENDER_MAIN, parentOption);
                                buf.str("");
                                buf << "This Week: " << weekWins << "-" << weekLosses << " (" << weekWinPercentage << " win), " << (weekWins + weekLosses) << " played"; 
                                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, buf.str(), GOSSIP_SENDER_MAIN, parentOption);
                                buf.str("");
                                buf << "This Season: " << seasonWins << "-" << seasonLosses << " (" << seasonWinPercentage << " win), " << (seasonWins + seasonLosses) << " played"; 
                                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, buf.str(), GOSSIP_SENDER_MAIN, parentOption);
                        ...
                        buf.str("");
                        buf << "Return " << type << "v" << type << " rankings!";
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, buf.str(), GOSSIP_SENDER_MAIN, parentOption);
