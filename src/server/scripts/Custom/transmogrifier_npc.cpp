...
const char* GetSlotName(uint8 slot, WorldSession* /*session*/)
{
    ...
    case EQUIPMENT_SLOT_SHOULDERS:
        return "Shoulders - Hombreras";
    case EQUIPMENT_SLOT_BODY:
        return "Shirt - Camisa";
    case EQUIPMENT_SLOT_CHEST:
        return "Chest - Pechera";
    case EQUIPMENT_SLOT_WAIST:
        return "Waist - Cintura";
    case EQUIPMENT_SLOT_LEGS:
        return "Legs - Piernas";
    case EQUIPMENT_SLOT_FEET:
        return "Feet - Botas";
    case EQUIPMENT_SLOT_WRISTS:
        return "Wrists - Brazales";
    case EQUIPMENT_SLOT_HANDS:
        return "Hands - Manos";
    case EQUIPMENT_SLOT_BACK:
        return "Back - Capa";
    case EQUIPMENT_SLOT_MAINHAND:
        return "Main hand - Arma Derecha";
    case EQUIPMENT_SLOT_OFFHAND:
        return "Off hand - Arma Izquierda";
    case EQUIPMENT_SLOT_RANGED:
        return "Ranged - Arma de Rango";
    case EQUIPMENT_SLOT_TABARD:
        return "Tabard - Tabardo";
    default:
        return NULL;
    }
}
...
{
    return Class == ITEM_CLASS_WEAPON && (SubClass == ITEM_SUBCLASS_WEAPON_BOW || SubClass == ITEM_SUBCLASS_WEAPON_GUN || SubClass == ITEM_SUBCLASS_WEAPON_CROSSBOW);
}
...
bool CanTransmogrifyItemWithItem(Player* player, ItemTemplate const* target, ItemTemplate const* source)
{
    ...
    if (source->InventoryType == INVTYPE_BAG || source->InventoryType == INVTYPE_RELIC || source->InventoryType == INVTYPE_FINGER || source->InventoryType == INVTYPE_TRINKET ||
        source->InventoryType == INVTYPE_AMMO || source->InventoryType == INVTYPE_QUIVER)
        return false;

    if (target->InventoryType == INVTYPE_BAG || target->InventoryType == INVTYPE_RELIC || target->InventoryType == INVTYPE_FINGER || target->InventoryType == INVTYPE_TRINKET ||
        target->InventoryType == INVTYPE_AMMO || target->InventoryType == INVTYPE_QUIVER)
        return false;
    ...
    if (source->InventoryType != target->InventoryType)
    {
        ...
        if (source->Class == ITEM_CLASS_ARMOR &&
            !((source->InventoryType == INVTYPE_CHEST || source->InventoryType == INVTYPE_ROBE) && (target->InventoryType == INVTYPE_CHEST || target->InventoryType == INVTYPE_ROBE)))
            return false;
    }
    ...
    if (oldItem)
    {
        ...
        if (collectionCache.find(accountId) != collectionCache.end())
        {
            ...
            for (uint32 i = startValue; i <= endValue; i++)
            {
                ...
                player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, GetItemLink(newItem->GetEntry(), session), slot, newItem->GetEntry(),
                    "Using this item for transmogrify will bind it to you and make it non-refundable and non-tradeable.\nDo you wish to continue?\n\n" + GetItemLink(newItem->GetEntry(), session) +
                        ss.str(),
                    price, false);
            }
        }
...
class PS_Transmogrification : public PlayerScript
{
    ...
    void AddToDatabase(Player* player, ItemTemplate const* itemTemplate)
    {
        ...
        if (AddCollectedAppearance(accountId, itemId) && player->IsInWorld())
        {
            ChatHandler(player->GetSession())
                .PSendSysMessage(R"(|c%s|Hitem:%u:0:0:0:0:0:0:0:0|h[%s]|h|r %s)", itemQuality.c_str(), itemId, itemName.c_str(), "has been added to your appearance collection.");
  ...
  public:
    ...
    void OnEquip(Player* player, Item* it, uint8 /*bag*/, uint8 /*slot*/, bool /*update*/) override { AddToDatabase(player, it); }
};
