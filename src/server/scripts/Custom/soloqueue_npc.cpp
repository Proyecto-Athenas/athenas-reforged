...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        infoQueue << "Solo wins/games/rating: " << player->GetSoloWins() << "/" << player->GetSoloGames() << "/" << player->GetSoloRating() << "\n\n";
        ...
        else
        {
            if (ArenaTeam* at = sArenaTeamMgr->GetArenaTeamById(player->GetArenaTeamId(ArenaTeam::GetSlotByType(ARENA_TEAM_5v5))))
            {
                if (at->IsSoloQueueTeam())
                {
                    ...
                    {
                        if(sWorld->getBoolConfig(CONFIG_ARENA_SOLO_QUEUE_ENABLED_SKIRMISH))
                            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_INTERACT_1, "Queue up for 1vs1 Skirmish \n", GOSSIP_SENDER_MAIN, 20);
                        if(sWorld->getBoolConfig(CONFIG_ARENA_SOLO_QUEUE_ENABLED_RATING))
                            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_INTERACT_1, "Queue up for 1vs1 Rated \n", GOSSIP_SENDER_MAIN, 30);
                        if(sWorld->getBoolConfig(CONFIG_ARENA_SOLO_QUEUE_ENABLED_3v3))
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action)
    {
        ...
        switch (action)
        {
            case 1: // Create new solo queue team
            {
                ...
                // delete arena 1v1 ratinf if exist
                CharacterDatabase.PExecute("DELETE FROM `character_arena_solo` WHERE `guid`='%u'", player->GetGUID());  
                // create arena 1v1 rating
                CharacterDatabase.PExecute("INSERT INTO `character_arena_solo`(`guid`, `wins`, `games`, `rating`) VALUES ('%u', '0', '0', '0')", player->GetGUID());
                break;
            }
            ...
            case 3: // Leave Queue
            {
                ...
                else if (player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_1v1_RATED))
                {
                    if (Battleground* bg = sBattlegroundMgr->GetBattlegroundTemplate(BATTLEGROUND_AA))
                    {
                        WorldPacket data;
                        sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, bg, player, player->GetBattlegroundQueueIndex(BATTLEGROUND_QUEUE_1v1_RATED), STATUS_NONE, player->GetBattlegroundQueueJoinTime(BATTLEGROUND_AA), 0, ARENA_TYPE_1v1_RATED);
                        player->GetSession()->SendPacket(&data);
                    }

                    BattlegroundQueue& queue = sBattlegroundMgr->GetBattlegroundQueue(BATTLEGROUND_QUEUE_1v1_RATED);
                    queue.RemovePlayer(player->GetGUID(), false);
                    player->RemoveBattlegroundQueueId(BATTLEGROUND_QUEUE_1v1_RATED);
                }
    ...
    void Join1v1Queue(Player* player, bool rated = false)
    {
        ...
        ArenaType arenaType = rated ? ARENA_TYPE_1v1_RATED : ARENA_TYPE_1v1_SOLO;
        ...
        if (Battleground* bg = sBattlegroundMgr->GetBattlegroundTemplate(BATTLEGROUND_AA))
        {
            ...
            GroupQueueInfo* ginfo = bgQueue.AddGroup(player, NULL, bgTypeId, bracketEntry, arenaType, false, false, 0, matchmakerRating, ateamId);
