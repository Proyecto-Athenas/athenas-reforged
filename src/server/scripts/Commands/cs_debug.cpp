...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        static std::vector<ChatCommand> debugCommandTable =
        {
            ...
            { "joinrbg",        SEC_PLAYER,  false, &HandleDebugJoinRbgCommand,         "" },
            ...
            { "unroot",         SEC_MODERATOR,      false, &HandleDebugUnRootCommand,          "" },
            { "combat",         SEC_MODERATOR,      false, &HandleDebugCombatCommand,          "" },
        };
        static std::vector<ChatCommand> commandTable =
        {
            { "debug",          SEC_PLAYER,      true,  NULL,                  "", debugCommandTable },
            ...
            for (GroupReference* itr = grp->GetFirstMember(); itr != NULL; itr = itr->next())
            {
                ...
                if (sInfoMgr->GetCharInfo(GUID_LOPART(member->GetGUID()), info))
                {
                    if (!info.MMR[3])
                    {
                        ...
                        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_REP_CHARACTER_ARENA_STATS);
