...
public:
    ...
    {
        return ShutdownServer(args, 0, SHUTDOWN_EXIT_CODE, handler);
    }

    static bool HandleServerRestartCommand(ChatHandler* handler, char const* args)
    {
        return ShutdownServer(args, SHUTDOWN_MASK_RESTART, RESTART_EXIT_CODE, handler);
    }

    static bool HandleServerIdleRestartCommand(ChatHandler* handler, char const* args)
    {
        return ShutdownServer(args, SHUTDOWN_MASK_RESTART | SHUTDOWN_MASK_IDLE, RESTART_EXIT_CODE, handler);
    }

    static bool HandleServerIdleShutDownCommand(ChatHandler* handler, char const* args)
    {
        return ShutdownServer(args, SHUTDOWN_MASK_IDLE, SHUTDOWN_EXIT_CODE, handler);
    }
...
private:
    ...
    static bool ShutdownServer(char const* args, uint32 shutdownMask, int32 defaultExitCode, ChatHandler* handler = nullptr)
    {
        if (!*args)
            return false;
