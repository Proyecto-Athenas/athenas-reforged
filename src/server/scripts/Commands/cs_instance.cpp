...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        static std::vector<ChatCommand> instanceCommandTable =
        {
            ...
            { "setdata",        SEC_ADMINISTRATOR,  false,  &HandleInstanceSetDataCommand,      "" },
        };
    ...
    static bool HandleInstanceSetDataCommand(ChatHandler* handler, char const* args)
    {
        ...
        uint32 type = uint32(atoi(strtok((char*)args, " ")));
        uint32 data = uint32(atoi(strtok(NULL, " ")));

        if (!type || !data)
            return false;

        if (handler->GetSession()->GetPlayer()->GetInstanceScript())
            handler->GetSession()->GetPlayer()->GetInstanceScript()->SetData(type, data);
