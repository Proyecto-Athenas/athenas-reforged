...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        static std::vector<ChatCommand> commandTable =
        {
            ...
            { "movie",          SEC_MODERATOR,      false, &HandleMovieMakerCommand,           "" },
        };
        return commandTable;
    }
    ...
    static bool HandleMovieMakerCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (!*args)
        {
            handler->PSendSysMessage("Mode Movie Maker is '%s', Syntaxe .movie on/off", player->isMovieMaker() ? "activate" : "not activate");
            return true;
        }
        ...
        if (param == "on")
        {
            ...
            handler->GetSession()->GetPlayer()->UpdateTriggerVisibility();
            handler->PSendSysMessage("oh yeaaaaaahhhh !!! let's start for a fantastic video !");
            return true;
        }
        else if (param == "off")
        {
            ...
            handler->GetSession()->GetPlayer()->UpdateTriggerVisibility();
            handler->PSendSysMessage("movie maker mode is now desactivate, take care all trigger and invisible npcs will appear on you're screen.");
            return true;
        }

        handler->PSendSysMessage("Syntaxe .movie on/off");
        return false;
    }
};
