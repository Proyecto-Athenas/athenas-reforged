...
public:
    ...
    static bool HandleAnticheatJailCommand(ChatHandler* handler, const char* args)
    {
		...
		// freeze PJ and silence PJ
		pTarget->AddAura(9454, pTarget); // spell freeze
		pTarget->AddAura(39331, pTarget);// spell Game In Session - Quiet!Do not disturb the players...
		// teleport GM
        handler->GetSession()->GetPlayer()->TeleportTo(1,16224.4f,16403.2f,-64.5f,0.09f);
