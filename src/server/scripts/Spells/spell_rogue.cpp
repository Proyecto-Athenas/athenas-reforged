...
enum RogueSpells
{
    ...
    SPELL_ROGUE_ASSASSINS_RESOLVE = 84601
};
...
// 84601 - Assassins Resolve Aura

class spell_rog_assassins_resolve : public SpellScriptLoader
{
public:
    spell_rog_assassins_resolve() : SpellScriptLoader("spell_rog_assassins_resolve") {}

    class spell_rog_assassins_resolve_AuraScript : public AuraScript
    {
        ...
        void OnUpdateEffect(AuraEffect *aurEff, const uint32 diff)
        {
            aurEff->RecalculateAmount();
        }
        ...
        void CalcAmount(AuraEffect const *aurEff, int32 &amount, bool &canBeRecalculated)
        {
            ...
            {
                Item *mainHand = pl->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAINHAND);
                Item *offHand = pl->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND);

                if (mainHand && offHand &&
                    mainHand->GetTemplate()->SubClass == ITEM_SUBCLASS_WEAPON_DAGGER &&
                    offHand->GetTemplate()->SubClass == ITEM_SUBCLASS_WEAPON_DAGGER)
                {
                    amount = 20;
                }
                else
...
void AddSC_rogue_spell_scripts()
{
    ...
    new spell_rog_assassins_resolve();
}
