...
// Tol Barad Spell - 88341 for alliance
// Tol Barad Spell - 88339 for horde 
class spell_tol_barad_spell : public SpellScriptLoader
{
public:
    spell_tol_barad_spell() : SpellScriptLoader("spell_tol_barad_spell") { }

    class spell_tol_barad_spell_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_tol_barad_spell_SpellScript);

        void RelocateTransportOffset(SpellEffIndex /*effIndex*/)
        {
            if (!GetCaster() || GetCaster()->getClass() != CLASS_DRUID)
                return;

            // Flight form
            GetCaster()->RemoveAura(33943);
            // Swift flight Form
            GetCaster()->RemoveAura(40120);
        }

        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_tol_barad_spell_SpellScript::RelocateTransportOffset, EFFECT_0, SPELL_EFFECT_TELEPORT_UNITS);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_tol_barad_spell_SpellScript();
    }
};
