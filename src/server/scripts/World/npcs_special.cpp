...
// Paladin Guardian of Ancient Kings (Retribution Only)
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (Owner)
            {
                if (!me->GetAura(GOAK.ANCIENT_CRUZADER))
                {
                    ...
                    Owner->AddAura(GOAK.ANCIENT_CRUZADER, me);
                }

                Unit *ownerVictim = Owner->getVictim();
                meVictim = me->getVictim();

                if (meVictim && (Owner->GetGUID() == meVictim->GetGUID()) && meVictim->IsFriendlyTo(me))
                {
                    me->AttackStop();
                    return;
                }
...
public:
    ...
    struct npc_training_dummyAI : Scripted_NoMovementAI
    {
        ...
        void Reset()
        {
            ...
            me->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_KNOCK_BACK, true); // removido SPELL_EFFECT_KNOCK_BACK_DEST
