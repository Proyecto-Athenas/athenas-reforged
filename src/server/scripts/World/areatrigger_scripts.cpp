	...
	bool OnTrigger(Player* player, AreaTriggerEntry const* trigger) override
	{
		...
		case 5340:
			if (player->GetQuestStatus(QUEST_LAST_RITES) == QUEST_STATUS_INCOMPLETE ||
				...
				pPosition = WorldLocation(571, 3734.57f, 3563.76f, 341.675f);
			break;
		default:
			return false;
		}
...
enum K3TeleporterMisc
{
	QUEST_OPENING_THE_BACKDOOR = 12821,
	AT_K3_TO_STORMPEAKS = 5190,
	AT_STORMPEAKS_TO_K3 = 5187
};

class AreaTrigger_at_k3_teleporter : public AreaTriggerScript
{
public:
	AreaTrigger_at_k3_teleporter() : AreaTriggerScript("at_k3_teleporter") { }

	bool OnTrigger(Player* player, AreaTriggerEntry const* trigger) override
	{
		switch (trigger->id)
		{
		case AT_K3_TO_STORMPEAKS:
			if (player->isAlive() && player->GetQuestRewardStatus(QUEST_OPENING_THE_BACKDOOR))
				player->TeleportTo(571, 6315.429f, -1742.006f, 456.695f, 1.462f);
			break;
		case AT_STORMPEAKS_TO_K3:
			if (player->isAlive() && player->GetQuestRewardStatus(QUEST_OPENING_THE_BACKDOOR))
				player->TeleportTo(571, 6139.259f, -1068.124f, 402.662f, 2.666f);
			break;
		}
...
void AddSC_areatrigger_scripts()
{
    ...
    new AreaTrigger_at_k3_teleporter();
}
