    ...
    class spell_clean_stinky_bomb_SpellScript : public SpellScript
    {
        ...
        void HandleCleanBombEffect(SpellEffIndex effIndex)
        {
            ...
            for (GameObject* stinkyCloud : stinkyBombList)
            {
                if (stinkyCloud->GetDistance2d(dest->GetPositionX(), dest->GetPositionY()) <= 4.00f)
                {
					//stinkyCloud->SetGoState(GO_STATE_ACTIVE); // cambia de fase a los gobjects del mundo impidiendo el correcto funcionamiento de la misión
					stinkyCloud->SetGoState(GO_STATE_READY);
					stinkyCloud->SetLootState(GO_READY);
					stinkyCloud->UseDoorOrButton(10000, false, 0);
					//stinkyCloud->RemoveFromWorld(); //borra del juego los gobjets impidiendo el correcto funcionamiento de la misión
					stinkyCloud->ResetDoorOrButton();
...
public:
    ...
    struct npc_halloween_crone_broomAI : public ScriptedAI
    {
        ...
        void PassengerBoarded(Unit* passenger, int8 seat, bool apply) override
        {
            ...
            {
                if (isHordeBroom)
                    me->GetMotionMaster()->MovePoint(POINT_TELEPORT_TO_CITY, 1795.651f, 194.305f, 115.434f);
                else
        ...
        void DoAction(int32 const action)
        {
            if (action == ACTION_RETURN_HOME)
            {
                if (isHordeBroom)
                {
                    ...
                    me->NearTeleportTo(1658.896f, 285.613f, 173.039f, 5.910f, true);
                }
                else
                {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_RETURN_PATH:
                        for (int8 seat = 0; seat < 8; seat++)
                            if (Unit* passenger = me->GetVehicleKit()->GetPassenger(seat))
                                if (Player* player = passenger->ToPlayer())
                                {
                                    if (isHordeBroom)
                                    {
                                        Position pos(1809.278f, 181.477f, 116.0318f, 1.6290f);
                                        player->UpdatePosition(pos, true);
                                    }
                                    else
                                    {
                        ...
                        if (isHordeBroom)
                            me->GetMotionMaster()->MovePoint(POINT_DESPAWN, 1819.43f, 226.091f, 59.6967f);
                        else
