...
public:
    ...
    struct boss_alakirAI : public BossAI
    {
        ...
        void StartP3()
        {
            ...
            events.ScheduleEvent(EVENT_GONGJI, 9000, 0, PHASE_3);
        }
        ...
        void JustDied(Unit * /*killer*/)
        {
			...
			instance->CompleteGuildCriteriaForGuildGroup(CRITERIA_GUILD_RUN_ALAKIR_3);
            _JustDied();
        }
        ...
        void UpdateAI(const uint32 diff)
        {
            if (me->isInCombat())
            {
                if (!me->GetMap()->GetPlayersCountExceptGMs() || !CheckPlayersAlive())
                {
                    me->DeleteThreatList();
                    EnterEvadeMode();
                }
            ...
            if (IsHeroic() && (instance->GetBossState(DATA_CONCLAVE_OF_WIND_EVENT == DONE) || instance->GetBossState(DATA_CONCLAVE_OF_WIND_EVENT) == DONE_HM))                
                me->SetVisible(true);
