...
class boss_lord_rhyolith : public CreatureScript
{
    ...
    struct boss_lord_rhyolithAI : public BossAI
    {
        ...
        void Reset() override
        {
            ...
            // default speed from sniffs until reach lava pool
            me->SetSpeed(MOVE_RUN, 0.3f, true);
            me->SetSpeed(MOVE_WALK, 0.3f, true);

            // Vehicles required delay before install
            scheduler
                .Schedule(Milliseconds(1500), [this](TaskContext context)
            {
                if (Creature* leftFoot = me->SummonCreature(NPC_LEFT_FOOT, *me, TEMPSUMMON_MANUAL_DESPAWN))
                    leftFoot->EnterVehicle(me, 0);

                if (Creature* rightFoot = me->SummonCreature(NPC_RIGHT_FOOT, *me, TEMPSUMMON_MANUAL_DESPAWN))
