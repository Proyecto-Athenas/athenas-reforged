...
class boss_majordomo_staghelm : public CreatureScript
{
    ...
    struct boss_majordomo_staghelmAI : public BossAI
    {
        ...
        {
            // achievement: 5799 - Only the Penitent...
            instance->DoCompleteCriteria(17418);
            ...
            if (IsHeroic())
            {
                ...
                {
                    if (itr.getSource() && itr.getSource()->HasQuestForItem(ITEM_HEART_OF_FLAME))
                    {
                    ...
                    case EVENT_INTRO_CINEMATIC_START:
                    {
                        for (auto&& itr : instance->instance->GetPlayers())
                            if (Player* target = itr.getSource())
                                target->SendCinematicStart(MOVIE_ID_BRIDGE_INTRO);
...
struct npc_burning_orb : public ScriptedAI
{
    npc_burning_orb(Creature* creature) : ScriptedAI(creature) { }

    TaskScheduler scheduler;

    void Reset() override
    {
        DoCast(me, SPELL_BURNING_ORBS_VISUAL);
...
class spell_searing_seeds : public SpellScript
{
    PrepareSpellScript(spell_searing_seeds);
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_searing_seeds::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENEMY);
    }
};

class spell_searing_seeds_aura : public AuraScript
{
    PrepareAuraScript(spell_searing_seeds_aura);
    ...
    void Register() override
    {
        OnEffectApply += AuraEffectApplyFn(spell_searing_seeds_aura::OnApply, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY, AURA_EFFECT_HANDLE_REAL);
        AfterEffectRemove += AuraEffectRemoveFn(spell_searing_seeds_aura::OnRemove, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY, AURA_EFFECT_HANDLE_REAL);
    }
};

class spell_clump_check : public SpellScript
{
    PrepareSpellScript(spell_clump_check);
    ...
    void HandleEffect(SpellEffIndex /*effIndex*/)
    {
        if (Creature* caster = GetCaster()->ToCreature())
            caster->AI()->DoAction(scorpionForm ? ACTION_START_SCORPION_PHASE : ACTION_START_CAT_PHASE);
    }
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_clump_check::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENEMY_COUNT_TARGETS);
        OnEffectHit += SpellEffectFn(spell_clump_check::HandleEffect, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};

class spell_concentration : public AuraScript
{
    PrepareAuraScript(spell_concentration);
    ...
    void Register() override
    {
        DoCheckProc += AuraCheckProcFn(spell_concentration::CheckProc);
        OnEffectProc += AuraEffectProcFn(spell_concentration::OnProc, EFFECT_2, SPELL_AURA_MOD_THREAT);
        OnEffectPeriodic += AuraEffectPeriodicFn(spell_concentration::onPeriodicTick, EFFECT_1, SPELL_AURA_PERIODIC_DUMMY);
    }
};

class spell_leaping_flames : public SpellScript
{
    PrepareSpellScript(spell_leaping_flames);
    ...
    void Register() override
    {
        OnCast += SpellCastFn(spell_leaping_flames::HandleOnCast);
    }
};

class spell_leaping_flames_damage : public AuraScript
{
    PrepareAuraScript(spell_leaping_flames_damage);
    ...
    void Register() override
    {
        DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_leaping_flames_damage::HandleEffectCalcAmount, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE);
    }
};

class spell_flame_scythe : public SpellScript
{
    PrepareSpellScript(spell_flame_scythe);
    ...
    {
        if (energizeAmount > 0)
            GetCaster()->CastCustomSpell(GetCaster(), SPELL_REVITALIZE, &energizeAmount, 0, 0, true);
    }
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_flame_scythe::FilterTargets, EFFECT_0, TARGET_UNIT_CONE_ENEMY_54);
        AfterCast += SpellCastFn(spell_flame_scythe::RewardExtraEnergy);
    }
};
...
void AddSC_boss_majordomo_staghelm()
{
    ...
    new CreatureScriptLoaderEx<npc_burning_orb>("npc_burning_orb");

    new SpellScriptLoaderEx<spell_searing_seeds>("spell_searing_seeds");
    new AuraScriptLoaderEx<spell_searing_seeds_aura>("spell_searing_seeds_aura");
    new SpellScriptLoaderEx<spell_clump_check>("spell_clump_check");
    new AuraScriptLoaderEx<spell_concentration>("spell_concentration");
    new SpellScriptLoaderEx<spell_leaping_flames>("spell_leaping_flames");
    new AuraScriptLoaderEx<spell_leaping_flames_damage>("spell_leaping_flames_damage");
    new SpellScriptLoaderEx<spell_flame_scythe>("spell_flame_scythe");
}
