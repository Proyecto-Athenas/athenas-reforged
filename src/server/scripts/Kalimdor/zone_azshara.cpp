    ...
    bool OnGossipHello(Player *player, GameObject *go)
    {
        ...
        if (player->hasQuest(QUEST_H) || player->hasQuest(QUEST_A))
        {
            if (player->GetQuestStatus(QUEST_H) == QuestStatus::QUEST_STATUS_INCOMPLETE || player->GetQuestStatus(QUEST_A) == QuestStatus::QUEST_STATUS_INCOMPLETE)
            {
                ...
                int32 l_Entry = l_questTemplate->RequiredNpcOrGo[0];
                if (!l_Entry)
                    return false;

                player->KilledMonsterCredit(l_Entry);
                return true;
            }
        }
        else
...
void AddSC_azshara()
{
    new gameobject_thundermar_keg();
    new go_azshara_twilight_cage();
    new npc_azshara_malicion();
    new npc_azshara_twilight_lord_katrana();
}
