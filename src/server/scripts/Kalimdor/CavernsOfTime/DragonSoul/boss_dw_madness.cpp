...
public:
    ...
    struct npc_ds_thrall_madnessAI : public Scripted_NoMovementAI
    {
        ...
        void Reset() override
        {
            ...
            if (Creature *portal = me->FindNearestCreature(NPC_TRAVEL_TO_WYRMREST_BASE, 200.00f))
                portal->SetVisible(true);
            if (Creature *portal = me->FindNearestCreature(NPC_TRAVEL_TO_SKYFIRE_DECK, 200.0f))
                portal->SetVisible(true);
        }
        ...
        void sGossipSelect(Player * /*player*/, uint32 sender, uint32 action) override
        {
            ...
            {
                if (!IsHeroic() || (IsHeroic() && instance->IsDoneInHeroic(DATA_MORCHOK) && instance->IsDoneInHeroic(DATA_YORSAHJ) && instance->IsDoneInHeroic(DATA_WARLORD_ZONOZZ) && instance->IsDoneInHeroic(DATA_HAGARA) && instance->IsDoneInHeroic(DATA_ULTRAXION) && instance->IsDoneInHeroic(DATA_WARMASTER_BLACKHORN) && instance->IsDoneInHeroic(DATA_SPINE_OF_DEATHWING)))
                {
