...
std::vector<uint64> entrancePortals = {0, 0, 0, 0};
...
public:
    ...
    struct instance_dragonsoul_InstanceMapScript : public InstanceScript
    {
        ...
        void changeEntrancePortal()
        {
            // Deactivate all of them and then activate the correct one
            for (auto itr : entrancePortals)
                if (Creature *portal = instance->GetCreature(itr))
                    portal->SetVisible(false);

            if (IsDone(DATA_SPINE_OF_DEATHWING))
                currentPortal = entrancePortals[3];
            else if (IsDone(DATA_WARMASTER_BLACKHORN))
                currentPortal = entrancePortals[2];
            else if (IsDone(DATA_YORSAHJ) && IsDone(DATA_WARLORD_ZONOZZ) || (!IsDone(DATA_YORSAHJ) && !IsDone(DATA_WARLORD_ZONOZZ) && LfrId == LFR_PART_TWO))
            {
                currentPortal = entrancePortals[1];
                if (raidMode == RAID_MODE_LFR && LfrId == LFR_PART_ONE)
                    currentPortal = entrancePortals[0];
            }
            else if (IsDone(DATA_MORCHOK))
                currentPortal = entrancePortals[0];
            else
                currentPortal = 0;

            if (currentPortal != 0)
                if (Creature *portal = instance->GetCreature(currentPortal))
        ...
        void OnCreatureCreate(Creature *creature) override
        {
            ...
            case NPC_TRAVEL_TO_WYRMREST_TEMPLE:
                if (!IsDone(DATA_MORCHOK))
                    creature->SetVisible(false);
                npcPortals.emplace(DATA_MORCHOK, creature->GetGUID());
                // ugly but it works, idc
                if (int(creature->GetPositionX()) == -2262)
                    entrancePortals[0] = creature->GetGUID();
                // this is ugly too, but if the server restarts and all the players are inside, we will encounter with all the entrance portals visibles.
                changeEntrancePortal();
                break;
            case NPC_TRAVEL_TO_WYRMREST_SUMMIT: // taxi to ultraxion
                if (!IsDone(DATA_YORSAHJ) && !IsDone(DATA_WARLORD_ZONOZZ) && LfrId != LFR_PART_TWO)
                    creature->SetVisible(false);
                npcPortals.emplace(DATA_YORSAHJ, creature->GetGUID());
                if (int(creature->GetPositionX()) == -2262)
                    entrancePortals[1] = creature->GetGUID();
                changeEntrancePortal();
                break;
            case NPC_TRAVEL_TO_THE_MAELSTORM:
                if (!IsDone(DATA_SPINE_OF_DEATHWING))
                    creature->SetVisible(false);
                npcPortals.emplace(DATA_SPINE_OF_DEATHWING, creature->GetGUID());
                if (int(creature->GetPositionX()) == -2262)
                    entrancePortals[3] = creature->GetGUID();
                changeEntrancePortal();
                break;
            case NPC_TRAVEL_TO_SKYFIRE_DECK:
                if (!IsDone(DATA_WARMASTER_BLACKHORN))
                    creature->SetVisible(false);
                npcPortals.emplace(DATA_WARMASTER_BLACKHORN, creature->GetGUID());
                if (int(creature->GetPositionX()) == -2262)
                    entrancePortals[2] = creature->GetGUID();
                changeEntrancePortal();
                break;
            case NPC_TRAVEL_TO_EYE_OF_ETERNITY:
                // Activated by Kalecgos' gossip
                if (!IsDone(DATA_HAGARA))
                    creature->SetVisible(false);
                npcPortals.emplace(DATA_HAGARA, creature->GetGUID());
                break;
            case NPC_MORCHOK:
        ...
        void OnGameObjectCreate(GameObject *go) override
        {
            ...
            case GO_ALLIANCE_SHIP_BATTLE:
                if (IsDone(DATA_ULTRAXION))
                {
                    ...
                    instance->SummonCreature(NPC_KANU_REEVS, kanuReevsPos);
        ...
        void SetData(uint32 type, uint32 value) override
        {
            ...
            case DATA_DW_SPINE_ROLL_INFO:
            {
                ...
                if (rollInfo.size() == 5)
                    if (std::all_of(rollInfo.cbegin(), rollInfo.cend(), ([firstValue](std::pair<uint32, uint32> u)
                                                                         { return u.second == firstValue; })))
        ...
        bool SetBossState(uint32 type, EncounterState state) override
        {
            ...
            case DATA_MORCHOK:
                if (state == DONE || state == DONE_HM)
                {
                    ...
                    for (auto itr : npcPortals)
                        if (itr.first == DATA_MORCHOK)
                            if (Creature *portal = instance->GetCreature(itr.second))
                                portal->SetVisible(true);
            ...
            case DATA_WARMASTER_BLACKHORN:
                ...
                {
                    for (auto itr : npcPortals)
                        if (itr.first == DATA_WARMASTER_BLACKHORN)
                            if (Creature *portal = instance->GetCreature(itr.second))
                                portal->SetVisible(true);
                    changeEntrancePortal();
                }
