    ...
    bool OnGossipHello(Player* pPlayer, Creature* pCreature)
    {
        ...
        if (InstanceScript* pInstance = pCreature->GetInstanceScript())
        {
            ...
            case NPC_NETHESTRASZ:
                if (pInstance->GetBossState(DATA_YORSAHJ) == DONE && pInstance->GetBossState(DATA_WARLORD_ZONOZZ) == DONE)
                    pPlayer->NearTeleportTo(teleportPos[3].GetPositionX(), teleportPos[3].GetPositionY(), teleportPos[3].GetPositionZ(), teleportPos[3].GetOrientation());
                break;
            ...
            case NPC_TRAVEL_TO_SKYFIRE_DECK:
                if (pInstance->GetBossState(DATA_WARMASTER_BLACKHORN) == DONE && pInstance->GetBossState(DATA_ULTRAXION) == DONE)
			...
			pPlayer->CastSpell(pPlayer, 49249, true); // desmontar a los PJ a la fuerza de cualquier montura
        }
        return true;
    }
};
