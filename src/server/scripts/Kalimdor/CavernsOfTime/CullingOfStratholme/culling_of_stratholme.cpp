...
public:
    ...
    struct npc_arthasAI : public npc_escortAI
    {
        ...
        {
            //if (instance) // el reinicio de la mazmorra no est� funcionando correctamente
        ...
        void WaypointReached(uint32 waypointId)
        {
            switch (waypointId)
            {
                ...
                case 22:
                case 23:
                //case 26: // est� linea estaba efectuando un ciclo de redundancia extra lo cual ocasiona problemas
                case 55:
                case 56:
                    SetHoldState(true);
                    bStepping = true;
                ...
                if (phaseTimer <= diff)
                {
                    switch (step)
                    {
                        ...
                        case 87:
                            if (Creature* malganis = Unit::GetCreature(*me, malganisGUID))
                            {
                                malganis->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_UNK_6 | UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_SWIMMING);
