...
enum DigSiteDistance
{
    ...
    ARCHAEOLOGY_DIG_SITE_TOO_FAR_DIST      = 500
};
...
class ArcheologyMgr
{
    public:
        ...
        void SetActualDigSitePosition(bool force = false);
        ...
        void LoadArcheologyDigSites(PreparedQueryResult digsiteResult, PreparedQueryResult currentArtifactsResult, PreparedQueryResult researchResult);
        void SaveArcheology();
        ...
        void UpdateWrongSkillDBValues(uint32 skillReq);

        std::map<uint32, ResearchHistoryEntry >* GetReasearchHistory() { return &m_researchHistory; }

    private:
        Player*    m_player;
        uint32     m_digSites[16];
        uint32     m_digSitesCount[16];
        uint32     m_researchProject[28];
        Position   m_actualDigPos;
        uint8      m_currentDigSite;
