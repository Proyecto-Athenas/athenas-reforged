...
enum Opcodes
{
    ...
    CMSG_BATTLEFIELD_LEAVE_BATTLE = 0x4B34,
...
{
    OpcodeHandler() {}
    OpcodeHandler(char const* _name, SessionStatus _status, PacketProcessing _processing, pOpcodeHandler _handler) : Name(_name), Status(_status), ProcessingPlace(_processing), Handler(_handler) {}
...
{
  public:
    OpcodeTable() { memset(_internalTable, 0, sizeof(_internalTable)); }
    ...
    OpcodeHandler const* operator[](uint32 index) const { return _internalTable[index]; }

  private:
    template <bool isInValidRange, bool isNonZero> void ValidateAndSetOpcode(uint16 opcode, char const* name, SessionStatus status, PacketProcessing processing, pOpcodeHandler handler);
