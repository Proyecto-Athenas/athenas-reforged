...
void WorldPacket::Compress(z_stream* compressionStream)
{
    ...
    {
        TC_LOG_ERROR("network.opcode", "Packet with opcode 0x%04X is already compressed!", uncompressedOpcode);
        return;
    }
    ...
    *this << uint32(size);
    ...
    SetOpcode(opcode);
    TC_LOG_INFO("network.opcode", "%s (len %u) successfully compressed to %04X (len %u)", GetOpcodeNameForLogging(uncompressedOpcode).c_str(), size, opcode, destsize);
}
...
void WorldPacket::Compress(void* dst, uint32 *dst_size, const void* src, int src_size)
{
    ...
    if (z_res != Z_OK)
    {
        TC_LOG_ERROR("network.opcode", "Can't compress packet (zlib: deflate) Error code: %i (%s, msg: %s)", z_res, zError(z_res), _compressionStream->msg);
        *dst_size = 0;
        return;
    }
    ...
    {
        TC_LOG_ERROR("network.opcode", "Can't compress packet (zlib: deflate not greedy)");
        *dst_size = 0;
        return;
    }
