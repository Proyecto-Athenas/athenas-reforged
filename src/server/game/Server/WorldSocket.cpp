...
struct ServerPktHeader
{
    ...
    ServerPktHeader(uint32 size, uint16 cmd) : size(size)
    {
        ...
        if (isLargePacket())
        {
            TC_LOG_DEBUG("network.opcode", "initializing large server to client packet. Size: %u, cmd: %u", size, cmd);
...
int WorldSocket::SendPacket(WorldPacket const& pct)
{
    ...
    else
    {
        ...
        {
            TC_LOG_ERROR("network.opcode", "WorldSocket::SendPacket enqueue_tail failed");
            mb->release();
            return -1;
        }
    }
...
int WorldSocket::open (void *a)
{
    ...
    {
        TC_LOG_ERROR("network.opcode", "WorldSocket::open: peer().get_remote_addr errno = %s", ACE_OS::strerror (errno));
        return -1;
    }
    ...
    {
        TC_LOG_ERROR("network.opcode", "WorldSocket::open: unable to register client handler errno = %s", ACE_OS::strerror (errno));
        return -1;
    }
...
int WorldSocket::handle_input (ACE_HANDLE)
{
    ...
    switch (handle_input_missing_data())
    {
        case -1 :
        {
            ...
            TC_LOG_DEBUG("network.opcode", "WorldSocket::handle_input: Peer error closing connection errno = %s", ACE_OS::strerror (errno));
        ...
        case 0:
        {
            TC_LOG_DEBUG("network.opcode", "WorldSocket::handle_input: Peer has closed connection");
...
int WorldSocket::handle_output_queue (GuardType& g)
{
    ...
    {
        TC_LOG_ERROR("network.opcode", "WorldSocket::handle_output_queue dequeue_head");
        return -1;
    }
    ...
    else if (n < (ssize_t)send_len) //now n > 0
    {
        ...
        {
            TC_LOG_ERROR("network.opcode", "WorldSocket::handle_output_queue enqueue_head");
            mblk->release();
            return -1;
        }
...
int WorldSocket::handle_input_header (void)
{
    ...
    if ((header.size < 4) || (header.size > 10240) || (header.cmd > 0xFFFF && (header.cmd >> 16) != 0x4C52))  // LR (from MSG_VERIFY_CONNECTIVITY)
    {
        ...
        TC_LOG_ERROR("network.opcode", "WorldSocket::handle_input_header(): client (account: %u, char [GUID: %u, name: %s]) sent malformed packet (size: %d, cmd: %d)",
...
int WorldSocket::handle_input_missing_data (void)
{
    ...
    while (message_block.length() > 0)
    {
        ...
        if (!m_RecvWPct)
        {
            TC_LOG_ERROR("network.opcode", "Forcing close on input m_RecvWPct = NULL");
            errno = EINVAL;
            return -1;
        }
...
int WorldSocket::cancel_wakeup_output (GuardType& g)
{
        ...
        (this, ACE_Event_Handler::WRITE_MASK) == -1)
    {
        ...
        TC_LOG_ERROR("network.opcode", "WorldSocket::cancel_wakeup_output");
        return -1;
    }
...
int WorldSocket::schedule_wakeup_output (GuardType& g)
{
    ...
    if (reactor()->schedule_wakeup
        (this, ACE_Event_Handler::WRITE_MASK) == -1)
    {
        TC_LOG_ERROR("network.opcode", "WorldSocket::schedule_wakeup_output");
        return -1;
    }
...
int WorldSocket::ProcessIncoming(WorldPacket* new_pct)
{
    ...
    try
    {
        switch (opcode)
        {
            ...
            case CMSG_AUTH_SESSION:
                if (m_Session)
                {
                    TC_LOG_ERROR("network.opcode", "WorldSocket::ProcessIncoming: received duplicate CMSG_AUTH_SESSION from %s", m_Session->GetPlayerInfo().c_str());
                    return -1;
                }
            ...
            case CMSG_KEEP_ALIVE:
                TC_LOG_DEBUG("network.opcode", "%s", opcodeName.c_str());
                sScriptMgr->OnPacketReceive(this, WorldPacket(*new_pct));
                return 0;
            ...
            default:
            {
                ...
                {
                    TC_LOG_INFO("misc", "STATUS_UNHANDLED: %s (len: %u)", GetOpcodeNameForLogging(new_pct->GetOpcode()).c_str(), new_pct->size());
    ...
    {
        TC_LOG_INFO("misc", "EXCEPTION: %s (len: %u)", GetOpcodeNameForLogging(new_pct->GetOpcode()).c_str(), new_pct->size());
        TC_LOG_ERROR("network.opcode", "WorldSocket::ProcessIncoming ByteBufferException occured while parsing an instant handled packet %s from client %s, accountid=%i. Disconnected client.",
...
int WorldSocket::HandleAuthSession(WorldPacket& recvPacket)
{
    ...
    if (sWorld->IsClosed())
    {
        ...
        TC_LOG_ERROR("network.opcode", "WorldSocket::HandleAuthSession: World closed, denying client (%s).", GetRemoteAddress().c_str());
        return -1;
    }
    ...
    if (!result)
    {
        ...
        TC_LOG_ERROR("network.opcode", "WorldSocket::HandleAuthSession: Sent Auth Response (unknown account).");
        return -1;
    }
    ...
    TC_LOG_DEBUG("network.opcode", "WorldSocket::HandleAuthSession: (s, v) check s: %s v: %s",
        fields[5].GetCString(),
        fields[4].GetCString());
    ...
    if (fields[3].GetUInt8() == 1) // if ip is locked
    {
        if (strcmp (fields[2].GetCString(), GetRemoteAddress().c_str()))
        {
            ...
            TC_LOG_DEBUG("network.opcode", "WorldSocket::HandleAuthSession: Sent Auth Response (Account IP differs).");
            return -1;
        }
    }
    ...
    {
        SendAuthResponseError(AUTH_REJECT);
        TC_LOG_ERROR("network.opcode", "WorldSocket::HandleAuthSession: Client %s attempted to log in using invalid client OS (%s).", GetRemoteAddress().c_str(), os.c_str());
        return -1;
        }*/
    ...
    if (banresult) // if account banned
    {
        ...
        TC_LOG_ERROR("network.opcode", "WorldSocket::HandleAuthSession: Sent Auth Response (Account banned).");
        return -1;
    }

    // Check locked state for server
    AccountTypes allowedAccountType = sWorld->GetPlayerSecurityLimit();
    TC_LOG_DEBUG("network.opcode", "Allowed Level: %u Player Level %u", allowedAccountType, AccountTypes(security));
    if (allowedAccountType > SEC_PLAYER && AccountTypes(security) < allowedAccountType)
    {
        SendAuthResponseError(AUTH_UNAVAILABLE);
        TC_LOG_INFO("network.opcode", "WorldSocket::HandleAuthSession: User tries to login but his security level is not enough");
        return -1;
    }
    ...
    {
        SendAuthResponseError(AUTH_FAILED);
        TC_LOG_ERROR("network.opcode", "WorldSocket::HandleAuthSession: Authentication failed for account: %u ('%s') address: %s", id, account.c_str(), address.c_str());
        return -1;
    }

    TC_LOG_DEBUG("network.opcode", "WorldSocket::HandleAuthSession: Client '%s' authenticated successfully from %s.",
        account.c_str(),
        address.c_str());
...
int WorldSocket::HandlePing (WorldPacket& recvPacket)
{
    ...
    else
    {
        ...
        if (diff_time < ACE_Time_Value (27))
        {
            ...
            if (max_count && m_OverSpeedPings > max_count)
            {
                ...
                {
                    TC_LOG_ERROR("network.opcode", "WorldSocket::HandlePing: %s kicked for over-speed pings (address: %s)",
    ...
    // critical section
    {
        ...
        {
            TC_LOG_ERROR("network.opcode", "WorldSocket::HandlePing: peer sent CMSG_PING, "
