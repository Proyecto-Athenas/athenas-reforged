...
namespace Trinity
{
    ...
    {
        public:
            AnyFriendlyUnitInObjectRangeCheck(WorldObject const* obj, Unit* funit, float range, ConditionList* condList = NULL) :
                i_obj(obj), i_funit(funit), i_range(range), _condList(condList)
            {
                if (condList)
                    _condSrcInfo = new ConditionSourceInfo(NULL, funit);
                else
                    _condSrcInfo = NULL;
            }
            ...
            bool operator()(Unit* u)
            {
                ...
                if (!_condSrcInfo)
                    return true;
                _condSrcInfo->mConditionTargets[0] = u;
    ...
    class AnyGroupedUnitInObjectRangeCheck
    {
        public:
            ...
            bool operator()(Unit* u)
            {
                if (!u || !_refUnit || !_source)
                    return false;

                if (!u->IsInWorld() || !_refUnit->IsInWorld() || !_source->IsInWorld())
                    return false;
    ...
    {
        public:
            AnyAoETargetUnitInObjectRangeCheck(WorldObject const* obj, Unit* funit, float range, ConditionList* condList = NULL)
                : i_obj(obj), i_funit(funit), _spellInfo(NULL), i_range(range), _condList(condList)
            {
                Unit* check = i_funit;
                ...
                if (condList)
                    _condSrcInfo = new ConditionSourceInfo(NULL, check);
                else
                    _condSrcInfo = NULL;
            }
    ...
    */
    ...
    class NearestCreatureDistinctEntryWithLiveStateInObjectRangeCheck
    {
        public:
            NearestCreatureDistinctEntryWithLiveStateInObjectRangeCheck(WorldObject const& obj, uint32 entry, bool alive, float range)
                : i_obj(obj), i_entry(entry), i_alive(alive), i_range(range) {}

            bool operator()(Creature* u)
            {
                if (u->GetEntry() == i_entry && u->isAlive() == i_alive && i_obj.IsWithinDistInMap(u, i_range) && u->GetGUID() != i_obj.GetGUID())
                {
                    i_range = i_obj.GetDistance(u);         // use found unit range as new range limit for next check
                    return true;
                }
        ...
        private:
            ...
            // prevent clone this object
            NearestCreatureDistinctEntryWithLiveStateInObjectRangeCheck(NearestCreatureDistinctEntryWithLiveStateInObjectRangeCheck const&);
    };

    class AnyPlayerInObjectRangeCheck
    {
        public:
            AnyPlayerInObjectRangeCheck(WorldObject const* obj, float range, bool reqAlive = true, bool withGamemaster = false) : _obj(obj), _range(range), _reqAlive(reqAlive), _withGamemaster(withGamemaster) {}
            bool operator()(Player* u)
            {
                if ((_reqAlive && !u->isAlive()) || (!_withGamemaster && u->isGameMaster()))
                    return false;
        ...
        private:
            ...
            float _range;
            bool _reqAlive;
            bool _withGamemaster;
    };

    class NearestPlayerInObjectRangeCheck
    {
        public:
            NearestPlayerInObjectRangeCheck(WorldObject const* obj, float range, int aura = 0) : i_obj(obj), i_range(range), i_aura(aura)
            {
            }

            bool operator()(Player* u)
            {
                bool present = i_aura < 0 ? false : true;
                if (!u->isGameMaster() && u->isAlive() && i_obj->IsWithinDistInMap(u, i_range) && (!i_aura || u->HasAura(std::abs(i_aura)) == present))
                {
