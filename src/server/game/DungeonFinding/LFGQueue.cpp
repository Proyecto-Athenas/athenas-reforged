...
LfgCompatibility LFGQueue::CheckCompatibility(LfgGuidList check)
{
    ...
    for (LfgGuidList::const_iterator it = check.begin(); it != check.end() && numLfgGroups <= 1 && numPlayers <= dungeon->GetMaxGroupSize(); ++it)
    {
        uint64 guid = (*it);
        LfgQueueDataContainer::iterator itQueue = QueueDataStore.find(guid);
        if (itQueue == QueueDataStore.end())
    ...
    else
    {
        ...
        LFGMgr::CheckGroupRoles(proposalRoles, dungeon);    // assing new roles
    }
...
std::string LFGQueue::DumpQueueInfo() const
{
    ...
    for (uint8 i = 0; i < 2; ++i)
    {
        ...
        for (LfgGuidList::const_iterator it = queue.begin(); it != queue.end(); ++it)
        {
            ...
            {
                players++;
                uint8 role = sLFGMgr->GetRoles(guid);
                role &= ~PLAYER_ROLE_LEADER;
...
void LFGQueue::UpdateBestCompatibleInQueue(LfgQueueDataContainer::iterator itrQueue, std::string const& key, LfgRolesMap const& roles)
{
    ...
    for (LfgRolesMap::const_iterator it = roles.begin(); it != roles.end(); ++it)
    {
        ...
        if (role & PLAYER_ROLE_TANK && queueData.tanks)
            --queueData.tanks;
        else if (role & PLAYER_ROLE_HEALER && queueData.healers)
            --queueData.healers;
