...
enum LfgExpType
{
    LFG_RANDOM_NORMAL_CATACLYSM = 300,
    LFG_RANDOM_HEROIC_CATACLYSM = 301,
    LFG_RANDOM_HEROIC_CATACLYSM_T2 = 434,
};
...
typedef std::map<uint32/*pair32(map, diff)*/, uint32> LfgMapToDungeonId;
...
struct LfgRoleCheck
{
    ...
    bool isBeginning;                                      ///< Helper for role check update opcode
};
...
struct LFGDungeonData
{
    ...
    LFGDungeonData(LFGDungeonEntry const* dbc): id(dbc->ID), name(dbc->name), map(dbc->map),
        ...
        x(0.0f), y(0.0f), z(0.0f), o(0.0f), neededILevel(0),
        requiredTanks(dbc->RequiredTanks), requiredHeals(dbc->RequiredHeals),
    ...
    uint32 requiredHeals;
    uint32 requiredDamagedealers;

    // Helpers
    uint32 Entry() const { return id + (type << 24); }
    uint32 GetMaxGroupSize() const { return requiredTanks + requiredHeals + requiredDamagedealers; }
};
...
class LFGMgr
{
    ...
    public:
        ...
        bool IsPermanentLFGDungeon(uint64 guid) const;
        // Multiple files
        ...
        /// Get current vote kick cooldown state
        bool IsKickCooldownActive(uint64 gguid);
        /// Get current vote kick cooldown timer
        uint32 GetVoteKickCooldownTimer(uint64 gguid);
        /// Start a new votkick cooldown
        void SetVoteKickCooldown(uint64 guid, uint32 cooldownTime);
        /// Get current dungeon
        ...
        uint32 GetLfgMapDungeonId(uint32 mapId, uint32 difficulty);
        bool IsCallToArmEligible(uint32, uint32);
    ...
    private:
        ...
        LfgMapToDungeonId MapToDungeonIdStore;
