...
LfgGroupData::LfgGroupData(): m_State(LFG_STATE_NONE), m_OldState(LFG_STATE_NONE),
    m_Leader(0), m_Dungeon(0), m_KicksLeft(LFG_GROUP_MAX_KICKS), m_VoteKickActive(false), m_VoteKickCooldown(time(NULL) + (15 * MINUTE))
{ }
...
uint32 LfgGroupData::GetVoteKickCooldown() const
{
    return m_VoteKickCooldown;
}

void LfgGroupData::UpdateVoteKickCooldown(uint32 cooldown)
{
    m_VoteKickCooldown = cooldown;
}
