...
{
    return (Effect == SPELL_EFFECT_SUMMON_PLAYER) || (Effect == SPELL_EFFECT_SUMMON_RAF_FRIEND) || (Effect == SPELL_EFFECT_RESURRECT) || (Effect == SPELL_EFFECT_RESURRECT_NEW) ||
           (Effect == SPELL_EFFECT_SKIN_PLAYER_CORPSE);
}

bool SpellEffectInfo::IsFarDestTargetEffect() const { return Effect == SPELL_EFFECT_TELEPORT_UNITS; }

bool SpellEffectInfo::IsUnitOwnedAuraEffect() const { return IsAreaAuraEffect() || Effect == SPELL_EFFECT_APPLY_AURA; }
...
int32 SpellEffectInfo::CalcValue(Unit const* caster, int32 const* bp, Unit const* target) const
{
        ...
        if (caster)
        {
            ...
            if (GtSpellScalingEntry const* gtScaling = sGtSpellScalingStore.LookupEntry((_spellInfo->ScalingClass != -1 ? _spellInfo->ScalingClass - 1 : MAX_CLASSES - 1) * 100 + level - 1))
            {
                ...
                if (_spellInfo->CastTimeMax > 0 && _spellInfo->CastTimeMaxLevel > level)
                    multiplier *=
                        float(_spellInfo->CastTimeMin + (level - 1) * (_spellInfo->CastTimeMax - _spellInfo->CastTimeMin) / (_spellInfo->CastTimeMaxLevel - 1)) / float(_spellInfo->CastTimeMax);
    ...
    if (caster)
    {
        ...
        if ((!caster->IsControlledByPlayer() || caster->GetEntry() == 15438) && !_spellInfo->GetSpellScaling() && !basePointsPerLevel &&
            _spellInfo->Attributes & SPELL_ATTR0_LEVEL_DAMAGE_CALCULATION && _spellInfo->SpellLevel != caster->getLevel())
        {
...
bool SpellEffectInfo::HasRadius() const { return RadiusEntry != NULL; }

bool SpellEffectInfo::HasMaxRadius() const { return MaxRadiusEntry != NULL; }
...
bool SpellInfo::IsLootCrafting() const
{
    return (Effects[0].Effect == SPELL_EFFECT_CREATE_RANDOM_ITEM ||
            ...
            (Effects[0].Effect == SPELL_EFFECT_CREATE_ITEM_2 && (TotemCategory[0] != 0 || Effects[0].ItemType == 0)));
}
...
bool SpellInfo::NeedsExplicitUnitTarget() const { return GetExplicitTargetMask() & TARGET_FLAG_UNIT_MASK; }
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        if (effectMask & (1 << i) && Effects[i].IsEffect())
        {
            if (Effects[i].TargetA.GetSelectionCategory() == TARGET_SELECT_CATEGORY_CHANNEL || Effects[i].TargetB.GetSelectionCategory() == TARGET_SELECT_CATEGORY_CHANNEL ||
                Effects[i].TargetA.GetSelectionCategory() == TARGET_SELECT_CATEGORY_CONE || Effects[i].TargetB.GetSelectionCategory() == TARGET_SELECT_CATEGORY_CONE)
                return true;
        }
    }
    return false;
...
bool SpellInfo::IsPassive() const { return Attributes & SPELL_ATTR0_PASSIVE; }
...
bool SpellInfo::IsPassiveStackableWithRanks() const { return IsPassive() && !HasEffect(SPELL_EFFECT_APPLY_AURA) && !HasEffect(SPELL_EFFECT_APPLY_AREA_AURA_RAID); }
...
bool SpellInfo::IsDeathPersistent() const { return AttributesEx3 & SPELL_ATTR3_DEATH_PERSISTENT; }

bool SpellInfo::IsRequiringDeadTarget() const { return AttributesEx3 & SPELL_ATTR3_ONLY_TARGET_GHOSTS; }

bool SpellInfo::IsAllowingDeadTarget() const { return AttributesEx2 & SPELL_ATTR2_CAN_TARGET_DEAD || Targets & (TARGET_FLAG_CORPSE_ALLY | TARGET_FLAG_CORPSE_ENEMY | TARGET_FLAG_UNIT_DEAD); }

bool SpellInfo::CanBeUsedInCombat() const { return !(Attributes & SPELL_ATTR0_CANT_USED_IN_COMBAT); }

bool SpellInfo::IsBattleResurrection() const { return AttributesEx8 & SPELL_ATTR8_BATTLE_RESURRECTION; }

bool SpellInfo::IsPositive() const { return !(AttributesCu & SPELL_ATTR0_CU_NEGATIVE); }
...
bool SpellInfo::IsChanneled() const { return (AttributesEx & (SPELL_ATTR1_CHANNELED_1 | SPELL_ATTR1_CHANNELED_2)); }
...
bool SpellInfo::IsRangedWeaponSpell() const
{
    return (SpellFamilyName == SPELLFAMILY_HUNTER && !(SpellFamilyFlags[1] & 0x10000000)) // for 53352, cannot find better way
           || ((EquippedItemSubClassMask & ITEM_SUBCLASS_MASK_WEAPON_RANGED) && DmgClass != SPELL_DAMAGE_CLASS_MAGIC && DmgClass != SPELL_DAMAGE_CLASS_MELEE);
}

bool SpellInfo::IsAutoRepeatRangedSpell() const { return AttributesEx2 & SPELL_ATTR2_AUTOREPEAT_FLAG; }

bool SpellInfo::IsAffectedBySpellMods() const { return !(AttributesEx3 & SPELL_ATTR3_NO_DONE_BONUS); }
...
bool SpellInfo::CanPierceImmuneAura(SpellInfo const* aura, const uint8 effectMask /*= MAX_EFFECT_MASK*/) const
{
    ...
    if (HasAttribute(SPELL_ATTR1_UNAFFECTED_BY_SCHOOL_IMMUNE) || HasAttribute(SPELL_ATTR2_UNAFFECTED_BY_AURA_SCHOOL_IMMUNE))
    {
        ...
        if (aura->Mechanic != MECHANIC_IMMUNE_SHIELD && aura->Mechanic != MECHANIC_INVULNERABILITY &&
    ...
    // TODO: Implement CanSpellProvideImmunityAgainstAura to validate if the unit can apply auras also if the immunity allows it
...
bool SpellInfo::CanDispelAura(SpellInfo const* aura) const
{
    ...
    if (aura->HasAttribute(SPELL_ATTR0_PASSIVE))
        return false;

    // These auras (Cyclone for example) are not dispelable
    if ((aura->HasAttribute(SPELL_ATTR1_UNAFFECTED_BY_SCHOOL_IMMUNE) && aura->Mechanic != MECHANIC_NONE) || aura->HasAttribute(SPELL_ATTR2_UNAFFECTED_BY_AURA_SCHOOL_IMMUNE))
        return false;
...
bool SpellInfo::IsSingleTargetWith(SpellInfo const* spellInfo) const
{
    ...
    if (SpellFamilyName == spellInfo->SpellFamilyName && SpellIconID == spellInfo->SpellIconID)
        return true;
...
SpellCastResult SpellInfo::CheckShapeshift(uint32 form) const
{
    ...
    if (GetTalentSpellCost(Id) > 0 && (Effects[0].Effect == SPELL_EFFECT_LEARN_SPELL || Effects[1].Effect == SPELL_EFFECT_LEARN_SPELL || Effects[2].Effect == SPELL_EFFECT_LEARN_SPELL))
...
SpellCastResult SpellInfo::CheckLocation(uint32 map_id, uint32 zone_id, uint32 area_id, Player const* player) const
{
    ...
    case 85269: // Throne of the Four Winds
    case 87740:
    case 82651:
    case 95751:
    case 82724:
    {
        ...
        if (!mapEntry->IsRaid() && !mapEntry->IsDungeon())
            return SPELL_FAILED_INCORRECT_AREA;

        return map_id == 754 ? SPELL_CAST_OK : SPELL_FAILED_INCORRECT_AREA;
    }
    }
...
SpellCastResult SpellInfo::CheckVehicle(Unit const* caster) const
{
    ...
    if (vehicle)
    {
        ...
        if (!(AttributesEx6 & SPELL_ATTR6_CASTABLE_WHILE_ON_VEHICLE) && !(Attributes & SPELL_ATTR0_CASTABLE_WHILE_MOUNTED) && (vehicleSeat->m_flags & checkMask) != checkMask)
...
bool SpellInfo::IsRanked() const { return ChainEntry != NULL; }
            ...
            if (!deep)
            {
                if (SpellInfo const* spellTriggeredProto = sSpellMgr->GetSpellInfo(Effects[effIndex].TriggerSpell))
                {
                    ...
                    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
                    {
                        ...
                        if (_IsPositiveTarget(spellTriggeredProto->Effects[i].TargetA.GetTarget(), spellTriggeredProto->Effects[i].TargetB.GetTarget()) &&
                            !spellTriggeredProto->_IsPositiveEffect(i, true))
                            return false;
                    }
                }
            }
...
SpellTargetRestrictionsEntry const* SpellInfo::GetSpellTargetRestrictions() const { return SpellTargetRestrictionsId ? sSpellTargetRestrictionsStore.LookupEntry(SpellTargetRestrictionsId) : NULL; }

SpellEquippedItemsEntry const* SpellInfo::GetSpellEquippedItems() const { return SpellEquippedItemsId ? sSpellEquippedItemsStore.LookupEntry(SpellEquippedItemsId) : NULL; }

SpellInterruptsEntry const* SpellInfo::GetSpellInterrupts() const { return SpellInterruptsId ? sSpellInterruptsStore.LookupEntry(SpellInterruptsId) : NULL; }

SpellLevelsEntry const* SpellInfo::GetSpellLevels() const { return SpellLevelsId ? sSpellLevelsStore.LookupEntry(SpellLevelsId) : NULL; }

SpellPowerEntry const* SpellInfo::GetSpellPower() const { return SpellPowerId ? sSpellPowerStore.LookupEntry(SpellPowerId) : NULL; }

SpellReagentsEntry const* SpellInfo::GetSpellReagents() const { return SpellReagentsId ? sSpellReagentsStore.LookupEntry(SpellReagentsId) : NULL; }

SpellScalingEntry const* SpellInfo::GetSpellScaling() const { return SpellScalingId ? sSpellScalingStore.LookupEntry(SpellScalingId) : NULL; }

SpellShapeshiftEntry const* SpellInfo::GetSpellShapeshift() const { return SpellShapeshiftId ? sSpellShapeshiftStore.LookupEntry(SpellShapeshiftId) : NULL; }

SpellTotemsEntry const* SpellInfo::GetSpellTotems() const { return SpellTotemsId ? sSpellTotemsStore.LookupEntry(SpellTotemsId) : NULL; }

SpellAuraOptionsEntry const* SpellInfo::GetSpellAuraOptions() const { return SpellAuraOptionsId ? sSpellAuraOptionsStore.LookupEntry(SpellAuraOptionsId) : NULL; }

SpellAuraRestrictionsEntry const* SpellInfo::GetSpellAuraRestrictions() const { return SpellAuraRestrictionsId ? sSpellAuraRestrictionsStore.LookupEntry(SpellAuraRestrictionsId) : NULL; }
...
SpellCategoriesEntry const* SpellInfo::GetSpellCategories() const { return SpellCategoriesId ? sSpellCategoriesStore.LookupEntry(SpellCategoriesId) : NULL; }

SpellClassOptionsEntry const* SpellInfo::GetSpellClassOptions() const { return SpellClassOptionsId ? sSpellClassOptionsStore.LookupEntry(SpellClassOptionsId) : NULL; }

SpellCooldownsEntry const* SpellInfo::GetSpellCooldowns() const { return SpellCooldownsId ? sSpellCooldownsStore.LookupEntry(SpellCooldownsId) : NULL; }
