...
void Spell::AddUnitTarget(Unit* target, uint32 effectMask, bool checkIfValid /*= true*/, bool implicit /*= true*/)
{
    ...
    std::list<TargetInfo>::iterator iterList = m_UniqueTargetInfo.begin();
...
{
    for (std::list<TargetInfo>::iterator iterList = m_UniqueTargetInfo.begin(); iterList != m_UniqueTargetInfo.end();)
    {
...
void Spell::prepare(SpellCastTargets const* targets, AuraEffect const* triggeredByAura)
{
    ...
    else
    {
        ...
        if (!IsTriggered() && !m_spellInfo->IsChanneled())
            if (Player *pl = GetCaster()->ToPlayer())
                pl->SendArenaSpectatorSpell(m_spellInfo->Id, GetCastTime());
    ...
    if (std::vector<int32> const* spell_triggered = sSpellMgr->GetSpellLinked(m_spellInfo->Id))
    {
        for (int32 id : *spell_triggered)
            ...
            else
            {
                ...
                {
                    SpellInfo const* triggeredSpell = sSpellMgr->GetSpellInfo(id);
                    if (triggeredSpell && !triggeredSpell->IsAffectingArea())
                    {
                        for (std::list<TargetInfo>::iterator ihit = m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end(); ++ihit)
                        {
                            if (Unit* unit = m_caster->GetGUID() == ihit->targetGUID ? m_caster : ObjectAccessor::GetUnit(*m_caster, ihit->targetGUID))
                                m_caster->CastSpell(unit, id, true);
                        }
                    }
                    else
...
void Spell::handle_immediate()
{
    ...
    if (m_spellInfo->IsChanneled())
    {
        ...
        if (duration > 0)
        {
            ...
            if (Player *pl = GetCaster()->ToPlayer())
                pl->SendArenaSpectatorSpell(m_spellInfo->Id, -duration);
        }
        else if (duration == -1)
        {
            m_spellState = SPELL_STATE_CASTING;
...
void Spell::SendSpellStart()
{
    ...
    data << uint32(0);                                      // seems to be always 0 in sniffs but ?
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (Player *tmpPlayer = m_caster->ToPlayer())
        if (tmpPlayer->isSpectator() && m_spellInfo->Id != 6277)
    ...
    if (Unit* target = m_targets.GetUnitTarget())
    {
        ...
        if (target != m_caster)
        {
            ...
            if (!IsTriggered() && m_caster->GetEntry() != WORLD_TRIGGER) // Ignore LOS for gameobjects casts (wrongly casted by a trigger)
                if (!DisableMgr::IsDisabledFor(DISABLE_TYPE_SPELL, m_spellInfo->Id, NULL, SPELL_DISABLE_LOS) && !m_caster->IsWithinLOSInMap(target, m_spellInfo))
    ...
    if (m_targets.HasDst())
    {
        ...
        if (!DisableMgr::IsDisabledFor(DISABLE_TYPE_SPELL, m_spellInfo->Id, NULL, SPELL_DISABLE_LOS) && !m_caster->IsWithinLOS(x, y, z, m_spellInfo))
        ...
        switch (m_spellInfo->Effects[i].Effect)
        {
            case SPELL_EFFECT_APPLY_AURA:
            {
                switch (m_spellInfo->Id)
                {
                    ...
                    {
                        if (Player* caster = m_caster->ToPlayer())
                            if (Aura* aur = caster->GetAura(m_spellInfo->Id))
                                if (caster->CalcSpellDuration(m_spellInfo) < aur->GetDuration())
                                    return SPELL_FAILED_MORE_POWERFUL;
                        break;
                    }
                }
    ...
    for (int i = 0; i < MAX_SPELL_EFFECTS; i++)
    {
        switch (m_spellInfo->Effects[i].Effect)
        {
            ...
            case SPELL_EFFECT_CREATE_ITEM_2:
            {
                ...
                {
                    if (m_spellInfo->SpellFamilyName == SPELLFAMILY_GENERIC && p_caster->GetTypeId() == TYPEID_PLAYER && p_caster->ToPlayer()->InArena())
