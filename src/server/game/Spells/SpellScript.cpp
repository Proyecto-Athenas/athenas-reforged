...
void SpellScript::SetExplTargetUnit(Unit *target)
{
...
int32 SpellScript::GetAbsorbedDamage()
{
    if (!IsInTargetHook())
    {
        TC_LOG_ERROR("scripts", "Script: `%s` Spell: `%u`: function SpellScript::GetAbsorbedDamage was called, but function has no effect in current hook!", m_scriptName->c_str(), m_scriptSpellId);
        return 0;
    }
...
int32 SpellScript::GetTrueDamage()
{
    ...
    {
        TC_LOG_ERROR("scripts", "Script: `%s` Spell: `%u`: function SpellScript::GetHitHeal was called while spell not in after-hit phase!", m_scriptName->c_str(), m_scriptSpellId);
        return 0;
    }
...
bool AuraScript::_Validate(SpellInfo const* entry)
{
    ...
    for (std::list<EffectUpdateHandler>::iterator itr = OnEffectUpdate.begin(); itr != OnEffectUpdate.end();  ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            TC_LOG_ERROR("scripts", "Spell `%u` Effect `%s` of script `%s` did not match dbc effect data - handler bound to hook `OnEffectUpdate` of AuraScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());
...
void AuraScript::EffectUpdateHandler::Call(AuraScript* auraScript, AuraEffect* aurEff, const uint32 diff)
{
    (auraScript->*pEffectHandlerScript)(aurEff, diff);
}
...
uint32 AuraScript::GetStackAmount() const
{
    return m_aura->GetStackAmount();
}

void AuraScript::SetStackAmount(uint32 num)
{
