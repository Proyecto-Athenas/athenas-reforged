...
void AuraEffect::HandleModInvisibility(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    {
        // apply glow vision
        if (target->GetTypeId() == TYPEID_PLAYER && m_spellInfo->Id != 32727 && m_spellInfo->Id != 68631)
...
void AuraEffect::HandleModCastingSpeed(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    for (Unit::AuraEffectList::const_iterator i = mTotalAuraList2.begin(); i != mTotalAuraList2.end(); ++i)
    {
        if (this == (*i))
            continue;
        if (sSpellMgr->CheckSpellGroupStackRules(aurApp->GetBase()->GetSpellInfo(), (*i)->GetSpellInfo(), true) == SPELL_GROUP_STACK_RULE_EXCLUSIVE_SAME_EFFECT)
...
void AuraEffect::HandlePeriodicDummyAuraTick(Unit* target, Unit* caster) const
{
    ...
    case SPELLFAMILY_GENERIC:
        ...
        case 76691: // Vengeance
            ...
            {
                // Remove partially while not in combat
            ...
            case 22842:
                {
                    ...
                    {
                        regen /= 4; // 25% de lo que se cura el druida
                        target->CastCustomSpell(target, 105739, &regen, 0, 0, true, 0, this);
                    }
