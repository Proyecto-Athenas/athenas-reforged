...
enum AuraType
{
    ...
    SPELL_AURA_PERIODIC_TRIGGER_SPELL_BY_CLIENT             = 48,
    ...
    SPELL_AURA_MOD_MELEE_DAMAGE_FROM_CASTER                 = 343,  // Bandit's Guile and Vendetta
    ...
    SPELL_AURA_MOD_HEALING_DONE_PERCENT_WITH_SPECIFIC_AURA  = 359,  // currently only used by ID - 99263 Vital Flame
    SPELL_AURA_PROC_TRIGGER_SPELL_COPY                      = 360,  // Procs the same spell that caused this proc (Dragonwrath, Tarecgosa's Rest)
    SPELL_AURA_PROC_TRIGGER_SPELL_OVERRIDE                  = 361,  // Proc overrides the action that caused this proc (only used for melee attacks)
