...
bool SoloQueue::RemovePlayer(uint64 playerGuid)
{
    ...
    for (std::multimap<uint32, SoloQueueInfo *>::iterator itr = queuedDps.begin(); itr != queuedDps.end(); ++itr)
        if (itr->second->playerGuid == playerGuid)
        {
            delete itr->second;
            queuedDps.erase(itr);
...
void SoloQueue::Update(const uint32 diff)
{
    ...
    for (std::multimap<uint32, SoloQueueInfo *>::iterator itr = queuedHealers.begin(); itr != queuedHealers.end();)
    {
        ...
        uint8 needDps = NEED_DPS_COUNT;
        for (auto &melees : queuedDps)
        {
            ...
            if (IsInMmrRange(minMMRMelee, maxMMRMelee, itr->second->ArenaMatchmakerRating) || IsInMmrRange(minMMRHeal, maxMMRHeal, melees.second->ArenaMatchmakerRating))
            {
...
bool SoloQueue::CheckRequirements(Player *player)
{
    ...
    if (!player->isGameMaster())
    {
        ...
        {
            ChatHandler(player->GetSession()).PSendSysMessage("Your current average item level is: %f! You should have at least an average item level of 370!", player->GetAverageItemLevel());
            return false;
        }
    }
...
{
    if (allPlayersInQueue.find(player->GetGUID()) != allPlayersInQueue.end() || player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_1v1_SOLO) || player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_1v1_RATED) || player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_3v3_SOLO))
        return true;
    return false;
}
