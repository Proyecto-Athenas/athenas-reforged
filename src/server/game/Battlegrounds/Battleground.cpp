...
inline void Battleground::_ProcessJoin(uint32 diff)
{
    ...
    else if (GetStartDelayTime() <= 0 && !(m_Events & BG_STARTING_EVENT_4))
    {
        ...
        if (isArena() && isRated())
        {
            ...
            sInfoMgr->AddBGInfo(Slot, GetInstanceID(), GetArenaTeamIdByIndex(0), GetArenaTeamIdByIndex(1), MMR1, MMR2);
        }
...
void Battleground::CreditQuestOnTeam(uint32 CreatureID, uint32 TeamID)
{
    for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
        if (Player* player = _GetPlayerForTeam(TeamID, itr, "QuestOnTeam"))
        {
            // credit quest PVP
            player->KilledMonsterCredit(CreatureID);
        }        
}
...
void Battleground::EndBattleground(uint32 winner)
{
                    ...
                    for (Battleground::BattlegroundScoreMap::const_iterator itr = GetPlayerScoresBegin(); itr != GetPlayerScoresEnd(); ++itr)
                        if (Player* player = ObjectAccessor::FindPlayer(itr->first))
                        {
                            if (itr->second->BgTeam == winner)
                            {
                                ...
                                winplayerguid[wincnt] = player->GetGUID();
                            ...
                            else
                            {
                                ...
                                losplayerguid[loscnt] = player->GetGUID();
                        ...
                        uint8 index = 0;
                        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_ARENA_LOG);
                        stmt->setUInt32(  index, uint32(now));
                        stmt->setUInt32(++index, GetElapsedTime());
                        stmt->setUInt32(++index, winnerArenaTeam->GetId());
                        stmt->setUInt32(++index, loserArenaTeam->GetId());
                        stmt->setUInt32(++index, winnerTeamRating);
                        stmt->setUInt32(++index, winnerMatchmakerRating);
                        stmt->setUInt32(++index, loserTeamRating);
                        stmt->setUInt32(++index, loserMatchmakerRating);
                        for (uint8 i = 0; i < 5; ++i)
                            stmt->setString(++index, winplayername[i]);
                        for (uint8 i = 0; i < 5; ++i)
                            stmt->setUInt32(++index, winplayerguid[i]);
                        for (uint8 i = 0; i < 5; ++i)
                            stmt->setString(++index, losplayername[i]);
                        for (uint8 i = 0; i < 5; ++i)
                            stmt->setUInt32(++index, losplayerguid[i]);
    ...
    for (BattlegroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
        ...
        if (isArena() && !isRated() && GetArenaType() == ARENA_TYPE_1v1_RATED)
        {
            if (team == winner)
            {
                player->AddSoloWins();
                player->AddSoloRating();
            }
            else
                player->RemoveSoloRating();

            player->AddSoloGames();
            player->SaveSoloRating();
        }
        ...
        if (isArena() && isRated() && winnerArenaTeam && loserArenaTeam && winnerArenaTeam != loserArenaTeam)
        {
            if (team == winner)
            {
                ...
                if ((player->GetQuestStatus(30576) == QUEST_STATUS_INCOMPLETE) || (player->GetQuestStatus(30577) == QUEST_STATUS_INCOMPLETE))
                    player->KilledMonsterCredit(3800003);

                // quest PVP-Arena-3v3
                if(GetArenaType() == ARENA_TYPE_3v3 && player->GetQuestStatus(30580) == QUEST_STATUS_INCOMPLETE)
					player->KilledMonsterCredit(3800003);
...
void Battleground::EndSoloBattleground(uint32 winner)
{
    ...
    if (winner != WINNER_NONE && winner != WINNER_NOT_JOINED)
    {
        ...
        winnerMatchmakerChange *= 0.2f;
        loserMatchmakerChange *= 1.5f;
        winnerChange *= 0.2f;
        loserChange *= 1.5f;
        SetArenaMatchmakerRating(winner, winnerMatchmakerRating + winnerMatchmakerChange);
        SetArenaMatchmakerRating(GetOtherTeam(winner), loserMatchmakerRating + loserMatchmakerChange);
...
void Battleground::RemovePlayerAtLeave(uint64 guid, bool Transport, bool SendPacket)
{
    ...
    if (participant) // if the player was a match participant, remove auras, calc rating, update queue
    {
        ...
        if (!isRated() && isBattleground() && GetStatus() < STATUS_WAIT_LEAVE)
        {
...
void Battleground::AddPlayer(Player* player)
{
    ...
    if (isArena())
    {
        if (isSoloQueueArena())
        {
            ...
            if (GetArenaType() == ARENA_TYPE_1v1_SOLO)
                player->DestroyItemCount(100606, 1, true);
        }
...
*/
    ...
    //if (go->IsTransport()) // for RoV maybe we should use a switch because at some transport objects the server freezes (not tested at other battlegrounds with transport)
