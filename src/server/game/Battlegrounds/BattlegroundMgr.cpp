...
BattlegroundMgr::BattlegroundMgr() : m_NextRatedArenaUpdate(sWorld->getIntConfig(CONFIG_ARENA_RATED_UPDATE_TIMER)),
...
void BattlegroundMgr::Update(uint32 diff)
{
    ...
    if (sWorld->getIntConfig(CONFIG_ARENA_MAX_RATING_DIFFERENCE) && sWorld->getIntConfig(CONFIG_ARENA_RATED_UPDATE_TIMER))
    {
        ...
        if (m_NextRatedArenaUpdate < diff)
        {
            ...
            for (int qtype = BATTLEGROUND_QUEUE_2v2; qtype <= BATTLEGROUND_QUEUE_1v1_RATED; ++qtype)
...
void BattlegroundMgr::BuildPvpLogDataPacket(WorldPacket *data, Battleground *bg)
{
    ...
    for (Battleground::BattlegroundScoreMap::const_iterator itr = bg->GetPlayerScoresBegin(); itr != bg->GetPlayerScoresEnd(); ++itr)
    {
        ...
        if (Player *player = ObjectAccessor::FindPlayer(playerGUID)) // Get the rating if player is online.
            RatedBGRating = (bg->GetWinner() == bg->GetPlayerTeam(playerGUID) ? (player->GetRBGPersonalRating() - 20) : (player->GetRBGPersonalRating() > 1500 ? (player->GetRBGPersonalRating() + 20) : player->GetRBGPersonalRating()));
        ...
        case BATTLEGROUND_RATED_10_VS_10:
            ...
            case 566:
            {
                if (bg->isRatedBattleground())
                {
                    ...
                    buff << uint32(((BattlegroundEYRScore *)itr->second)->FlagCaptures);   // flag captures
                    buff << uint32(((BattlegroundEYRScore *)itr->second)->BasesAssaulted); // bases assaulted
                    buff << uint32(((BattlegroundEYRScore *)itr->second)->BasesDefended);  // bases defended
                }
                else
                {
...
Battleground *BattlegroundMgr::CreateNewBattleground(BattlegroundTypeId originalBgTypeId, PvPDifficultyEntry const *bracketEntry, uint8 arenaType, bool isRated, BattlegroundTypeId bgTypeIdRandom)
{
...
void BattlegroundMgr::CreateInitialBattlegrounds()
{
    ...
    do
    {
        ...
        if (data.bgTypeId == BATTLEGROUND_AA || data.bgTypeId == BATTLEGROUND_RB || data.bgTypeId == BATTLEGROUND_RATED_10_VS_10 || data.bgTypeId == BATTLEGROUND_RATED_15_VS_15 || data.bgTypeId == BATTLEGROUND_RATED_25_VS_25)
        {
        ...
        if (data.bgTypeId == BATTLEGROUND_WS || data.bgTypeId == BATTLEGROUND_AB || data.bgTypeId == BATTLEGROUND_EY_RATED || data.bgTypeId == BATTLEGROUND_BFG || data.bgTypeId == BATTLEGROUND_TP)
...
{
    return bgTypeId == BATTLEGROUND_AA || bgTypeId == BATTLEGROUND_BE || bgTypeId == BATTLEGROUND_NA || bgTypeId == BATTLEGROUND_DS || bgTypeId == BATTLEGROUND_RV || bgTypeId == BATTLEGROUND_RL;
}
...
BattlegroundQueueTypeId BattlegroundMgr::BGQueueTypeId(BattlegroundTypeId bgTypeId, uint8 arenaType)
{
    ...
    case BATTLEGROUND_RV:
        ...
        case ARENA_TYPE_1v1_RATED:
            return BATTLEGROUND_QUEUE_1v1_RATED;
        default:
...
BattlegroundTypeId BattlegroundMgr::BGTemplateId(BattlegroundQueueTypeId bgQueueTypeId)
{
    ...
    case BATTLEGROUND_QUEUE_1v1_RATED:
...
HolidayIds BattlegroundMgr::BGTypeToWeekendHolidayId(BattlegroundTypeId bgTypeId)
{
    ...
    case BATTLEGROUND_BFG:
        return HOLIDAY_CALL_TO_ARMS_BFG;
    default:
        return HOLIDAY_NONE;
    }
}
