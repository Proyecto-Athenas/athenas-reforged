...
void BattlegroundQueue::RemovePlayer(uint64 guid, bool decreaseInvitedCount)
{
    ...
    for (int32 bracket_id_tmp = MAX_BATTLEGROUND_BRACKETS - 1; bracket_id_tmp >= 0 && bracket_id == -1; --bracket_id_tmp)
    {
        ...
        {
            GroupsQueueType::iterator k = m_QueuedGroups[bracket_id_tmp][BG_QUEUE_RBG].begin();
...
void BattlegroundQueue::BattlegroundQueueUpdate(uint32 /*diff*/, BattlegroundTypeId bgTypeId, BattlegroundBracketId bracket_id, uint8 arenaType, bool isRated, uint32 arenaRating)
{
    ...
    {
        MinPlayersPerTeam = sBattlegroundMgr->isArenaTesting() ? 1 : (arenaType == ARENA_TYPE_1v1_RATED ? 1 : arenaType);
        MaxPlayersPerTeam = arenaType == ARENA_TYPE_1v1_RATED ? 1 : arenaType;
    }
    else if (bgTypeId == BATTLEGROUND_RB)
    {
        if (GetNextRandomBattleground() == BATTLEGROUND_TYPE_NONE || ((GetNextRandomBattleground() == BATTLEGROUND_AV || GetNextRandomBattleground() == BATTLEGROUND_IC) && time(NULL) > GetRandomBattlegroundDecayTimer()))
        {
    ...
    if (!isRated)
    {
        ...
        if (CheckNormalMatch(bg_template, bracket_id, MinPlayersPerTeam, MaxPlayersPerTeam) || (bg_template->isArena() && CheckSkirmishForSameFaction(bracket_id, MinPlayersPerTeam)) || CheckCrossFactionMatch(bracket_id, bg_template, MinPlayersPerTeam, MaxPlayersPerTeam))
        {
                ...
                while (citr != m_QueuedGroups[bracket_id][queueGroupType].end())
                {
                    ...
                    for (uint8 i = BG_QUEUE_PREMADE_ALLIANCE; i < BG_QUEUE_NORMAL_ALLIANCE && found < 2; i++)
                    {
                        ...
                        for (; itr2 != m_QueuedGroups[bracket_id][i].end(); ++itr2)
                        {
                            ...
                            if (!(*itr2)->IsInvitedToBGInstanceGUID && (((*itr2)->ArenaMatchmakerRating >= arenaMinRating && (*itr2)->ArenaMatchmakerRating <= arenaMaxRating) || (*itr2)->JoinTime < discardTime) && (*itr_teams[0])->ArenaTeamIdOrRbgLeaderGuid != (*itr2)->ArenaTeamIdOrRbgLeaderGuid)
                            {
    ...
    {
        GroupsQueueType::iterator citr = m_QueuedGroups[bracket_id][BG_QUEUE_RBG].begin();
        while (citr != m_QueuedGroups[bracket_id][BG_QUEUE_RBG].end())
        {
            ...
            GroupsQueueType::iterator itr2 = m_QueuedGroups[bracket_id][BG_QUEUE_RBG].begin();
