...
bool BattlegroundAB::SetupBattleground()
{
    for (int i = 0; i < BG_AB_DYNAMIC_NODES_COUNT; ++i)
    {
        if (!AddObject(BG_AB_OBJECT_BANNER_NEUTRAL + 8 * i, isRatedBattleground() ? BG_AB_RBG_OBJECTID_NODE_BANNER_0 + i: BG_AB_OBJECTID_NODE_BANNER_0 + i, BG_AB_NodePositions[i][0], BG_AB_NodePositions[i][1], BG_AB_NodePositions[i][2], BG_AB_NodePositions[i][3], 0, 0, std::sin(BG_AB_NodePositions[i][3] / 2), std::cos(BG_AB_NodePositions[i][3] / 2), RESPAWN_ONE_DAY)
            || !AddObject(BG_AB_OBJECT_BANNER_CONT_A + 8 * i, isRatedBattleground() ? BG_AB_RBG_OBJECTID_BANNER_CONT_A : BG_AB_OBJECTID_BANNER_CONT_A, BG_AB_NodePositions[i][0], BG_AB_NodePositions[i][1], BG_AB_NodePositions[i][2], BG_AB_NodePositions[i][3], 0, 0, std::sin(BG_AB_NodePositions[i][3] / 2), std::cos(BG_AB_NodePositions[i][3] / 2), RESPAWN_ONE_DAY)
            || !AddObject(BG_AB_OBJECT_BANNER_CONT_H + 8 * i, isRatedBattleground() ? BG_AB_RBG_OBJECTID_BANNER_CONT_H : BG_AB_OBJECTID_BANNER_CONT_H, BG_AB_NodePositions[i][0], BG_AB_NodePositions[i][1], BG_AB_NodePositions[i][2], BG_AB_NodePositions[i][3], 0, 0, std::sin(BG_AB_NodePositions[i][3] / 2), std::cos(BG_AB_NodePositions[i][3] / 2), RESPAWN_ONE_DAY)
            || !AddObject(BG_AB_OBJECT_BANNER_ALLY + 8 * i, isRatedBattleground() ? BG_AB_RBG_OBJECTID_BANNER_A : BG_AB_OBJECTID_BANNER_A, BG_AB_NodePositions[i][0], BG_AB_NodePositions[i][1], BG_AB_NodePositions[i][2], BG_AB_NodePositions[i][3], 0, 0, std::sin(BG_AB_NodePositions[i][3] / 2), std::cos(BG_AB_NodePositions[i][3] / 2), RESPAWN_ONE_DAY)
            || !AddObject(BG_AB_OBJECT_BANNER_HORDE + 8 * i, isRatedBattleground() ? BG_AB_RBG_OBJECTID_BANNER_H : BG_AB_OBJECTID_BANNER_H, BG_AB_NodePositions[i][0], BG_AB_NodePositions[i][1], BG_AB_NodePositions[i][2], BG_AB_NodePositions[i][3], 0, 0, std::sin(BG_AB_NodePositions[i][3] / 2), std::cos(BG_AB_NodePositions[i][3] / 2), RESPAWN_ONE_DAY)
...
void BattlegroundAB::EndBattleground(uint32 winner)
{
    ...
    // creadit quest PVP Custom
    CreditQuestOnTeam(3800004, winner);
...
void BattlegroundAB::UpdatePlayerScore(Player* Source, uint32 type, uint32 value, bool doAddHonor)
{
    ...
    switch (type)
    {
        case SCORE_BASES_ASSAULTED:
            ...
            // credit quest PVP
            if ((Source->GetQuestStatus(300070) == QUEST_STATUS_INCOMPLETE))
                Source->KilledMonsterCredit(3800006);
            break;
