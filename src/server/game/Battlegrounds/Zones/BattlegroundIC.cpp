...
void BattlegroundIC::HandleKillUnit(Creature* unit, Player* killer)
{
    ...
    {
        RewardHonorToTeam(GetBonusHonorFromKill(4), HORDE);
        EndBattleground(HORDE);
    }
    else if (entry == NPC_OVERLORD_AGMAR)
    {
        RewardHonorToTeam(GetBonusHonorFromKill(4), ALLIANCE);
...
{
    CreditQuestOnTeam(13458, winner);
