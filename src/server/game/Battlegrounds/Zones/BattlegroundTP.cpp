                ...
                if (_flagDebuffState <= 5) ///< Cast on player Focused Assaultupposed to maintain number of stacks for debuff
                {
                    for (uint8 team = TEAM_ALLIANCE; team <= TEAM_HORDE; ++team)
                        if (Player* player = ObjectAccessor::FindPlayer(_flagKeepers[team]))
                            ...
                            TC_LOG_ERROR("bg.battleground", "BattlegroundTP: Can't find flag keeper with GUID: %u, in team: %s", GUID_LOPART(_flagKeepers[team]), team == TEAM_ALLIANCE ? "ALLIANCE" : "HORDE");
                }
...
bool BattlegroundTP::SetupBattleground()
{
    ...
    if (!AddObject(BG_TP_OBJECT_A_FLAG, BG_OBJECT_A_FLAG_TP_ENTRY, 2118.210f, 191.621f, 44.052f, 5.741259f, 0, 0, 0.9996573f, 0.02617699f, BG_TP_FLAG_RESPAWN_TIME/1000)
        ...
        || !AddObject(BG_TP_OBJECT_DOOR_A_2, BG_OBJECT_DOOR_A_2_TP_ENTRY, 2155.851f, 219.024f, 43.649f, 2.544690f, 0, 0, 0, 0, RESPAWN_IMMEDIATELY)
    ...
    for (uint8 team = TEAM_ALLIANCE; team <= TEAM_HORDE; ++team)
    {
        if (_flagKeepers[team ^ 1] == guid)
        {
            if (_bothFlagsKept)
            {
                ...
                {
                    if (Player* pl = ObjectAccessor::FindPlayer(_flagKeepers[teamTemp]))
                    {
                        pl->RemoveAurasDueToSpell(TP_SPELL_FOCUSED_ASSAULT);
                        pl->RemoveAurasDueToSpell(TP_SPELL_BRUTAL_ASSAULT);
                    }
                    else
                        TC_LOG_ERROR("bg.battleground", "BattlegroundTP: An error has occurred in RemovePlayer: player with GUID: %u haven't been found. (_bothflagsKept is TRUE).", GUID_LOPART(_flagKeepers[team]));
                }
