...
bool SmartAIMgr::IsEventValid(SmartScriptHolder& e)
{
    ...
    else
    {
        ...
        switch (type)
        {
            ...
            case SMART_EVENT_DISTANCE_PLAYER:
            {
                ...
                if (e.event.playerDistance.aura && !IsSpellValid(e, e.event.playerDistance.aura))
                    return false;
                break;
            }
    ...
    switch (e.GetActionType())
    {
        ...
        case SMART_ACTION_SOUND:
            ...
            {
                TC_LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses invalid Text Range %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.sound.range);
                return false;
            }
            break;
