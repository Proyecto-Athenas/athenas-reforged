...
bool CasterSpecTargetSelector::operator()(WorldObject* target) const
{
    ...
    switch (target->ToPlayer()->getClass())
    {
        ...
        case CLASS_DRUID:
            return _spellId ? (target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec()) == TALENT_TREE_DRUID_BALANCE || target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec()) == TALENT_TREE_DRUID_RESTORATION) && !target->ToPlayer()->HasAura(_spellId)
                : target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec()) == TALENT_TREE_DRUID_BALANCE || target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec()) == TALENT_TREE_DRUID_RESTORATION;
        case CLASS_SHAMAN:
            return _spellId ? (target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec()) == TALENT_TREE_SHAMAN_ELEMENTAL || target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec()) == TALENT_TREE_SHAMAN_RESTORATION) && !target->ToPlayer()->HasAura(_spellId)
                : target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec()) == TALENT_TREE_SHAMAN_ELEMENTAL || target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec()) == TALENT_TREE_SHAMAN_RESTORATION;
        case CLASS_PALADIN:
            return _spellId ? target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec()) == TALENT_TREE_PALADIN_HOLY && !target->ToPlayer()->HasAura(_spellId)
                : target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec()) == TALENT_TREE_PALADIN_HOLY;
    }
...
bool MeeleSpecTargetSelector::operator()(WorldObject* target) const
{
    ...
    switch (target->ToPlayer()->getClass())
    {
        case CLASS_ROGUE:
            return _spellId ? !target->ToPlayer()->HasAura(_spellId) : true;
        case CLASS_WARRIOR:
            return _spellId ? target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec()) != TALENT_TREE_WARRIOR_PROTECTION && !target->ToPlayer()->HasAura(_spellId)
                : target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec()) != TALENT_TREE_WARRIOR_PROTECTION;
        case CLASS_PALADIN:
            return _spellId ? target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec()) == TALENT_TREE_PALADIN_RETRIBUTION && !target->ToPlayer()->HasAura(_spellId)
                : target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec()) == TALENT_TREE_PALADIN_RETRIBUTION;
            break;
        case CLASS_DRUID:
            return _spellId ? target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec()) == TALENT_TREE_DRUID_FERAL_COMBAT && !target->ToPlayer()->HasAura(_spellId)
                : target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec()) == TALENT_TREE_DRUID_FERAL_COMBAT;
        case CLASS_SHAMAN:
            return _spellId ? target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec()) == TALENT_TREE_SHAMAN_ENHANCEMENT && !target->ToPlayer()->HasAura(_spellId)
                : target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec()) == TALENT_TREE_SHAMAN_ENHANCEMENT;
        case CLASS_DEATH_KNIGHT:
            return _spellId ? target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec()) != TALENT_TREE_DEATH_KNIGHT_BLOOD && !target->ToPlayer()->HasAura(_spellId)
                : target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec()) != TALENT_TREE_DEATH_KNIGHT_BLOOD;
    }
...
bool DpsSpecTargetSelector::operator()(WorldObject* target) const
{
    ...
    return _spellId ? target->ToPlayer()->GetRoleForGroup(target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec())) == ROLES_DPS && !target->ToPlayer()->HasAura(_spellId)
        : target->ToPlayer()->GetRoleForGroup(target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec())) == ROLES_DPS;
}
...
bool TankSpecTargetSelector::operator()(WorldObject* target) const
{
    ...
    return _spellId ? target->ToPlayer()->GetRoleForGroup(target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec())) == ROLES_TANK && !target->ToPlayer()->HasAura(_spellId)
        : target->ToPlayer()->GetRoleForGroup(target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec())) == ROLES_TANK;
}
...
bool HealerSpecTargetSelector::operator()(WorldObject* target) const
{
    ...
    return _spellId ? target->ToPlayer()->GetRoleForGroup(target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec())) == ROLES_HEALER && !target->ToPlayer()->HasAura(_spellId)
        : target->ToPlayer()->GetRoleForGroup(target->ToPlayer()->GetPrimaryTalentTree(target->ToPlayer()->GetActiveSpec())) == ROLES_HEALER;
}
