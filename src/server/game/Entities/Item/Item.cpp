    ...
    for (uint32 x = 0; x < MAX_ITEM_SET_SPELLS; ++x)
    {
        ...
        for (uint32 y = 0; y < MAX_ITEM_SET_SPELLS; ++y)
        {
            if (!eff->spells[y])                             // free slot
            {
                ...
                if (!spellInfo)
                {
                    TC_LOG_DEBUG("entities.player.items", "WORLD: unknown spell id %u in items set %u effects", set->spells[x], setid);
                    break;
                }
...
Item* Item::CreateItem(uint32 item, uint32 count, Player const* player)
{
    ...
    if (pProto)
    {
        ...
        uint32 guid = 0;
        if (item == GUILD_CHARTER)
            guid = sObjectMgr->GenerateFreeItemGuid();
        else
            guid = sObjectMgr->GenerateLowGuid(HIGHGUID_ITEM);
...
bool Item::CanBeTransmogrified(bool transmogrifyBonus /* = false */) const
{
    ItemTemplate const* proto = GetTemplate();
    ...
    if (!transmogrifyBonus && (
        proto->Quality == ITEM_QUALITY_LEGENDARY ||
    ...
    if (!transmogrifyBonus && (proto->Flags2 & ITEM_FLAGS_EXTRA_CANNOT_BE_TRANSMOG))
        return false;
...
bool Item::CanTransmogrify(bool transmogrifyBonus /* = false */) const
{
    ItemTemplate const* proto = GetTemplate();
    ...
    if (!transmogrifyBonus && (proto->Flags2 & ITEM_FLAGS_EXTRA_CANNOT_TRANSMOG))
        return false;

    if (!transmogrifyBonus && proto->Quality == ITEM_QUALITY_LEGENDARY)
        return false;
    ...
    if (!transmogrifyBonus && (proto->Flags2 & ITEM_FLAGS_EXTRA_CAN_TRANSMOG))
        return true;
...
{
    // Check transmogrify
    if (uint32 transmogrifier = GetEnchantmentId(TRANSMOGRIFY_ENCHANTMENT_SLOT))
    {
        bool wrong = false;

        ItemTemplate const* proto1 = GetTemplate();
        ItemTemplate const* proto2 = sObjectMgr->GetItemTemplate(transmogrifier);
        if (!proto2)
            wrong = true;
        ...
        if (proto1->InventoryType != proto2->InventoryType && !proto1->IsRangedWeapon() && !proto2->IsRangedWeapon())
        {
        ...
        if (wrong)
            SetUInt32Value(ITEM_FIELD_ENCHANTMENT_1_1 + TRANSMOGRIFY_ENCHANTMENT_SLOT * MAX_ENCHANTMENT_OFFSET + ENCHANTMENT_ID_OFFSET, 0);
    }

    // Check engineering enchantments
    ...
    if (pEnchant && pEnchant->requiredSkill == SKILL_ENGINEERING)
    {
        ...
        if (!GetEnchantmentId(ENGINEERING_ENCHANT_SLOT))
            SetUInt32Value(ITEM_FIELD_ENCHANTMENT_1_1 + ENGINEERING_ENCHANT_SLOT * MAX_ENCHANTMENT_OFFSET + ENCHANTMENT_ID_OFFSET, pEnchant->ID);
    }
}

bool Item::CanTransmogrifyItemWithItem(Item const* transmogrified, Item const* transmogrifier, bool transmogrifyBonus /* = false */)
{
    ...
    if (!transmogrified->CanTransmogrify(transmogrifyBonus) || !transmogrifier->CanBeTransmogrified(transmogrifyBonus))
        return false;
