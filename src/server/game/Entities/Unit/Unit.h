...
enum SpellValueMod
{
    ...
    SPELLVALUE_SPEED,
    SPELLVALUE_DURATON,    // final values, no mods anymore
...
{
  public:
    void AddSpellMod(SpellValueMod mod, int32 value) { push_back(std::make_pair(mod, value)); }
};
...
enum UnitFlags
{
    UNIT_FLAG_SERVER_CONTROLLED = 0x00000001, // set only when unit movement is controlled by server - by SPLINE/MONSTER_MOVE packets, together with UNIT_FLAG_STUNNED; only set to units controlled by
                                              // client; client function CGUnit_C::IsClientControlled returns false when set for owner
...
enum MovementFlags
{
    ...
    MOVEMENTFLAG_MASK_MOVING = MOVEMENTFLAG_FORWARD | MOVEMENTFLAG_BACKWARD | MOVEMENTFLAG_STRAFE_LEFT | MOVEMENTFLAG_STRAFE_RIGHT | MOVEMENTFLAG_PITCH_UP | MOVEMENTFLAG_PITCH_DOWN |
                               MOVEMENTFLAG_FALLING | MOVEMENTFLAG_FALLING_FAR | MOVEMENTFLAG_ASCENDING | MOVEMENTFLAG_DESCENDING | MOVEMENTFLAG_SPLINE_ELEVATION,
    ...
    MOVEMENTFLAG_MASK_CREATURE_ALLOWED = MOVEMENTFLAG_FORWARD | MOVEMENTFLAG_DISABLE_GRAVITY | MOVEMENTFLAG_ROOT | MOVEMENTFLAG_SWIMMING | MOVEMENTFLAG_CAN_FLY | MOVEMENTFLAG_WATERWALKING |
                                         MOVEMENTFLAG_FALLING_SLOW | MOVEMENTFLAG_HOVER,
...
class Unit : public WorldObject
{
  public:
    ...
    {
        return !HasUnitState(UNIT_STATE_CONFUSED | UNIT_STATE_FLEEING | UNIT_STATE_IN_FLIGHT | UNIT_STATE_ROOT | UNIT_STATE_STUNNED | UNIT_STATE_DISTRACTED) && GetOwnerGUID() == 0;
    }
    ...
    uint32 DealDamage(Unit* victim, uint32 damage, CleanDamage const* cleanDamage = NULL, DamageEffectType damagetype = DIRECT_DAMAGE, SpellSchoolMask damageSchoolMask = SPELL_SCHOOL_MASK_NORMAL,
        SpellInfo const* spellProto = NULL, bool durabilityLoss = true, bool splitted = false);
    void Kill(Unit* victim, bool durabilityLoss = true, bool isRewardAllowed = true);
    int32 DealHeal(Unit* victim, uint32 addhealth);

    void ProcDamageAndSpell(Unit* victim, uint32 procAttacker, uint32 procVictim, uint32 procEx, uint32 amount, uint32 absorbed, uint32 resisted, uint32 blocked, uint32 armorMitigated,
        WeaponAttackType attType = BASE_ATTACK, SpellInfo const* procSpell = NULL, SpellInfo const* procAura = NULL, SpellInfo const* interruptedSpell = NULL, bool procSpellIsHeal = false,
        bool onCast = false, const uint8 effectMask = MAX_EFFECT_MASK);
    void ProcDamageAndSpellForAttacker(Unit* victim, uint32 procAttacker, uint32 procVictim, uint32 procEx, uint32 amount, uint32 absorb, uint32 resisted, uint32 blocked, uint32 armorMitigated,
        WeaponAttackType attType = BASE_ATTACK, SpellInfo const* procSpell = NULL, SpellInfo const* procAura = NULL, SpellInfo const* interruptedSpell = NULL, bool procSpellIsHeal = false,
        bool onCast = false, const uint8 effectMask = MAX_EFFECT_MASK);
    void ProcDamageAndSpellForVictim(Unit* victim, uint32 procAttacker, uint32 procVictim, uint32 procEx, uint32 amount, uint32 absorb, uint32 resisted, uint32 blocked, uint32 armorMitigated,
        WeaponAttackType attType = BASE_ATTACK, SpellInfo const* procSpell = NULL, SpellInfo const* procAura = NULL, SpellInfo const* interruptedSpell = NULL, bool procSpellIsHeal = false,
        bool onCast = false, const uint8 effectMask = MAX_EFFECT_MASK);
    void ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, uint32 damage, uint32 absorbed, uint32 resisted,
        uint32 blocked, uint32 armorMitigated, SpellInfo const* procAura = NULL, SpellInfo const* interruptedSpell = NULL, bool procSpellIsHeal = false, bool onCast = false,
    ...
    void CastSpell(SpellCastTargets const& targets, SpellInfo const* spellInfo, CustomSpellValues const* value, TriggerCastFlags triggerFlags = TRIGGERED_NONE, Item* castItem = NULL,
        AuraEffect const* triggeredByAura = NULL, uint64 originalCaster = 0);
    ...
    void CastSpell(
        Unit* victim, SpellInfo const* spellInfo, TriggerCastFlags triggerFlags = TRIGGERED_NONE, Item* castItem = NULL, AuraEffect const* triggeredByAura = NULL, uint64 originalCaster = 0);
    ...
    void CastCustomSpell(
        Unit* victim, uint32 spellId, int32 const* bp0, int32 const* bp1, int32 const* bp2, bool triggered, Item* castItem = NULL, AuraEffect const* triggeredByAura = NULL, uint64 originalCaster = 0);
    void CastCustomSpell(Unit* victim, uint32 spellId, int32 const* bp0, int32 const* bp1, int32 const* bp2, TriggerCastFlags triggerFlags = TRIGGERED_NONE, Item* castItem = NULL,
        AuraEffect const* triggeredByAura = NULL, uint64 originalCaster = 0);
    ...
    void CastCustomSpell(uint32 spellId, SpellValueMod mod, int32 value, Unit* victim = NULL, TriggerCastFlags triggerFlags = TRIGGERED_NONE, Item* castItem = NULL,
        AuraEffect const* triggeredByAura = NULL, uint64 originalCaster = 0);
    void CastCustomSpell(uint32 spellId, CustomSpellValues const& value, Unit* victim = NULL, TriggerCastFlags triggerFlags = TRIGGERED_NONE, Item* castItem = NULL,
        AuraEffect const* triggeredByAura = NULL, uint64 originalCaster = 0);
    ...
    */
    ...
    void AddPlayerToVision(uint64 const& guid);
    void RemovePlayerFromVision(uint64 const& guid);
    ...
    void RemoveAurasByType(
        AuraType auraType, uint64 casterGUID = 0, Aura* except = NULL, bool negative = true, bool positive = true, SpellFamilyNames SpellFamilyName = SPELLFAMILY_GENERIC, uint32 schoolMask = 0);
    ...
    void SaveSoulSwapDotsListAndTarget(uint64 targetGUID, std::list<uint32> dotsList)
    {
        m_soulSwapTargetGuid = targetGUID;
        m_soulSwapDotsList = dotsList;
    }
    ...
    void ClearSoulSwapDotsList()
    {
        m_soulSwapTargetGuid = 0;
        m_soulSwapDotsList.clear();
    }
    ...
    inline bool IsInBearForm() const { return GetShapeshiftForm() == FORM_BEAR; }

    inline bool IsInDisallowedMountForm() const
    {
        ShapeshiftForm form = GetShapeshiftForm();
        return form != FORM_NONE && form != FORM_BATTLESTANCE && form != FORM_BERSERKERSTANCE && form != FORM_DEFENSIVESTANCE && form != FORM_SHADOW && form != FORM_STEALTH && form != FORM_UNDEAD &&
  ...
  private:
    bool IsTriggeredAtSpellProcEvent(Unit* victim, Aura* aura, SpellInfo const* procSpell, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, bool isVictim, bool active,
        SpellProcEventEntry const*& spellProcEvent, const uint8 effectMask = MAX_EFFECT_MASK);
    bool HandleAuraProcOnPowerAmount(
    ...
    bool HandleDummyAuraProc(Unit* victim, uint32 damage, uint32 absorbed, uint32 resisted, uint32 blocked, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlag, uint32 procEx,
        uint32 cooldown, SpellInfo const* interruptedSpell, bool procSpellIsHeal, bool onCast = false, const uint8 effectMask = MAX_EFFECT_MASK);
    bool HandleAuraProc(
        Unit* victim, uint32 damage, Aura* triggeredByAura, SpellInfo const* procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown, bool* handled, const uint8 effectMask = MAX_EFFECT_MASK);
    bool HandleProcTriggerSpell(Unit* victim, uint32 damage, uint32 absorbed, uint32 resisted, uint32 blocked, uint32 armorMitigated, AuraEffect* triggeredByAura, SpellInfo const* procSpell,
        uint32 procFlag, uint32 procEx, uint32 cooldown, const uint8 effectMask = MAX_EFFECT_MASK);
