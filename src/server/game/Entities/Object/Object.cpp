...
{
    if (!m_inWorld || GetTypeId() != TYPEID_PLAYER)
        return;
...
bool WorldObject::IsWithinLOS(float ox, float oy, float oz, SpellInfo const *m_spellInfo) const
{
    ...
    if (IsInWorld() && !DisableMgr::IsDisabledFor(DISABLE_TYPE_VMAP, GetMapId(), NULL, VMAP_DISABLE_LOS))
    {
        //        if (
    ...
    switch (GetTypeId())
    {
        case TYPEID_UNIT:
        {
            ...
            if (!ToCreature()->CanFly())
            {
                ...
                float ground_z = z;
                float max_z = canSwim
                    ? map->GetWaterOrGroundLevel(PHASEMASK_NORMAL, x, y, groundzcheck, &ground_z, !ToUnit()->HasAuraType(SPELL_AURA_WATER_WALK))
        ...
        case TYPEID_PLAYER:
        {
            ...
            {
                float ground_z = z;
                float max_z = GetBaseMap()->GetWaterOrGroundLevel(PHASEMASK_NORMAL, x, y, z, &ground_z, !ToUnit()->HasAuraType(SPELL_AURA_WATER_WALK));
                if (max_z > INVALID_HEIGHT)
                {
                    if (z > max_z)
                        z = max_z;
...
float WorldObject::GetSightRange(const WorldObject* target) const
{
    if (ToUnit())
    {
        if (ToPlayer())
        {
            if (target && (target->GetEntry() == 53087 || target->GetEntry() == 52577)) // Rhyolit Legs
                return 55.0f;
            else if (target && target->isActiveObject() && !target->ToPlayer())
                return MAX_VISIBILITY_DISTANCE;
            else if (GetMapId() == 754) // Throne of the Four Winds
                return MAX_VISIBILITY_DISTANCE + 1000.0f;
            else
                return GetMap()->GetVisibilityRange();
        }
        else if (ToCreature())
...
void WorldObject::MoveBlink(Position &pos, float dist, float angle)
{
    ...
    while (newDist <= dist)
    {
        ...
        // for server controlled moves playr work same as creature (but it can always swim)
        if (ToPlayer() ? !ToPlayer()->CanFly() : !ToUnit()->CanFly())
        {
            ground = GetMap()->GetHeight(GetPhaseMask(), destx, desty, MAX_HEIGHT, true);
            floor = std::max<float>(GetMap()->GetHeight(GetPhaseMask(), destx, desty, pos.m_positionZ, true), GetMap()->GetHeight(GetPhaseMask(), destx, desty, pos.m_positionZ, true, 5.0f));
            destz = fabs(ground - pos.m_positionZ) <= fabs(floor - pos.m_positionZ) ? ground : floor;
        ...
        for (PlayerMapType::iterator iter = m.begin(); iter != m.end(); ++iter)
        {
            ...
            if (!source->GetSharedVisionList().empty())
            {
                ...
                for (; it != source->GetSharedVisionList().end(); ++it)
                    if (Player* player = sObjectAccessor->FindPlayer(*it))
                        BuildPacket(player);
            }
        }
    }
