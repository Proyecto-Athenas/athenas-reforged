...
void GameObject::RemoveFromOwner()
{
    ...
    TC_LOG_DEBUG("misc", "Removed GameObject (GUID: %u Entry: %u SpellId: %u LinkedGO: %u) that just lost any reference to the owner (GUID: %u Type: '%s') GO list",
...
bool GameObject::Create(uint32 guidlow, uint32 name_id, Map* map, uint32 phaseMask, float x, float y, float z, float ang, float rotation0, float rotation1, float rotation2, float rotation3, uint32 animprogress, GOState go_state, uint32 artKit)
{
    ...
    switch (goinfo->type)
    {
        ...
        case GAMEOBJECT_TYPE_TRANSPORT:
            ...
            SetByteValue(GAMEOBJECT_BYTES_1, 0, goinfo->transport.startOpen ? GO_STATE_TRANSPORT_READY : GO_STATE_TRANSPORT_ACTIVE);
        ...
        case GAMEOBJECT_TYPE_TRAP:
            if (GetGOInfo()->trap.stealthed)
            {
                ...
                m_stealth.AddValue(STEALTH_TRAP, 30);
            }
                    ...
                    if (ok)
                    {
                        ...
                        if (goInfo->trap.spellId)
                        {
                            ...
                            else
                            {
                                ...
                                if (ok->GetTypeId() == TYPEID_PLAYER && !IsBattlegroundTrap && goInfo->trap.radius < 8)
                                    ok->NearTeleportTo(GetPositionX(), GetPositionY(), GetPositionZ(), ok->GetOrientation());
...
void GameObject::SetGoState(GOState state)
{
    ...
    if (m_model && !IsTransport())
    {
        ...
        if (state == GO_STATE_TRANSPORT_ACTIVE || state == GO_STATE_TRANSPORT_READY)
            EnableCollision(true);
        else
            EnableCollision(collision);
    }
