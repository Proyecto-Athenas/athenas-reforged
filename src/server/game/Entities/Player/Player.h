...
struct SoloStats
{
    SoloStats() : wins(0), games(0), rating(0) {}
...
enum ArenaSpecatorFlags
{
    ASPEC_FLAG_HEALTH = 0x0001,
    ASPEC_FLAG_MAXHEALTH = 0x0002,
    ASPEC_FLAG_POWER = 0x0004,
    ASPEC_FLAG_MAXPOWER = 0x0008,
    ASPEC_FLAG_POWERTYPE = 0x0010,
    ASPEC_FLAG_STATUS = 0x0020,
    ASPEC_FLAG_TEAM = 0x0040,
    ASPEC_FLAG_CLASS = 0x0080,
    ASPEC_FLAG_TARGET = 0x0100,
    ASPEC_FLAG_SPELL = 0x0200,
    ASPEC_FLAG_AURA = 0x0400,
    ASPEC_FLAG_PET = 0x0800,
    ASPEC_FLAG_PHP = 0x1000,
    ASPEC_FLAG_NAME = 0x2000,
    ASPEC_FLAG_TIME = 0x4000,

    ASPEC_ALL_FLAGS = 0xFFFF
};
...
enum PlayerLoginQueryIndex
{
    ...
    PLAYER_LOGIN_QUERY_MAILS = 40,
    PLAYER_LOGIN_QUERY_MAILS_ITEMS = 41,
    PLAYER_LOGIN_QUERY_LOAD_DIGSITES = 42,
    PLAYER_LOGIN_QUERY_LOAD_CURRENT_ARTIFACTS = 43,
    PLAYER_LOGIN_QUERY_LOAD_RESEARCH_HISTORY = 44,
    PLAYER_LOGIN_QUERY_LOAD_GUILD_SPELLS = 45,
    PLAYER_LOGIN_QUERY_LOAD_LFR_LOOT_DATA = 46,
...
enum BattlegroundConquestRewards
{
...
struct BGData
{
    ...
    WorldLocation leavePos; ///< From where player entered BG
...
{
public: // constructors
    TradeData(Player *player, Player *trader) : m_player(player), m_trader(trader), m_accepted(false), m_acceptProccess(false),
...
{
    PlayerTalentInfo() : FreeTalentPoints(0), UsedTalentCount(0), QuestRewardedTalentCount(0),
...
public:
    ...
    void ForceBGFactions(bool apply);
    ...
    bool SendBattleGroundChat(uint8 msgtype, std::string message);
    ...
    {
        return TeleportTo(m_homebindMapId, m_homebindX, m_homebindY, m_homebindZ, 0.0f);
    }
    ...
    bool ActivateTaxiPathTo(std::vector<uint32> const &nodes, Creature *npc = NULL, uint32 spellid = 0, bool skipPrice = false);
    bool ActivateTaxiPathTo(uint32 taxi_path_id, uint32 spellid = 0, bool skipPrice = false);
    ...
    bool isMovieMaker() const { return m_IsMovieMaker; }
    void SetGameMaster(bool on);
    void SetMovieMaker(bool on);
    ...
    void SendArenaSpectatorUpdate();
    void SendArenaSpectatorSpell(uint32 id, int32 time);
    void ResendAllSpectatorAuras(Player *target);
    void SendArenaSpectatorAura(AuraApplication *app, bool remove, Player *target = NULL);
    void SendArenaSpectatorCooldown(uint32 spell, uint32 endTime);
    ...
    void BuildPlayerChat(WorldPacket *data, uint8 msgtype, const std::string &text, uint32 language, const std::string &addonPrefix, Unit *receiver = nullptr);
    ...
    InventoryResult CanRollForItemInLFG(ItemTemplate const *item, WorldObject const *lootedObject);
    ...
    Item *EquipItem(uint16 pos, Item *pItem, bool update, bool slotSwitch = false);
    void AutoUnequipOffhandIfNeed(bool force = false);
    bool StoreNewItemInBestSlots(uint32 item_id, uint32 item_count);
    void AutoStoreLoot(uint8 bag, uint8 slot, uint32 loot_id, LootStore const &store, bool broadcast = false);
    void AutoStoreLoot(uint32 loot_id, LootStore const &store, bool broadcast = false) { AutoStoreLoot(NULL_BAG, NULL_SLOT, loot_id, store, broadcast); }
    void StoreLootItem(uint8 lootSlot, Loot *loot, GameObject *go = NULL);
    ...
    /// return count of earned currency wthin season
    ...
    */
    ...
    void RemoveItem(uint8 bag, uint8 slot, bool update, bool slotSwitch = false);
    ...
    bool HasTwoHandWeaponEquipped() const
    {
        ...
        return (offItem && offItem->GetTemplate()->InventoryType == INVTYPE_2HWEAPON) || (mainItem && mainItem->GetTemplate()->InventoryType == INVTYPE_2HWEAPON);
    }
    void SendNewItem(Item *item, uint32 count, bool received, bool created, bool broadcast = false);
    bool BuyItemFromVendorSlot(uint64 vendorguid, uint32 vendorslot, uint32 item, uint32 count, uint8 bag, uint8 slot);
    ...
    void SetSeasonalQuestStatus(Quest const *quest);
    ...
    void CurrencyAddedQuestCheck(uint32 entry, uint32 count);
    void CurrencyRemovedQuestCheck(uint32 entry, uint32 count);
    ...
    void SendQuestEvent(uint32 questId, uint64 questgiver = 0);
    ...
    static void SavePositionInDB(WorldLocation const &loc, uint16 zoneId, uint64 guid, SQLTransaction &trans);
    ...
    void SetSelection(uint64 guid)
    {
        ...
        m_arenaSpectatorFlags |= ASPEC_FLAG_TARGET;
    }
    ...
    void RemoveAll30MinCdSpellCooldowns();
    ...
    static uint32 GetArenaTeamIdFromDB(uint64 guid, uint8 type);
    ...
    uint32 GetRBGPersonalRating();
    uint32 GetRbgOrSoloQueueRatingForCapCalculation();
    ...
    int16 GetCombatRating(CombatRating cr) { return m_baseRatingValue[cr]; }
    ...
    void UpdateMastery();
    float GetMasteryAmount(uint32 spellId, SpellEffIndex effIndex);
    ...
    uint32 GetProfesionSpellBySkillId(uint32);
    ...
    TeamId GetOppositeTeamId() const { return GetTeam() == ALLIANCE ? TEAM_HORDE : TEAM_ALLIANCE; }
    ...
    uint32 GetMaxPersonalRatingRequirement(uint32 minarenaslot);
    ...
    uint32 GetBlockPercent() override { return GetUInt32Value(PLAYER_SHIELD_BLOCK); }
    ...
    bool IsOutdoor() const { return m_outdoor; }
    ...
    bool InArena() const;
    bool IsInRatedBattleground() const;
    ...
    bool IsRandomDailyBgRewarded() { return m_IsRandomDailyBgRewarded; }
    void SetRandomDailyBgRewarded(bool isRewarded);
    ...
    uint32 GetLastPetSlot() const { return m_lastPetSlot; }
    uint32 GetLastPetId() const { return m_lastPetId; }
    void SetLastPetNumber(uint32 petnumber) { m_lastpetnumber = petnumber; }
    void SetLastPetSlot(uint32 petslot) { m_lastPetSlot = petslot; }
    void SetLastPetId(uint32 petid) { m_lastPetId = petid; }

    // wrath cast sequence system
    uint32 GetWrathCastCount() const { return m_warthCastCount; }
    void SetWrathCastCount(uint32 count)
    {
        m_warthCastCount = count;
        if (m_warthCastCount > 3)
            m_warthCastCount = 1;
    }
    ...
    void UpdateAchievementCriteria(AchievementCriteriaTypes type, uint64 miscValue1 = 0, uint64 miscValue2 = 0, uint64 miscValue3 = 0, Unit *unit = NULL, GameObject *go = NULL);
    ...
    bool IsCataclysmFaction()
    {
        switch (m_ChampioningFaction)
        {
        case 1173: // Ramkahen
        case 1135: // Earthen Ring
        case 1158: // Hyjal
        case 1171: // Therazane
        case 1174: // Wildhammer Clan
        case 1172: // Dragonmaw Clan
            return true;
        default:
            return false;
        }
    ...
    */
    ...
    bool IsWatchingMovie() const { return m_IsWatchingMovie; }
    void SetWatchingMovie(bool state) { m_IsWatchingMovie = state; }
    ...
    bool m_isInLoadscreen;
    void SetPlayerIsInLoadscreen(bool active) { m_isInLoadscreen = active; }
    bool IsPlayerInLoadscreen() { return m_isInLoadscreen; }
...
public:
    ...
    // transmog custom
    TransmogMapType transmogMap;
    std::vector<uint64> GetTransmogItemList(const Player *player);

    void AddSoloGames() { _soloStats.games++; }
    void AddSoloWins() { _soloStats.wins++; }
    void AddSoloRating() { _soloStats.rating += 16; }
    void RemoveSoloRating()
    {
        if (_soloStats.rating >= 16)
            _soloStats.rating -= 16;
    }
    void LoadSoloRating();
    void SaveSoloRating();
    uint32 GetSoloWins() const { return _soloStats.wins; }
    uint32 GetSoloGames() const { return _soloStats.games; }
    uint32 GetSoloRating() const { return _soloStats.rating; }
...
protected:
    ...
    */
    ...
    bool m_IsRandomDailyBgRewarded;
    ...
    void _LoadMails(PreparedQueryResult result);
    void _LoadMailsItems(PreparedQueryResult result);
    ...
    void _LoadGuildSpells(PreparedQueryResult result);
    ...
    */
    ...
    uint32 GetConquestWeekCap(CurrencyTypesEntry const *currency) const;
...
private:
    ...
    void AdjustQuestReqCurrencyCount(Quest const *quest, QuestStatusData &questStatusData);
    ...
    SoloStats _soloStats;

    bool m_IsWatchingCinematic;

    bool m_IsWatchingMovie;
...
T Player::ApplySpellMod(uint32 spellId, SpellModOp op, T &basevalue, Spell *spell)
{
    ...
    // some special checks for buff consume priority
    bool skipMod = false;
    bool isResto = false;
    bool isBalance = false;
    bool hasPredatoryStirkes = false;
    switch (getClass())
    {
    case CLASS_WARLOCK:
    {
        if (op == SPELLMOD_CASTING_TIME)
            if ((spellInfo->Id == 686 && HasAura(17941)) || (spellInfo->Id == 6353 && HasAura(47283) && HasAura(74434)))
                skipMod = true;
        break;
    }
    case CLASS_DRUID:
    {
        if (op == SPELLMOD_NOT_LOSE_CASTING_TIME || op == SPELLMOD_COST)
        {
            skipMod = true;
            if (GetPrimaryTalentTree(GetActiveSpec()) == TALENT_TREE_DRUID_BALANCE)
                isBalance = true;

            if (GetPrimaryTalentTree(GetActiveSpec()) == TALENT_TREE_DRUID_RESTORATION)
                isResto = true;

            if (HasAura(69369)) // skip Clearcasting if caster have Predatory Strikes
                hasPredatoryStirkes = true;
        }
        break;
    }
    case CLASS_DEATH_KNIGHT:
    {
        if (op == SPELLMOD_DAMAGE)
            if (HasAura(81330) || HasAura(81332) || HasAura(81333)) // Might of the Frozen Wastes needs a weapon check
                skipMod = true;
        break;
    }
    case CLASS_HUNTER:
    {
        if (op == SPELLMOD_COST || op == SPELLMOD_COOLDOWN)
            if (spellInfo->Id == 53301 && HasAura(99060) && HasAura(56453))
                skipMod = true;
        break;
    }
    default:
    ...
    for (SpellModList::iterator itr = m_spellMods[op].begin(); itr != m_spellMods[op].end(); ++itr)
    {
        ...
        if (skipMod)
        {
            if (mod->spellId == 81330 || mod->spellId == 81332 || mod->spellId == 81333)
                if (GetTypeId() == TYPEID_PLAYER)
                    if (!ToPlayer()->HasTwoHandWeaponEquipped())
                        continue;

            if ((spellInfo->Id == 686 && (mod->spellId == 54274 || mod->spellId == 54276 || mod->spellId == 54277)) /* skip Backdraft mods if caster have shadow trance mod */
                || spellInfo->Id == 6353 && mod->spellId == 47283                                                   /* skip Soulburn casting mod if owner has Empowered Imp mod */
                || (isResto || !isResto && !isBalance) && mod->spellId == 84738                                     /* skip Nature's Focus on balance or feral spec */
                || (isBalance || !isResto && !isBalance) && mod->spellId == 84736                                   /* skip Celestial Focus on resto or feral spec */
                || spellInfo->Id == 53301 && mod->spellId == 99060)                                                 /* skip Explosive Shot casting mod if owner has Lock and Load and T12 4p*/
                continue;

            if (mod->spellId == 16870)
            {
                // skip Clearcasting consume on Ravage! cast (from Stampede)
                if (spellInfo->Id == 81170)
                    continue;

                // Predatory Strikes should take priority
                if (hasPredatoryStirkes && (spellInfo->Id == 339 || spellInfo->Id == 5176 || spellInfo->Id == 5185 || spellInfo->Id == 8936 ||
                                            spellInfo->Id == 19975 || spellInfo->Id == 40363 || spellInfo->Id == 66070 || spellInfo->Id == 78777 ||
                                            spellInfo->Id == 94358))
                    continue;
            }
        }
    ...
    T oldBase = basevalue;
    basevalue = T(float(basevalue) + float(totalflat));
    AddPct(basevalue, totalmul);
    return T(float(basevalue) - float(oldBase));
}
#endif
