...
#include "AnticheatMgr.h" //System Anti-cheat
...
void World::LoadConfigSettings(bool reload)
{
    ...
    rate_values[RATE_ARCHAEOLOGY_AMOUNT] = sConfigMgr->GetFloatDefault("Rate.Archaeology.Amount", 1.0f);
    rate_values[RATE_ARCHAEOLOGY_NEXT] = sConfigMgr->GetFloatDefault("Rate.Archaeology.Next", 1.0f);
    ...
    m_bool_configs[CONFIG_ARENA_READYMARK_ENABLED] = sConfigMgr->GetBoolDefault("ReadymarkEnabled", false);
    ...
    // class token
    m_int_configs[CONFIG_CLASS_MASK_TOKEN] = sConfigMgr->GetIntDefault("ClassEnabled", 11);
    ...
    m_int_configs[CONFIG_CURRENCY_START_VALOR_POINTS] = sConfigMgr->GetIntDefault("Currency.StartValorPoints", 0);
    if (int32(m_int_configs[CONFIG_CURRENCY_START_VALOR_POINTS]) < 0)
    {
        TC_LOG_ERROR("server.loading", "Currency.StartValorPoints (%i) must be >= 0, set to default 0.", m_int_configs[CONFIG_CURRENCY_START_VALOR_POINTS]);
    ...
    m_int_configs[CONFIG_CURRENCY_CONQUEST_POINTS_SOLO_ARENA_REWARD] = sConfigMgr->GetIntDefault("Currency.ConquestPointsSoloArenaReward", 50);
    if (int32(m_int_configs[CONFIG_CURRENCY_CONQUEST_POINTS_SOLO_ARENA_REWARD]) <= 0)
    {
        TC_LOG_ERROR("server.loading", "Currency.ConquestPointsSoloArenaReward (%i) must be > 0, set to default 50.", m_int_configs[CONFIG_CURRENCY_CONQUEST_POINTS_SOLO_ARENA_REWARD]);
        m_int_configs[CONFIG_CURRENCY_CONQUEST_POINTS_SOLO_ARENA_REWARD] = 50;
    }
    m_int_configs[CONFIG_CURRENCY_CONQUEST_POINTS_SOLO_ARENA_REWARD] *= 100; // precision mod
    ...
    m_int_configs[CONFIG_LOGDB_CLEARTIME] = sConfigMgr->GetIntDefault("LogDB.Opt.ClearTime", 0);
    ...
    m_int_configs[CONFIG_BATTLEGROUND_EXCEPTION_START] = sConfigMgr->GetIntDefault("Battleground.Exception.Starttime", 13);
    if (m_int_configs[CONFIG_BATTLEGROUND_EXCEPTION_START] > 23)
    {
        TC_LOG_ERROR("server.loading", "Battleground.Exception.Starttime (%i) can't be load. Set to 13.", m_int_configs[CONFIG_BATTLEGROUND_EXCEPTION_START]);
        m_int_configs[CONFIG_BATTLEGROUND_EXCEPTION_START] = 13;
    }

    m_int_configs[CONFIG_BATTLEGROUND_EXCEPTION_END] = sConfigMgr->GetIntDefault("Battleground.Exception.Endtime", 22);
    if (m_int_configs[CONFIG_BATTLEGROUND_EXCEPTION_END] > 23)
    {
        TC_LOG_ERROR("server.loading", "Battleground.Exception.Endtime (%i) can't be load. Set to 6.", m_int_configs[CONFIG_BATTLEGROUND_EXCEPTION_END]);
        m_int_configs[CONFIG_BATTLEGROUND_EXCEPTION_END] = 22;
    }

    m_int_configs[CONFIG_BATTLEGROUND_DECAY_TIMER] = sConfigMgr->GetIntDefault("Battleground.Random.DecayTime", 15);
    ...
    m_bool_configs[CONFIG_ARENA_SOLO_QUEUE_ENABLED] = sConfigMgr->GetBoolDefault("Arena.Soloqueue.Enable", false);
    m_bool_configs[CONFIG_ARENA_SOLO_QUEUE_ENABLED_SKIRMISH] = sConfigMgr->GetBoolDefault("Arena.Soloqueue.Enable1v1skirmish", false);
    m_bool_configs[CONFIG_ARENA_SOLO_QUEUE_ENABLED_RATING] = sConfigMgr->GetBoolDefault("Arena.Soloqueue.Enable1v1Rating", false);
    m_bool_configs[CONFIG_ARENA_SOLO_QUEUE_ENABLED_3v3] = sConfigMgr->GetBoolDefault("Arena.Soloqueue.Enable3v3", false);
    m_bool_configs[CONFIG_ARENA_LOG_EXTENDED_INFO] = sConfigMgr->GetBoolDefault("ArenaLog.ExtendedInfo", false);

    m_bool_configs[CONFIG_RATED_BATTLEGROUND_ENABLED] = sConfigMgr->GetBoolDefault("RatedBattleground.Enable", true);
    ...
    m_bool_configs[CONFIG_RESET_ARENA_COOLDOWNS_AFTER_DUEL] = sConfigMgr->GetBoolDefault("ResetCoolDownAfterDuel", false);
    ...
    // Athenas-Coins
    m_bool_configs[CONFIG_WOW_TOKEN] = sConfigMgr->GetBoolDefault("Athenas.Coins", false);
    ...
    m_int_configs[CONFIG_GUILD_CHALLENGE_DUNGEON_NEEDED] = sConfigMgr->GetIntDefault("Guild.Challenge.DungeonNedeed", 7);
    ...
    m_int_configs[CONFIG_GUILD_CHALLENGE_RAID_NEEDED] = sConfigMgr->GetIntDefault("Guild.Challenge.RaidNedeed", 1);
    m_int_configs[CONFIG_GUILD_CHALLENGE_RATEDBG_XP] = sConfigMgr->GetIntDefault("Guild.Challenge.RaidXP", 1500000);
    m_int_configs[CONFIG_GUILD_CHALLENGE_RATEDBG_GOLD] = sConfigMgr->GetIntDefault("Guild.Challenge.RaidGold", 250);
    m_int_configs[CONFIG_GUILD_CHALLENGE_RATEDBG_NEEDED] = sConfigMgr->GetIntDefault("Guild.Challenge.RaidNedeed", 3);
    ...
    m_int_configs[CONFIG_TOL_BARAD_PLR_MIN_LVL] = sConfigMgr->GetIntDefault("TolBarad.PlayerMinLvl", 80);
    ...
    m_int_configs[CONFIG_TOL_BARAD_RESTART_AFTER_CRASH] = sConfigMgr->GetIntDefault("TolBarad.CrashRestartTimer", 40);

    ///- ALLOW ZONE AND AREA VALUES CORRECTION AT STARTUP
    m_bool_configs[CONFIG_ALLOW_ZONE_AND_AREA_VALUES_CORRECTION_AT_STARTUP] = sConfigMgr->GetBoolDefault("AllowZoneAndAreaCheckForCreatureAndGameobject", false);
    ...
    //- DISABLE RAIDS NERF
    m_bool_configs[CONFIG_DISABLE_406_NERFS] = sConfigMgr->GetBoolDefault("RaidNerf.406.enable", false);
    m_bool_configs[CONFIG_DISABLE_420_NERFS] = sConfigMgr->GetBoolDefault("RaidNerf.420.enable", false);
    m_bool_configs[CONFIG_DISABLE_430_NERFS] = sConfigMgr->GetBoolDefault("RaidNerf.430.enable", false);

    m_bool_configs[CONFIG_OMFG_VERIFICATIONMGR_ENABLE] = sConfigMgr->GetBoolDefault("OMFG.VerificationMgr.Enable", false);

    m_bool_configs[CONFIG_ENABLE_IP_HISTORY] = sConfigMgr->GetBoolDefault("IPHistory.Enable", false);
    ...
    // transmogrification custom
    m_bool_configs[CONFIG_TRANSMOG_ENABLE] = sConfigMgr->GetBoolDefault("Transmogrification.EnableTransmog", false);
    m_int_configs[CONFIG_TRANSMOG_GOLD] = sConfigMgr->GetIntDefault("Transmogrification.GoldCost", 0);
    m_bool_configs[CONFIG_TRANSMOG_TOKEN] = sConfigMgr->GetBoolDefault("Transmogrification.RequireToken", false);
    m_int_configs[CONFIG_TRANSMOG_TOKEN_ID] = sConfigMgr->GetIntDefault("Transmogrification.TokenEntry", 49426);
    m_int_configs[CONFIG_TRANSMOG_TOKEN_AMOUNT] = sConfigMgr->GetIntDefault("Transmogrification.TokenAmount", 1);
    m_bool_configs[CONFIG_TRANSMOG_MIXED_ARMOR] = sConfigMgr->GetBoolDefault("Transmogrification.AllowMixedArmorTypes", false);
    m_bool_configs[CONFIG_TRANSMOG_MIXED_WEAPON] = sConfigMgr->GetBoolDefault("Transmogrification.AllowMixedWeaponTypes", false);
    m_bool_configs[CONFIG_TRANSMOG_MIXED_INVE] = sConfigMgr->GetBoolDefault("Transmogrification.AllowMixedInventoryTypes", false);
    m_bool_configs[CONFIG_TRANSMOG_MIXED_CLASS] = sConfigMgr->GetBoolDefault("Transmogrification.AllowAllClassTypes", false);
...
void World::SetInitialWorldSettings()
{
    ...
    if (!MapManager::ExistMapAndVMap(0, -6240.32f, 331.033f) || !MapManager::ExistMapAndVMap(0, -8949.95f, -132.493f) || !MapManager::ExistMapAndVMap(1, -618.518f, -4251.67f) || !MapManager::ExistMapAndVMap(0, 1676.35f, 1677.45f) || !MapManager::ExistMapAndVMap(1, 10311.3f, 832.463f) || !MapManager::ExistMapAndVMap(1, -2917.58f, -257.98f) || (m_int_configs[CONFIG_EXPANSION] && (!MapManager::ExistMapAndVMap(530, 10349.6f, -6357.29f) || !MapManager::ExistMapAndVMap(530, -3961.64f, -13931.2f))))
    {
    ...
    TC_LOG_INFO("server.loading", "Loading Spell Pre Nerf corrections...");
    sSpellMgr->LoadSpellNerfCorrections();
    ...
    TC_LOG_INFO("server.loading", ">> Generate new item guids for petitions and creatures, it will take a while");
    sObjectMgr->FillItemGuids();
    sObjectMgr->FillCreatureGuids();
    TC_LOG_INFO("server.loading", ">> %u new item guid generated in %u ms", sObjectMgr->FreeItemGuidSize() + sObjectMgr->FreeCreatureGuidSize(), GetMSTimeDiffToNow(oldMSTime));
    ...
    TC_LOG_INFO("server.loading", "Loading Instance Teleportations Destinations Data...");
    ...
    TC_LOG_INFO("server.loading", "Loading Research Dig Sites info...");
    ...
    extmail_timer.SetInterval(1 * MINUTE * IN_MILLISECONDS);
    // 1440
    ...
    TC_LOG_INFO("server.loading", "Loading Transport GOs...");
    sMapMgr->LoadTransportGOs();
    ...
    TC_LOG_INFO("server.loading", "Calculate next weekly LFR looted reset time...");
    ...
    TC_LOG_INFO("server.loading", "Calculate Guild cap reset time...");
    InitGuildResetTime();
    ...
    TC_LOG_INFO("misc", "Initializing Performance logger...");
    sPerfLog->Initialize();

    TC_LOG_INFO("misc", "Initializing Info Manager...");
    sInfoMgr->Initialize();
...
void World::LoadAutobroadcasts()
{
    ...
    do
    {
        ...
        AutobroadcastLocale &data = m_Autobroadcasts[id];
        for (uint8 i = 0; i < 7; ++i)
        {
            LocaleConstant locale = (LocaleConstant)i;
            ObjectMgr::AddLocaleString(fields[2 + i].GetString(), locale, data.Name);
        }
...
/// Send a packet to all players with dbc locales
void World::SendGlobalMessageLoc(int32 id)
{
    std::string text;
    SessionMap::const_iterator itr;
    for (itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
        ...
        {
            std::string text = "";
            int32 locale = itr->second->GetSessionDbLocaleIndex();
            if (locale >= 0)
                if (AutobroadcastLocale const *locales = GetAutobroadcastLocales(id))
                    ObjectMgr::GetLocaleString(locales->Name, locale, text);
            WorldPacket data(SMSG_NOTIFICATION, 2 + text.length());
            data.WriteBits(text.length(), 13);
            data.FlushBits();
...
namespace Trinity
{
    ...
    public:
        ...
        void operator()(WorldPacketList &data_list, LocaleConstant loc_idx)
        {
            ...
            {
                ObjectMgr::GetLocaleString(locales->Name, loc_idx, broadcastText);
                if (broadcastText.empty())
                    ObjectMgr::GetLocaleString(locales->Name, 0, broadcastText);
            }
            ...
            {
                char str[2048];
                snprintf(str, 2048, text, broadcastText.c_str());
...
void World::SendAutoBroadcast()
{
    ...
    if (abcenter == 0)
        sWorld->SendBroadcastText(LANG_AUTO_BROADCAST, id);
    else if (abcenter == 1)
    {
        sWorld->SendGlobalMessageLoc(id);
    }
    else if (abcenter == 2)
    {
        sWorld->SendBroadcastText(LANG_AUTO_BROADCAST, id);
    ...
    TC_LOG_DEBUG("misc", "Sent AutoBroadcast");
}
...
void World::InitCurrencyResetTime()
{
    ...
    if (currencytime < curTime)
        m_NextCurrencyReset = curTime;
    else
...
void World::ResetDailyQuests()
{
    ...
    CharacterDatabase.Execute(stmt);
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_GOLD_STATS);
    CharacterDatabase.Execute(stmt);

    uint8 sequence = sWorld->getWorldState(WS_HALFUS_DRAGONS);
    sWorld->setWorldState(20070, (++sequence < 11 ? sequence : 1));

    sequence = sWorld->getWorldState(WS_DAILY_ENDTIME_BOSSES);
    sWorld->setWorldState(WS_DAILY_ENDTIME_BOSSES, (++sequence < 6 ? sequence : 0));
    ...
    sAnticheatMgr->ResetDailyReportStates(); // System Anti-cheat
}
...
void World::ResetCurrencyWeekCap()
{
    ...
    CharacterDatabase.Execute("UPDATE character_currency c SET c.week_cap = c.new_cap");
    // CharacterDatabase.Execute("UPDATE `character_currency` SET `new_cap` = 0");
...
void World::ResetRandomBG()
{
    ...
    for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
        if (itr->second->GetPlayer())
            itr->second->GetPlayer()->SetRandomDailyBgRewarded(false);
...
void World::ResetGuildCap()
{
    ...
    TC_LOG_INFO("misc", "Guild Daily Cap reset. Week: %u", weekReset);
...
void World::InitPacketThrottling()
{
    ...
    opcodePerSecond[CMSG_CHANNEL_UNBAN] = 1;

    opcodePerSecond[CMSG_JOIN_CHANNEL] = 25;
    opcodePerSecond[CMSG_ATTACKSTOP] = 25;
    ...
    opcodePerSecond[CMSG_MOVE_FORCE_FLIGHT_BACK_SPEED_CHANGE_ACK] = 200;
    opcodePerSecond[SMSG_MOVE_START_SWIM] = 200;
}
