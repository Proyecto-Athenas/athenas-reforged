...
void MovementInfo::Read_CMSG_MOVE_KNOCK_BACK_ACK(ByteBuffer &buff)
{
    ...
    if (hasPitch)
        pitch = G3D::wrap(buff.read<float>(), float(-M_PI), float(M_PI));
...
void MovementInfo::Write_SMSG_MOVE_TELEPORT(ByteBuffer &buff)
{
    ...
    buff << uint32(ackCount);
    buff.WriteByteSeq(id[1]);
    buff.WriteByteSeq(id[2]);
    buff.WriteByteSeq(id[3]);
    buff.WriteByteSeq(id[5]);
    buff << float(tid ? t_pos.m_positionX : pos.m_positionX);
    buff.WriteByteSeq(id[4]);
    buff << float(tid ? t_pos.m_orientation : pos.m_orientation);
    buff.WriteByteSeq(id[7]);
    buff << float(tid ? t_pos.m_positionZ : pos.m_positionZ);
    buff.WriteByteSeq(id[0]);
    buff.WriteByteSeq(id[6]);
    buff << float(tid ? t_pos.m_positionY : pos.m_positionY);
}
...
void MovementInfo::Sanitize(Unit *target)
{
    ...
    */
    REMOVE_VIOLATING_FLAGS(HasMovementFlag(MOVEMENTFLAG_ROOT) && guid == target->GetGUID(), MOVEMENTFLAG_ROOT);
