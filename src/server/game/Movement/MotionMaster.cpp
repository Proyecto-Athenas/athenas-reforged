...
void MotionMaster::MoveChase(Unit* target, float dist, float angle, uint32 spellId, MovementSlot slot)
{
    ...
    if (_owner->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        Mutate(new ChaseMovementGenerator<Player>(target, dist, angle), slot);
    }
    else
    {
        TC_LOG_DEBUG("misc", "Creature (Entry: %u GUID: %u) chase to %s (GUID: %u)",
            ...
            target->GetTypeId() == TYPEID_PLAYER ? "player" : "creature",
            target->GetTypeId() == TYPEID_PLAYER ? target->GetGUIDLow() : target->ToCreature()->GetDBTableGUIDLow());
        Mutate(new ChaseMovementGenerator<Creature>(target, dist, angle), slot);
    }
}
...
void MotionMaster::MoveSmoothPath(G3D::Vector3 const* pathPoints, uint32 pathSize)
{
...
void MotionMaster::ForceMoveJump(float x, float y, float z, float speedXY, float speedZ, uint32 id /*= EVENT_JUMP*/)
{
    Clear(false);
    _owner->StopMoving();
    ...
    float max_height = -Movement::computeFallElevation(moveTimeHalf, false, -speedZ);
    Movement::MoveSplineInit init(_owner);
	if (id == EVENT_CAT_JUMP)
	{
		init.SetFacing(_owner->GetOrientation() * -1.0f); // original, cambio para la carga feral del druida
		//_owner->ToPlayer()->GetSession()->SendAreaTriggerMessage("prueba de carga feral");
	}		
...
// en todo el sistema de movimiento no existe un m�todo con el campo de orientaci�n por eso agrega este nuevo m�todo
void MotionMaster::MoveJump(float x, float y, float z, float o, float speedXY, float speedZ, uint32 id, bool hasOrientation)
{
	Clear(false);
	_owner->StopMoving();
	_owner->DisableSpline();
	TC_LOG_DEBUG("misc", "Unit (GUID: %u) jump to point (X: %f Y: %f Z: %f)", _owner->GetGUID(), x, y, z);
	if (speedXY <= 0.1f)
		return;
	...
	init.SetVelocity(speedXY);
	if (hasOrientation)
		init.SetFacing(o); //aqui se cambia la orientaci�n del PJ por la del objetivo
	init.Launch();
	Mutate(new EffectMovementGenerator(id), MOTION_SLOT_CONTROLLED);
}
...
void MotionMaster::MoveAroundPoint(Position const& centerPos, float radius, bool clockwise, uint8 numberOfPoints, float z)
{
