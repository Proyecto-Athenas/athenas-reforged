...
public:
    ...
    bool CalculatePath(float destX, float destY, float destZ, bool forceDest = false, bool straightLine = false, float startX = 0.0f, float startY = 0.0f, float startZ = 0.0f);
	...
	// Aps: For charge re-calculation
	void SetUnitTarget(Unit* target) { _target = target; }
	Unit* GetUnitTarget() { return _target; }
