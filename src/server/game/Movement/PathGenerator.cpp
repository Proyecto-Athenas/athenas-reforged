...
bool PathGenerator::CalculatePath(float destX, float destY, float destZ, bool forceDest, bool straightLine, float startX, float startY, float startZ)
{
    Clear();
    ...
    if (!_navMesh || !_navMeshQuery || _sourceUnit->HasUnitState(UNIT_STATE_IGNORE_PATHFINDING) ||
        (_sourceUnit->ToCreature() && _sourceUnit->ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_IGNORE_PATHFINDING) ||
...
void PathGenerator::BuildPolyPath(G3D::Vector3 const& startPos, G3D::Vector3 const& endPos)
{
    ...
    if (_polyLength)
    {
        for (; pathStartIndex < _polyLength; ++pathStartIndex)
        {
            ...
            {
                /*TC_LOG_DEBUG("maps", "Invalid poly ref in BuildPolyPath. _polyLength: %u, pathStartIndex: %u,"
...
void PathGenerator::BuildPointPath(const float *startPoint, const float *endPoint)
{
    ...
    if (!_antiLoop && !_forceDestination && (_type & PATHFIND_INCOMPLETE))
    {
        if (int(32 - GetEndPosition().x / SIZE_OF_GRIDS) != int(32 - GetStartPosition().x / SIZE_OF_GRIDS) || int(32 - GetEndPosition().y / SIZE_OF_GRIDS) != int(32 - GetStartPosition().y / SIZE_OF_GRIDS))
        {
            ...
            bool result = _complementPath->CalculatePath(GetEndPosition().x, GetEndPosition().y, GetEndPosition().z, false, false, startX, startY, startZ);
