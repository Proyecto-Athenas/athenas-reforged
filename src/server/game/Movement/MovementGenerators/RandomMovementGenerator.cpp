...
void RandomMovementGenerator<Creature>::_setRandomLocation(Creature* creature)
{
    ...
    else                                                    // 2D only
    {
        ...
        if (fabs(destZ - respZ) > travelDistZ)              // Map check
        {
            ...
            destZ = std::max<float>(map->GetHeight(creature->GetPhaseMask(), destX, destY, respZ - 2.0f, true), map->GetHeight(creature->GetPhaseMask(), destX, destY, respZ - 2.0f, true, 5.0f));

            if (fabs(destZ - respZ) > travelDistZ)
            {
                // Vmap Higher
                destZ = std::max<float>(map->GetHeight(creature->GetPhaseMask(), destX, destY, respZ+travelDistZ-2.0f, true), map->GetHeight(creature->GetPhaseMask(), destX, destY, respZ+travelDistZ-2.0f, true));
