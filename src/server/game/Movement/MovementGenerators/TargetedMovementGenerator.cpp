...
void TargetedMovementGeneratorMedium<T, D>::_setTargetLocation(T* owner, bool updateDestination)
{
    ...
    if (!owner || !owner->IsInWorld() || (owner->GetTypeId() == TYPEID_UNIT && !owner->ToCreature()))
        return;
    ...
    if (!result || (i_path->GetPathType() & PATHFIND_NOPATH) || (i_path->GetPathType() & PATHFIND_SHORTCUT))
    {
        // Cant reach target
