        ...
        switch (type)
        {
            case DTT_CHARACTER:
            {
                ...
                if (name == "")
                {
                    ...
                    if (result)
                    {
                        loginFlags = uint32(atol(getnth(line, 39).c_str()));
                        if (!(loginFlags & 0x01))              // check if the character already have rename flag (we dont overwrite factionchange or other login flags)
                            loginFlags |= 0x01;
                        std::string s = std::to_string(loginFlags);
                        char const* logFlags = s.c_str();
                        if (!changenth(line, 39, logFlags))    // characters.at_login set to "rename on login"
                            ROLLBACK(DUMP_FILE_BROKEN);
                    }
                }
