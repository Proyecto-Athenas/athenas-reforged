...
inline ZLiquidStatus GridMap::GetLiquidStatus(float x, float y, float z, uint8 ReqLiquidType, LiquidData* data /* = 0 */, float collisionHeight /* = 2.03128f */)
{
...
ZLiquidStatus Map::GetLiquidStatus(float x, float y, float z, uint8 ReqLiquidType, LiquidData* data /* = 0 */, float collisionHeight /* = 2.03128f */) const
{
...
uint16 Map::GetAreaId(float x, float y, float z, bool *isOutdoors) const
{
    uint32 mogpFlags;
    int32 adtId, rootId, groupId;
...
void Map::SendToPlayers(WorldPacket const* data) const
{
    ...
    for (MapRefManager::const_iterator itr = m_mapRefManager.begin(); itr != m_mapRefManager.end(); ++itr)
    {
        ...
        if (auto session = itr->getSource()->GetSession())
