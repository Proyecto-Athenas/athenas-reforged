...
void WorldSession::SendItemSparseDb2Reply(uint32 entry)
{
    ...
    int32 localeIndex = GetSessionDbLocaleIndex();
    if (localeIndex >= 0)
    {
    ...
    {
        if (ItemLocale const* itemLocale = sObjectMgr->GetItemLocale(proto->ItemId))
            ObjectMgr::GetLocaleString(itemLocale->Description, localeIndex, desc);
    }
...
void WorldSession::HandleReadItem(WorldPacket& recvData)
{
    ...
    if (pItem && pItem->GetTemplate()->PageText)
    {
        ...
        if (msg == EQUIP_ERR_OK)
        {
            ...
            TC_LOG_INFO("network.opcode", "STORAGE: Item page sent");
        }
        else
        {
            data.Initialize(SMSG_READ_ITEM_FAILED, 8);
            TC_LOG_INFO("network.opcode", "STORAGE: Unable to read item");
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_SELL_ITEM");
    ...
    if (!creature)
    {
        TC_LOG_DEBUG("network.opcode", "WORLD: HandleSellItemOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(vendorguid)));
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_BUYBACK_ITEM");
    uint64 vendorguid;
    uint32 slot;
    ...
    if (!creature)
    {
        TC_LOG_DEBUG("network.opcode", "WORLD: HandleBuybackItem - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(vendorguid)));
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_BUY_ITEM_IN_SLOT");
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Received CMSG_BUY_ITEM");
    uint64 vendorguid, bagGuid;
    uint32 item, slot, count;
    ...
    if (itemType == ITEM_VENDOR_TYPE_ITEM)
    {
        ...
        TC_LOG_DEBUG("network.opcode", "WORLD: received wrong itemType (%u) in HandleBuyItemOpcode", itemType);
}
...
void WorldSession::HandleListInventoryOpcode(WorldPacket& recvData)
{
    ...
    TC_LOG_DEBUG("network.opcode", "WORLD: Recvd CMSG_LIST_INVENTORY");
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: Sent SMSG_LIST_INVENTORY");

    Creature* vendor = GetPlayer()->GetNPCIfCanInteractWith(vendorGuid, UNIT_NPC_FLAG_VENDOR);
    if (!vendor)
    {
        TC_LOG_DEBUG("network.opcode", "WORLD: SendListInventory - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(vendorGuid)));
        _player->SendSellError(SELL_ERR_CANT_FIND_VENDOR, NULL, 0);
        return;
    }
    ...
    for (uint16 slot = 0; slot < rawItemCount; ++slot)
    {
        if (count == 300)
        {
            TC_LOG_ERROR("sql.sql", "Table `npc_vendor` has too many items (%u >= %i) for vendor (Entry: %u), ignore", count, MAX_VENDOR_ITEMS, vendor->GetEntry());
            break;
        }
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: CMSG_BUY_BANK_SLOT");
    ...
    */
    ...
    TC_LOG_INFO("network.opcode", "PLAYER: Buy bank bag slot, slot number = %u", slot);
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: CMSG_AUTOBANK_ITEM");
    uint8 srcbag, srcslot;
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: CMSG_AUTOSTORE_BANK_ITEM");
    uint8 srcbag, srcslot;
...
{
    TC_LOG_DEBUG("network.opcode", "Received opcode CMSG_WRAP_ITEM");
    ...
    TC_LOG_DEBUG("network.opcode", "WRAP: receive gift_bag = %u, gift_slot = %u, item_bag = %u, item_slot = %u", gift_bag, gift_slot, item_bag, item_slot);
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: CMSG_SOCKET_GEMS");
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: CMSG_CANCEL_TEMP_ENCHANTMENT");
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: CMSG_ITEM_REFUND_INFO");
    ...
    if (!item)
    {
        TC_LOG_DEBUG("network.opcode", "Item refund: item not found!");
        return;
    }
...
{
    TC_LOG_DEBUG("network.opcode", "WORLD: CMSG_ITEM_REFUND");
    uint64 guid;
    recvData >> guid;                                      // item guid
...
void WorldSession::HandleItemTextQuery(WorldPacket& recvData )
{
    ...
    TC_LOG_DEBUG("network.opcode", "CMSG_ITEM_TEXT_QUERY item guid: %u", GUID_LOPART(itemGuid));
...
void WorldSession::HandleTransmogrifyItems(WorldPacket& recvData)
{
    ...
    {
        TC_LOG_DEBUG("network.opcode", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) sent a wrong count (%u) when transmogrifying items.", player->GetGUIDLow(), player->GetName().c_str(), count);
        recvData.rfinish();
        return;
    }
    ...
    {
        TC_LOG_DEBUG("network.opcode", "WORLD: HandleTransmogrifyItems - Unit (GUID: %u) not found or player can't interact with it.", GUID_LOPART(npcGuid));
        return;
    }
    ...
    for (uint8 i = 0; i < count; ++i)
    {
        ...
        {
            TC_LOG_DEBUG("network.opcode", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify an item (lowguid: %u) with a wrong slot (%u) when transmogrifying items.", player->GetGUIDLow(), player->GetName().c_str(), GUID_LOPART(itemGuids[i]), slots[i]);
            return;
        }
        ...
        {
            TC_LOG_DEBUG("network.opcode", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify an invalid item in a valid slot (slot: %u).", player->GetGUIDLow(), player->GetName().c_str(), slots[i]);
            return;
        }
        ...
        if (newEntries[i])
        {
            ...
            if (!proto)
            {
                TC_LOG_DEBUG("network.opcode", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify to an invalid item (entry: %u).", player->GetGUIDLow(), player->GetName().c_str(), newEntries[i]);
                return;
            }
            ...
            {
                TC_LOG_DEBUG("network.opcode", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify with an invalid item (lowguid: %u).", player->GetGUIDLow(), player->GetName().c_str(), GUID_LOPART(itemGuids[i]));
                return;
            }
            ...
            {
                TC_LOG_DEBUG("network.opcode", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify with an invalid entry (entry: %u) for %u.", player->GetGUIDLow(), player->GetName().c_str(), newEntries[i], GUID_LOPART(itemGuids[i]));
                return;
            }
            ...
            {
                TC_LOG_DEBUG("network.opcode", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) failed CanTransmogrifyItemWithItem (%u with %u).", player->GetGUIDLow(), player->GetName().c_str(), itemTransmogrified->GetEntry(), itemTransmogrifier->GetEntry());
                return;
            }
...
void WorldSession::HandleReforgeItemOpcode(WorldPacket& recvData)
{
    ...
    {
        TC_LOG_DEBUG("network.opcode", "WORLD: HandleReforgeItemOpcode - Unit (GUID: %u) not found or player can't interact with it.", GUID_LOPART(guid));
    ...
    if (!item)
    {
        TC_LOG_DEBUG("network.opcode", "WORLD: HandleReforgeItemOpcode - Player (Guid: %u Name: %s) tried to reforge an invalid/non-existant item.", player->GetGUIDLow(), player->GetName().c_str());
        SendReforgeResult(false);
        return;
    }
    ...
    if (!reforgeEntry)
    {
        ...
        {
            TC_LOG_ERROR("network.opcode", "WORLD: HandleReforgeItemOpcode - Player (Guid: %u Name: %s) tried to remove reforge from non-reforged item (Entry: %u)", player->GetGUIDLow(), player->GetName().c_str(), item->GetEntry());
            SendReforgeResult(false);
            return;
        }
    ...
    if (item->GetEnchantmentId(REFORGE_ENCHANTMENT_SLOT))
    {
        TC_LOG_ERROR("network.opcode", "WORLD: HandleReforgeItemOpcode - Player (Guid: %u Name: %s) tried to reforge an already reforged item (Entry: %u)", player->GetGUIDLow(), player->GetName().c_str(), item->GetEntry());
        SendReforgeResult(false);
        return;
    }
    ...
    if (!stats)
    {
        TC_LOG_DEBUG("network.opcode", "WORLD: HandleReforgeItemOpcode - Player (Guid: %u Name: %s) tried to reforge an item with invalid reforge entry (%u).", player->GetGUIDLow(), player->GetName().c_str(), reforgeEntry);
        SendReforgeResult(false);
        return;
    }
