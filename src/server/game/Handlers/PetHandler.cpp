...
void WorldSession::HandlePetActionHelper(Unit* pet, uint64 guid1, uint32 spellid, uint16 flag, uint64 guid2, float x, float y, float z)
{
    ...
    if (!charmInfo)
    {
        TC_LOG_DEBUG("network.opcode", "WorldSession::HandlePetAction(petGuid: " UI64FMTD ", tagGuid: " UI64FMTD ", spellId: %u, flag: %u): object (entry: %u TypeId: %u) is considered pet-like but doesn't have a charminfo!",
...
void WorldSession::HandlePetSetAction(WorldPacket& recvData)
{
    ...
    if (!charmInfo)
    {
        TC_LOG_DEBUG("network.opcode", "WorldSession::HandlePetSetAction: object (GUID: %u TypeId: %u) is considered pet-like but doesn't have a charminfo!", pet->GetGUIDLow(), pet->GetTypeId());
        return;
    }
    ...
    for (uint8 i = 0; i < count; ++i)
    {
        ...
        TC_LOG_INFO("network.opcode", "Player %s has changed pet spell action. Position: %u, Spell: %u, State: 0x%X",
...
void WorldSession::HandlePetSpellAutocastOpcode(WorldPacket& recvPacket)
{
    ...
    if (!charmInfo)
    {
        TC_LOG_DEBUG("network.opcode", "WorldSession::HandlePetSpellAutocastOpcod: object (GUID: %u TypeId: %u) is considered pet-like but doesn't have a charminfo!", pet->GetGUIDLow(), pet->GetTypeId());
        return;
    }
