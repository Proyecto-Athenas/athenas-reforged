...
void WorldSession::HandleBfEntryInviteResponse(WorldPacket& recvData)
{
    ...
    TC_LOG_ERROR("misc", "HandleBattlefieldInviteResponse: GUID:" UI64FMTD " Accepted:%i", (uint64)guid, accepted);
    ...
    else
    {
        ...
        {
            _player->GetSession()->SendBfLeaveMessage(guid, BF_LEAVE_REASON_UNK1);
            bf->HandlePlayerLeaveZone(_player, 0);
        }
    }
}
...
void WorldSession::HandleBfQueueRequest(WorldPacket& recvData)
{
    ...
    TC_LOG_DEBUG("misc", "HandleBfJoinRequest: GUID:" UI64FMTD " ", (uint64)guid);
    if (Battlefield* bf = sBattlefieldMgr->GetBattlefieldByGUID(guid))
        if (bf->CanQueue() || bf->IsWarTime()) // if not possible cheater ?
            bf->InvitePlayerToQueue(_player);
}

void WorldSession::HandleBattleFieldLeaveBattle(WorldPacket& recvData)
{
    ObjectGuid guid;
    ...
    if (Battlefield* pBf = sBattlefieldMgr->GetBattlefieldByGUID(guid))
        pBf->AskToLeaveBattle(_player);
}

void WorldSession::HandleSendBattlefieldState(WorldPacket& recvData)
{
    GetPlayer()->SendBattlefieldWorldStates();
}