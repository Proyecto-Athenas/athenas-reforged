...
void WorldSession::HandleClientCastFlags(WorldPacket& recvPacket, uint8 castFlags, SpellCastTargets& targets)
{
    ...
    if (castFlags & 0x02)
    {
        ...
        if (hasMovementData)
        {
            //recvPacket.SetOpcode(Opcodes(recvPacket.read<uint32>())); est� linea cambia la estructura de los c�digos de operaci�n por lo cual no se puede procesar correctamente
...
void WorldSession::HandleCastSpellOpcode(WorldPacket& recvPacket)
{
    ...
    for (Unit::AuraEffectList::const_iterator i = aur.begin(); i != aur.end(); ++i)
    {
        if ((*i)->GetSpellInfo()->Effects[(*i)->GetEffIndex()].TriggerSpell == spellId)
        {
            forceCast = true;
