...
void WorldSession::HandleArenaTeamCreateOpcode(WorldPacket &recvData)
{
    ...
    auto type = ArenaTeam::GetTypeBySlot(slot);
    auto arenaTeam = new ArenaTeam();
    if (!arenaTeam->Create(_player->GetGUIDLow(), type, name, background, icon, iconcolor, border, bordercolor))
    {
