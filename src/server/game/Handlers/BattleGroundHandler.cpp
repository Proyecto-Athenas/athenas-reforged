...
void WorldSession::HandleBattleFieldPortOpcode(WorldPacket &recvData)
{
    ...
    if (!bracketEntry)
    {
        if (bgQueueTypeId == BATTLEGROUND_QUEUE_RB)
            bracketEntry = GetBattlegroundBracketByLevel(bg->GetMapId(), 85);
...
void WorldSession::HandleBattlemasterJoinArena(WorldPacket &recvData)
{
    ...
    // ignore if we already in BG or BG queue or freeze debuff
    if (_player->InBattleground() || _player->HasAura(9454))
        return;
