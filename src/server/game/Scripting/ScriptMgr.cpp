    ...
    for (uint32 i = 0; i < sSpellMgr->GetSpellInfoStoreSize(); ++i)
    {
        ...
        for (uint32 j = 0; j < MAX_SPELL_EFFECTS; ++j)
        {
            ...
            if (pTempSpell->Effects[j].TargetA.GetTarget() == TARGET_UNIT_SRC_AREA_ENEMY ||
                ...
                pTempSpell->Effects[j].TargetA.GetTarget() == TARGET_UNIT_SRC_AREA_ENEMY_COUNT_TARGETS)
...
void ScriptMgr::OnAddPassenger(Creature* veh, Unit* passenger, int8 seatId)
{
    /*   ASSERT(veh);
    ASSERT(passenger);
    GET_SCRIPT(CreatureScript, veh->GetScriptId(), tmpscript);
    tmpscript->OnAddPassenger(veh, passenger, seatId);*/
}
...
void ScriptMgr::OnRemovePassenger(Creature* veh, Unit* passenger)
{
    /*    ASSERT(veh);
    ASSERT(passenger);
...
void ScriptMgr::OnPlayerBindToInstance(Player* player, Difficulty difficulty, uint32 mapid, bool permanent, bool isLfg)
{
    FOREACH_SCRIPT(PlayerScript)->OnBindToInstance(player, difficulty, mapid, permanent, isLfg);
}
...
void ScriptMgr::OnPlayerEnterMap(Player* player, uint32 mapId)
{
    FOREACH_SCRIPT(PlayerScript)->OnEnterMap(player, mapId);
}
