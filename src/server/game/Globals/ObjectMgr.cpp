...
uint32 ObjectMgr::GenerateLowGuid(HighGuid guidhigh)
{
    switch (guidhigh)
    {
        case HIGHGUID_ITEM:
        {
            if (_hiItemGuid >= (0x00FFFFFE - 1))
            {
                uint32 guid = GenerateFreeItemGuid();
                ASSERT(guid != 0);
                return guid;
            }
        ...
        case HIGHGUID_UNIT:
        {
            ...
            {
                uint32 guid = GenerateFreeCreatureGuid();
                ASSERT(guid != 0);
                return guid;
            }
...
void ObjectMgr::LoadNPCSpellClickSpells()
{
    ...
    do
    {
        ...
        if (!spellinfo)
        {
            TC_LOG_DEBUG("sql.sql", "Table npc_spellclick_spells references unknown spellid %u. Skipping entry.", spellid);
            continue;
        }

        uint8 userType = fields[3].GetUInt16();
        if (userType >= SPELL_CLICK_USER_MAX)
            TC_LOG_DEBUG("sql.sql", "Table npc_spellclick_spells references unknown user type %u. Skipping entry.", uint32(userType));
...
uint32 ObjectMgr::GenerateFreeCreatureGuid()
{
    if (m_creatureGuidStore.empty())
        return 0;

    std::lock_guard<std::recursive_mutex> lock(CreatureLock);
    uint32 guid = m_creatureGuidStore.back();
    m_creatureGuidStore.pop_back();
    return guid;
}

uint32 ObjectMgr::GenerateFreeItemGuid()
{
    if (m_itemGuidStore.empty())
    {
        TC_LOG_INFO("server.loading", ">> item guid list is empty!!!");
        return 0;
    }

    std::lock_guard<std::recursive_mutex> lock(ItemLock);
    uint32 guid = m_itemGuidStore.back();
    m_itemGuidStore.pop_back();
    return guid;
}

void ObjectMgr::FillCreatureGuids()
{
    constexpr uint64 maxSafeGuid = 155000000;
    auto result = WorldDatabase.PQuery("SELECT t1.guid + 1 AS nextID FROM creature t1 LEFT JOIN creature t2 ON t1.guid + 1 = t2.guid WHERE t2.guid IS NULL and t1.guid < %u", maxSafeGuid);
    if (!result)
        return;
    ...
    do
    {
        ...
        m_creatureGuidStore.push_back(fields[0].GetUInt64());
    } 
    while (result->NextRow());

    TC_LOG_INFO("server.loading", ">> Loaded %u free creature guids.", m_creatureGuidStore.size());
}

void ObjectMgr::FillItemGuids()
{
    constexpr uint64 maxSafeGuid = 1200000;
    auto result = CharacterDatabase.PQuery("SELECT guid+1 FROM item_instance WHERE guid < 4294967263 and guid NOT IN (SELECT guid-1 FROM item_instance) ORDER BY 1");
    if (!result)
        return;
    ...
    do 
    {
        ...
        auto fields = result->Fetch();
        m_itemGuidStore.push_back(fields[0].GetUInt64());
    }
    while (result->NextRow());

    TC_LOG_INFO("server.loading", ">> Loaded %u free item guids.", m_itemGuidStore.size());
}