...
/// Key for storing temp summon data in TeleporterDataContainer
struct TeleporterGroupKey
{
    TeleporterGroupKey(uint32 teleporterEntry, TeleporterType teleporterType, uint32 teleportationId)
    : _teleporterEntry(teleporterEntry), _teleporterType(teleporterType), _teleportationId(teleportationId)
    {
    }

    bool operator<(TeleporterGroupKey const& rhs) const
    {
        // memcmp is only reliable if struct doesn't have any padding (packed)
        return memcmp(this, &rhs, sizeof(TeleporterGroupKey)) < 0;
    }

private:
    uint32 _teleporterEntry;      ///< Teleporter's entry
    TeleporterType _teleporterType;
    uint32 _teleportationId;      ///< Teleportation id
};
...
struct BroadcastText
{
    BroadcastText() : Id(0), Language(0), EmoteId0(0), EmoteId1(0), EmoteId2(0),
        EmoteDelay0(0), EmoteDelay1(0), EmoteDelay2(0), SoundId(0), CreatureTextEmote(0), Unk2(0)
    {
...
typedef std::unordered_multimap<uint32, uint32> BroadcastTextGroupContainer;
...
typedef std::map<TeleporterGroupKey, std::vector<TeleporterData> > TeleporterDataContainer;
...
class ObjectMgr
{
    ...
    public:
        ...
        {
            ObjectQuestRelationCache::const_iterator itr = _creatureInvolvedPerQuest.find(questId);
            if (itr != _creatureInvolvedPerQuest.end())
                return &itr->second;
            else
                return NULL;
        }

        std::set<uint32> const* GetInvolvedGosForQuest(uint32 const questId) const
        {
            ObjectQuestRelationCache::const_iterator itr = _goInvolvedPerQuest.find(questId);
            if (itr != _goInvolvedPerQuest.end())
                return &itr->second;
            else
                return NULL;
        }
        ...
        uint64 FreeItemGuidSize() const { return m_itemGuidStore.size(); }
        uint32 GenerateFreeItemGuid();
        void FillItemGuids();

        uint32 FreeCreatureGuidSize() const { return m_creatureGuidStore.size(); }
        uint32 GenerateFreeCreatureGuid();
        void FillCreatureGuids();

        std::recursive_mutex ItemLock;
        std::recursive_mutex CreatureLock;
        ...
        std::vector<TeleporterData> const* GetTeleporterGroup(uint32 teleporterId, TeleporterType teleporterType, uint32 teleportationId) const
        {
            TeleporterDataContainer::const_iterator itr = _teleporterDataStore.find(TeleporterGroupKey(teleporterId, teleporterType, teleportationId));
            if (itr != _teleporterDataStore.end())
                return &itr->second;
        ...
        uint32 FindBroadcastTextId(std::string const& text, uint32 soundId = 0, bool* soundMatch = NULL, bool* textMatch = NULL) const;

        std::set<uint32> GetBroadcastTextGroup(uint32 id)
        {
            std::set<uint32> groups;
            auto range = _broadcastTextGroups.equal_range(id);
            for (; range.first != range.second; ++range.first)
            {
                uint32 spell = range.first->second;
                groups.insert(spell);
            }
            return groups;
        }
    ...
    private:
        ...
        typedef UNORDERED_MAP<uint32, std::set<uint32> > ObjectQuestRelationCache;
        ObjectQuestRelationCache _creatureInvolvedPerQuest;
        ObjectQuestRelationCache _goInvolvedPerQuest;
    ...
    private:
        ...
        BroadcastTextGroupContainer _broadcastTextGroups;
        ...
        TeleporterDataContainer _teleporterDataStore;
        ...
        std::vector<uint32> m_itemGuidStore;
        std::vector<uint32> m_creatureGuidStore;
        ...
        struct BroadcastTextHelper
        {
            ...
            typedef std::multimap<uint32 /*soundId*/, BroadcastText const*> SoundMap;
            typedef std::multimap<std::reference_wrapper<std::string const> /*text*/, BroadcastText const*, ValueCompare> TextMap;
            typedef std::pair<SoundMap::const_iterator, SoundMap::const_iterator> SoundMapBounds;
            typedef std::pair<TextMap::const_iterator, TextMap::const_iterator> TextMapBounds;

            SoundMap sounds;
            TextMap maleTexts;
            TextMap femaleTexts;

            size_t size() const { return sounds.size() + maleTexts.size() + femaleTexts.size(); }
