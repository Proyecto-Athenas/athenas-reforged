...
void InfoMgr::Initialize()
{
    ...
    start = getMSTime();
    count = 0;
    if (QueryResult RBGResult = CharacterDatabase.Query("SELECT guid, win, played, rating FROM character_battleground_stats WHERE guid != 0"))
    {
        do
        {
...
void InfoMgr::UpdateCharMMR(uint32 guid, uint8 slot, uint16 mmr)
{
    ACE_Guard<ACE_Thread_Mutex> g(m_charMutex);
...
void InfoMgr::UpdateCharRBGstats(uint32 guid, uint16 win, uint16 played, uint16 rating)
{
    ACE_Guard<ACE_Thread_Mutex> g(m_charMutex);
    ...
    if (itr != m_charInfos.end())
    {
        itr->second->RbgPlayed = played;
        itr->second->RbgWon = win;
        itr->second->RBGRating = rating;
    }
}
...
void InfoMgr::AddBGInfo(SlotStoreType Slot, uint32 instanceID, uint32 team1, uint32 team2, uint16 mmr1, uint16 mmr2)
{
    ...
    m_bgStore[Slot][instanceID] = info;
}

void InfoMgr::RemoveBGInfo(SlotStoreType Slot, uint32 instanceID)
{
    ACE_Guard<ACE_Thread_Mutex> g(m_BgInfoMutex);

    m_bgStore[Slot].erase(instanceID);
}

InfoBGEntry InfoMgr::GetBgInfo(SlotStoreType Slot, uint32 InstanceID)
{
    ACE_Guard<ACE_Thread_Mutex> g(m_BgInfoMutex);

    return m_bgStore[Slot][InstanceID];
}
...
void InfoMgr::UpdatePetAuras(uint32 guid, uint64 casterGuid, uint32 spellId, uint8 effectMask, uint8 recalcMask, uint32 stacks,
                             ...
                             int32 maxDuration, int32 remDuration, uint8 charges)
{
    ...
    if (itr != petsMap.end())
    {
        if (SavedPet* pet = itr->second)
        {
            ...
            {
                ACE_Guard<ACE_Thread_Mutex> g(pet->m_auras.petAuraMapMutex);
                pet->m_auras.auraList.push_back(aura);
            }
        }
    }
}
