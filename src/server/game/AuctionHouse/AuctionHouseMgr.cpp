...
std::string AuctionEntry::BuildAuctionMailBody(uint32 lowGuid, uint32 bid, uint32 buyout, uint32 deposit, uint32 cut, time_t distrTime /*= 0*/)
{
    std::ostringstream strm;
    strm.width(16);
    ...
    strm << ':' << deposit << ':' << cut;
    if (distrTime != 0)
        strm << ":0:" << secsToTimeBitFields(distrTime);
    return strm.str();
}
