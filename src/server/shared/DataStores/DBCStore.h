...
class DBCStorage
{
    ...
    public:
        ...
        bool Load(char const* fn, SqlDbc* sql, bool force)
        {
            DBCFileLoader dbc;
                        ...
                        for (; columnNumber < sql->formatString->size(); ++columnNumber)
                        {
                            ...
                            else if ((*sql->formatString)[columnNumber] == FT_SQL_PRESENT)
                            {
                                ...
                                switch (fmt[columnNumber])
                                {
                                    ...
                                    case FT_STRING:
                                    {
                                        char const* dbString = fields[sqlColumnNumber].GetCString();
                                        size_t length = strlen(dbString);
                                        char* string = new char[length];
                                        memcpy(string, dbString, length);
                                        stringPoolList.push_back(string);
                                        *reinterpret_cast<char**>(&sqlDataTable[offset]) = string;
                                        offset += sizeof(char*);
                                        break;
                                    }
                                    case FT_NA:
