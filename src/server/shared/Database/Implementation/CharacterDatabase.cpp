...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(
        CHAR_SEL_BANINFO, "SELECT FROM_UNIXTIME(bandate), unbandate-bandate, active, unbandate, banreason, bannedby FROM character_banned WHERE guid = ? ORDER BY bandate ASC", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_SEL_ENUM_DECLINED_NAME,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_SEL_FREE_NAME, "SELECT guid, name FROM characters WHERE guid = ? AND account = ? AND (at_login & ?) = ? AND NOT EXISTS (SELECT NULL FROM characters WHERE name = ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(
        CHAR_SEL_CHARACTER_INSTANCE, "SELECT id, permanent, map, difficulty, resettime, isLFG FROM character_instance LEFT JOIN instance ON instance = id WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_CHARACTER_AURAS,
        "SELECT caster_guid, spell, effect_mask, recalculate_mask, stackcount, amount0, amount1, amount2, "
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_GUILD_LEVEL, "SELECT level from guild_member, guild WHERE guild_member.guildid = guild.guildid AND guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_ACTIONS,
        "SELECT a.button, a.action, a.type FROM character_action as a, characters as c WHERE a.guid = c.guid AND a.spec = c.activespec AND a.guid = ? ORDER BY button", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_LFRLOOTDATA, "SELECT objectEntry FROM character_lfr_looted WHERE playerGUID = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_MAILITEMS,
        "SELECT creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, randomPropertyId, durability, playedTime, text, item_guid, itemEntry, owner_guid FROM mail_items mi JOIN "
        "item_instance ii ON mi.item_guid = ii.guid WHERE mail_id = ?",
        CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_AUCTION_ITEMS,
        "SELECT creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, randomPropertyId, durability, playedTime, text, itemguid, itemEntry FROM auctionhouse ah JOIN "
        "item_instance ii ON ah.itemguid = ii.guid",
        CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_AUCTIONS,
        "SELECT id, auctioneerguid, itemguid, itemEntry, count, itemowner, buyoutprice, time, buyguid, lastbid, startbid, deposit FROM auctionhouse ah INNER JOIN item_instance ii ON ii.guid = "
        "ah.itemguid",
        CONNECTION_SYNCH);
    PrepareStatement(CHAR_INS_AUCTION,
    ...
    PrepareStatement(CHAR_UPD_ITEM_INSTANCE,
        "UPDATE item_instance SET itemEntry = ?, owner_guid = ?, creatorGuid = ?, giftCreatorGuid = ?, count = ?, duration = ?, charges = ?, flags = ?, enchantments = ?, randomPropertyId = ?, "
        "durability = ?, playedTime = ?, text = ? WHERE guid = ?",
        CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_GUILD_RANK_UPD, "UPDATE guild_rank SET rid = rid - 1 WHERE guildid = ? AND rid > ?", CONNECTION_ASYNC); // 0: uint32, 1: uint8
    ...
    PrepareStatement(CHAR_DEL_GUILD_BANK_RIGHTS_FOR_RANK_UPD, "UPDATE guild_bank_right set rid = rid - 1 WHERE guildid = ? AND rid > ?", CONNECTION_ASYNC); // 0: uint32, 1: uint8
    ...
    PrepareStatement(CHAR_UPD_GUILD_MEMBER_PROFESSION,
        "UPDATE guild_member SET profession1_level=?, profession1_skillID=?, profession1_rank=?, "
        "profession2_level=?, profession2_skillID=?, profession2_rank=? WHERE guid=?",
        CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_GUILD_MEMBER_WITHDRAW,
        ...
        "VALUES (tab7), money = VALUES (money)",
        CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHAR_DATA_FOR_GUILD,
        "SELECT c.name, c.level, c.class, c.zone, c.account, c.achievementPoint, r.standing FROM characters c LEFT JOIN character_reputation r ON c.guid = r.guid AND r.faction = 1168 WHERE c.guid = "
        "?",
        CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_INS_AURA,
        "INSERT INTO character_aura (guid, caster_guid, item_guid, spell, effect_mask, recalculate_mask, stackcount, amount0, amount1, amount2, base_amount0, base_amount1, base_amount2, maxduration, "
        "remaintime, remaincharges) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        CONNECTION_ASYNC);

    // Currency
    PrepareStatement(CHAR_SEL_PLAYER_CURRENCY, "SELECT currency, week_count, total_count, week_cap, new_cap, season_cap FROM character_currency WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_UPD_PLAYER_CURRENCY, "UPDATE character_currency SET week_count = ?, total_count = ?, week_cap = ?, new_cap = ?, season_cap = ? WHERE guid = ? AND currency = ?", CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_REP_PLAYER_CURRENCY, "REPLACE INTO character_currency (guid, currency, week_count, total_count, week_cap, new_cap, season_cap) VALUES (?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_PLAYER_CURRENCY_NEW_CAP_OVERALL, "UPDATE character_currency SET new_cap = ? WHERE guid = ? AND currency = 390", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_PLAYER_CURRENCY_NEW_CAP_ARENA, "UPDATE character_currency SET new_cap = ? WHERE guid = ? AND currency = 483", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_PLAYER_CURRENCY_NEW_CAP_RBG, "UPDATE character_currency SET new_cap = ? WHERE guid = ? AND currency = 484", CONNECTION_ASYNC);
    // Account data
    ...
    // Instance saves
    PrepareStatement(CHAR_INS_INSTANCE_SAVE, "INSERT INTO instance (id, map, resettime, difficulty, completedEncounters, hardMode, data, isLFG) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_INSTANCE_DATA, "UPDATE instance SET completedEncounters=?, data=?, hardMode=? WHERE id=?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_INSTANCE_DIFFICULTY, "UPDATE instance SET difficulty=? WHERE id=? AND isLFG = 0", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_ARENA_TEAM,
        "INSERT INTO arena_team (arenaTeamId, name, captainGuid, type, rating, backgroundColor, emblemStyle, emblemColor, borderStyle, borderColor, isSoloQueueTeam) VALUES (?, ?, ?, ?, ?, ?, ?, ?, "
        "?, ?, ?)",
        CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_RBG_STATS, "DELETE FROM character_battleground_stats WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_REP_RBG_STATS, "REPLACE INTO character_battleground_stats (guid, win, played, rating) VALUES (?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_PLAYER_ARENA_TEAMS,
    ...
    PrepareStatement(CHAR_INS_ARENA_LOG,
        "INSERT INTO arena_log (endTime, duration, winnerTeam, looserTeam, winnerRating, winnerMMR, looserRating, looserMMR, winpl1, winpl2, winpl3, winpl4, winpl5, winguid1, winguid2, winguid3, "
        "winguid4, winguid5, loosepl1, loosepl2, loosepl3, loosepl4, loosepl5, looseguid1, looseguid2, looseguid3, looseguid4, looseguid5, windmg, winheal, winkill, loosedmg, looseheal, loosekill, "
    ...
    // Character LFR looted datas
    PrepareStatement(CHAR_INS_CHARACTER_LFRLOOTDATA, "INSERT INTO character_lfr_looted(playerGUID, objectEntry) VALUES(?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_CHARACTER_LFRLOOTDATA, "DELETE FROM character_lfr_looted WHERE playerGUID = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_CHARACTER_LFRLOOTDATA_BY_ENTRY, "DELETE FROM character_lfr_looted WHERE playerGUID = ? AND objectEntry = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_ALL_CHARACTER_LFRLOOTDATA, "DELETE FROM character_lfr_looted", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CORPSES,
        "SELECT posX, posY, posZ, orientation, mapId, displayId, itemCache, bytes1, bytes2, flags, dynFlags, time, corpseType, instanceId, phaseMask, corpseGuid, guid FROM corpse WHERE corpseType <> "
        "0",
        CONNECTION_SYNCH);
    PrepareStatement(CHAR_INS_CORPSE,
        "INSERT INTO corpse (corpseGuid, guid, posX, posY, posZ, orientation, mapId, displayId, itemCache, bytes1, bytes2, flags, dynFlags, time, corpseType, instanceId, phaseMask) VALUES (?, ?, ?, "
        "?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_GM_TICKETS,
        "SELECT ticketId, guid, name, message, createTime, mapId, posX, posY, posZ, lastModifiedTime, closedBy, assignedTo, comment, response, completed, escalated, viewed, haveTicket FROM "
        "gm_tickets",
        CONNECTION_SYNCH);
    PrepareStatement(CHAR_REP_GM_TICKET,
        "REPLACE INTO gm_tickets (ticketId, guid, name, message, createTime, mapId, posX, posY, posZ, lastModifiedTime, closedBy, assignedTo, comment, response, completed, escalated, viewed, "
        "haveTicket) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_EXPIRED_AUCTIONS,
        "SELECT id, auctioneerguid, itemguid, itemEntry, count, itemowner, buyoutprice, time, buyguid, lastbid, startbid, deposit FROM auctionhouse ah INNER JOIN item_instance ii ON ii.guid = "
        "ah.itemguid WHERE ah.time <= ?",
        CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_UPD_CHARACTER,
        "UPDATE characters SET name=?,race=?,class=?,gender=?,level=?,xp=?,money=?,playerBytes=?,playerBytes2=?,playerFlags=?,"
        ...
        "equipmentCache=?,knownTitles=?,actionBars=?,grantableLevels=?,achievementPoint=?,online=? WHERE guid=?",
        CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_GROUP,
        "INSERT INTO `groups` (guid, leaderGuid, lootMethod, looterGuid, lootThreshold, icon1, icon2, icon3, icon4, icon5, icon6, icon7, icon8, groupType, difficulty, raiddifficulty) VALUES (?, ?, "
        "?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_RAID_DIFFICULTY, "UPDATE instance SET difficulty = ? where id = ? AND isLFG = 0", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_RAID_DIFFICULTY_BY_GROUP, "UPDATE instance SET difficulty = ? where id IN (select instance from group_instance where guid = ?) AND isLFG = 0", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UDP_RESTORE_DELETE_INFO,
    ...
    PrepareStatement(CHAR_UPD_CHARACTER_POSITION,
        "UPDATE characters SET position_x = ?, position_y = ?, position_z = ?, orientation = ?, map = ?, zone = ?, trans_x = 0, trans_y = 0, trans_z = 0, transguid = 0, taxi_path = '' WHERE guid = ?",
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_CHARACTER_AURA_FROZEN, "SELECT characters.name FROM characters LEFT JOIN character_aura ON (characters.guid = character_aura.guid) WHERE character_aura.spell = 9454",
        CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_SEL_INSTANCE, "SELECT data, completedEncounters, hardMode FROM instance WHERE map = ? AND id = ?", CONNECTION_SYNCH);
    PrepareStatement(
    ...
    PrepareStatement(CHAR_SEL_ARENA_TEAM_ID_BY_PLAYER_GUID,
    ...
    PrepareStatement(CHAR_SEL_CHAR_INVENTORY_COUNT_ITEM, "SELECT COUNT(itemEntry) FROM character_inventory ci INNER JOIN item_instance ii ON ii.guid = ci.item WHERE itemEntry = ? AND ci.guid = ?",
        CONNECTION_SYNCH);
    PrepareStatement(
        CHAR_SEL_MAIL_COUNT_ITEM, "SELECT COUNT(itemEntry) FROM mail_items mi INNER JOIN item_instance ii ON ii.guid = mi.item_guid WHERE itemEntry = ? AND mi.receiver = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_AUCTIONHOUSE_COUNT_ITEM, "SELECT COUNT(itemEntry) FROM auctionhouse ah INNER JOIN item_instance ii ON ii.guid = ah.itemguid WHERE itemEntry = ? AND ah.itemowner = ?",
        CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_GUILD_BANK_COUNT_ITEM,
        "SELECT COUNT(itemEntry) FROM guild_bank_item gbi INNER JOIN item_instance ii ON ii.guid = gbi.item_guid WHERE itemEntry = ? AND ii.owner_guid = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_CHAR_INVENTORY_ITEM_BY_ENTRY,
        ...
        "LEFT JOIN character_inventory cb ON cb.item = ci.bag WHERE ii.itemEntry = ? AND c.guid = ? LIMIT ?",
        CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_MAIL_ITEMS_BY_ENTRY,
        ...
        "INNER JOIN characters cs ON cs.guid = m.sender INNER JOIN characters cr ON cr.guid = m.receiver WHERE ii.itemEntry = ? AND m.receiver = ? LIMIT ?",
        CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_AUCTIONHOUSE_ITEM_BY_ENTRY,
        ...
        "ii.itemEntry = ? AND ah.itemowner = ? LIMIT ?",
        CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_GUILD_BANK_ITEM_BY_ENTRY,
        "SELECT gi.item_guid, gi.guildid, g.name FROM guild_bank_item gi INNER JOIN guild g ON g.guildid = gi.guildid INNER JOIN item_instance ii ON ii.guid = gi.item_guid WHERE ii.itemEntry = ? AND "
        "ii.owner_guid = ? "
        "LIMIT ?",
        CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_DEL_CHAR_INSTANCE_BY_MAP_DIFF,
        "DELETE FROM character_instance USING character_instance LEFT JOIN instance ON character_instance.instance = id WHERE map = ? and difficulty = ?", CONNECTION_ASYNC);
    PrepareStatement(
    ...
    PrepareStatement(CHAR_DEL_CHAR_SKILL_LANGUAGES, "DELETE FROM character_skills WHERE skill IN (98, 113, 759, 111, 313, 109, 115, 315, 673, 137, 791, 792) AND guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_CHAR_STATS,
        ...
        "spellPower, resilience, mastery) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_CHAR_GLYPHS, "INSERT INTO character_glyphs (guid, spec, glyph1, glyph2, glyph3, glyph4, glyph5, glyph6, glyph7, glyph8, glyph9) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_REP_CHAR_VOID_STORAGE_ITEM,
        "REPLACE INTO character_void_storage (itemId, playerGuid, itemEntry, slot, creatorGuid, randomProperty, suffixFactor) VALUES (?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_PET_SLOTS,
        "SELECT id, entry, owner, modelid, level, exp, Reactstate, slot, name, renamed, curhealth, curmana, abdata, savetime, CreatedBySpell, PetType FROM character_pet WHERE owner = ?",
        CONNECTION_ASYNC);
    ...
    PrepareStatement(
        CHAR_SEL_PET_SPELL_COOLDOWN, "SELECT guid, spell, time FROM pet_spell_cooldown,character_pet WHERE pet_spell_cooldown.guid = character_pet.id and character_pet.owner = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_PET_AURA,
        "SELECT guid, caster_guid, spell, effect_mask, recalculate_mask, stackcount, amount0, amount1, amount2, base_amount0, base_amount1, base_amount2, maxduration, remaintime, remaincharges FROM "
        "pet_aura,character_pet WHERE pet_aura.guid = character_pet.id and character_pet.owner = ?",
        CONNECTION_ASYNC);
    ...
    // Insert
    PrepareStatement(CHAR_INS_PET_BASE,
        "INSERT INTO character_pet (id, entry, owner, modelid, CreatedBySpell, PetType, level, exp, Reactstate, name, renamed, slot, curhealth, curmana, savetime, abdata) VALUES (?, ?, ?, ?,?, ?, ?, "
        "?,?, ?, ?, ?,?, ?, ?, ?)",
        CONNECTION_BOTH);
    ...
    PrepareStatement(CHAR_INS_PET_AURA,
        "REPLACE INTO pet_aura (guid, caster_guid, spell, effect_mask, recalculate_mask, stackcount, amount0, amount1, amount2, "
    ...
    PrepareStatement(
        CHAR_ADD_CHAR_PET_DECLINEDNAME, "INSERT INTO character_pet_declinedname (id, owner, genitive, dative, accusative, instrumental, prepositional) VALUES (?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_CHAR_DIGSITES, "REPLACE INTO character_digsites VALUES (?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_CHAR_DIGSITES, "SELECT entry, count FROM character_digsites WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHAR_CURRENT_ARTEFACTS, "SELECT entry, branchId FROM character_current_artifacts WHERE guid = ?", CONNECTION_ASYNC);

    PrepareStatement(CHAR_SEL_MAILS,
        "SELECT id, messageType, sender, receiver, subject, body, has_items, expire_time, deliver_time, money, cod, checked, stationery, mailTemplateId FROM mail WHERE receiver = ? ORDER BY id DESC",
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_MAILS_ITEMS,
        "SELECT creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, randomPropertyId, durability, playedTime, text, item_guid, itemEntry, owner_guid, mail_id FROM mail INNER "
        "JOIN mail_items ON (mail.id = mail_items.mail_id) INNER JOIN item_instance ON (mail_items.item_guid = item_instance.guid) WHERE mail.has_items = 1 AND mail.receiver = ? ORDER BY id DESC",
        CONNECTION_ASYNC);

    PrepareStatement(CHAR_INS_CHAR_RESEARCH_HISTORY, "INSERT INTO character_research_history VALUES(?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_CHAR_RESEARCH_HISTORY, "SELECT guid, projectid, count, time FROM character_research_history WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_CHAR_RESEARCH_HISTORY, "UPDATE character_research_history SET count = ? WHERE guid = ? AND projectid = ?", CONNECTION_ASYNC);

    PrepareStatement(
        CHAR_INS_GOLD_STATS, "INSERT INTO gold_stats (AverageGold, OverallGold) SELECT AVG(characters.`money`), SUM(characters.`money`) FROM characters WHERE LEVEL = 85;", CONNECTION_ASYNC);
